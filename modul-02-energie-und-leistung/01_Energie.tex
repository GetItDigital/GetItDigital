...
\begin{figure}[H]
	\centering
		...
		\alttext{Die Abbildung zeigt die Flugbahn eines Balls, der aus einer gewissen Höhe abgeworfen wird. Die Kurve beschreibt den fallenden Verlauf, gefolgt von einem Aufprallpunkt und einem kleineren, abgeflachten zweiten Sprung. Ein Koordinatensystem mit x- und y-Achse ist eingezeichnet. Die Darstellung verdeutlicht den Übergang von potenzieller Energie in kinetische Energie während des Falls sowie den Energieverlust beim Aufprall.}
...
\begin{figure}[H]
	\centering
			...
			\caption{\textbf{Beispiel der Energieberechnug.} Zerlegung der Kraft \(\vec{F}\) in eine parallele Komponente \(\vec{F}_\mathrm{p}\) und eine senkrechte Komponente \(\vec{F}_\mathrm{s}\). Nur die parallele Komponente wirkt entlang des Weges \(\vec{s}\) und überträgt Energie an das Objekt.}
			\alttext{Die Abbildung zeigt einen Block, der sich auf einer horizontalen Fläche von Punkt P eins nach Punkt P zwei bewegt. Eine Kraft wirkt schräg nach oben rechts und wird in zwei Komponenten zerlegt: eine horizontale, parallele Kraftkomponente F P, die in Bewegungsrichtung zeigt, und eine senkrechte Kraftkomponente F S, die nach oben wirkt. Der Weg s ist als horizontale Strecke zwischen P eins und P zwei eingezeichnet. Ein Winkel Alpha beschreibt die Richtung der schräg wirkenden Kraft. Die Darstellung verdeutlicht, dass nur die parallele Komponente entlang des Weges Arbeit verrichtet und Energie auf das Objekt überträgt.}
			\label{fig:E=F*s}
\end{figure}
\begin{equation}
		...
		\s{\begin{figure}[H]
		  ...
		  \end{minipage}
			...
			\caption{\textbf{Elektrische Energiespeicher.} Idealer Plattenkokondensator (links) speichert die Energie im elektrischen Feld
			 ...
			 \alttext{Die Abbildung zeigt zwei Arten elektrischer Energiespeicher. Links ist ein Plattenkondensator dargestellt. Die beiden parallelen Metallplatten sind jeweils positiv und negativ geladen, zwischen ihnen verlaufen gleichmäßig verteilte elektrische Feldlinien von der positiven zur negativen Platte. Rechts daneben ist eine Spule dargestellt, durch die ein elektrischer Strom fließt. Um die Spule herum bilden sich geschlossene magnetische Feldlinien, die den erzeugten magnetischen Feldverlauf zeigen. Die Darstellung verdeutlicht, dass ein Kondensator Energie im elektrischen Feld speichert und eine Spule Energie im magnetischen Feld.}
...
\begin{frame}
	...
	\s{\begin{figure}[H] % Hier vielleicht Beschriftung oberhalb der Batterie anbringen, damit die Bildunterschrift besser zur Geltung kommt.
		\centering
					...
					\alttext{Die Abbildung zeigt den vereinfachten Aufbau einer Batterie. Links befindet sich die Anode mit einem Minuszeichen, rechts die Kathode mit einem Pluszeichen. Zwischen beiden Elektroden liegt ein Elektrolyt, dargestellt als hellblauer Bereich mit vielen kleinen Punkten, die geladene Teilchen symbolisieren. In der Mitte befindet sich ein Separator, der die Bereiche voneinander trennt. Pfeile im Elektrolyt zeigen die Bewegungsrichtung der geladenen Teilchen von der Anode zur Kathode. Die Darstellung verdeutlicht den grundlegenden inneren Aufbau und die Funktionsweise eines chemischen Energiespeichers.}
