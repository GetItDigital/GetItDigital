...
\begin{figure}[H]
	\centering
	    \b{\includesvg[width=0.4\textwidth]{Ball}}
		\s{\includesvg[width=0.4\textwidth]{Ball}  
...
\b{\vspace*{-0.5cm}
\only<3->{
  \begin{equation} E = \int_{P_\mathrm{1}}^{P_\mathrm{2}} \vec{F} \cdot \mathrm{d}\vec{s}\end{equation}
}}
...
Das hat mit dem Impulserhaltungssatz zu tun, den lernst du später im Physikunterricht.
...
Fast, aber eben nicht ganz. Warum?
...
\speech{01_Energie1}{2}{
	...
	Wie so häufig in der Physik oder auch in der Elektrotechnik versuchen wir das mit einer Formel zu beschreiben.
...
\begin{figure}[H]
	\centering
			...
			\caption{\textbf{Beispiel der Energieberechnug.} Zerlegung der Kraft \(\vec{F}\) in eine parallele Komponente \(\vec{F}_\mathrm{p}\) und eine senkrechte Komponente \(\vec{F}_\mathrm{s}\). Nur die parallele Komponente wirkt entlang des Weges \(\vec{s}\) und verrichtet Arbeit.}
			\label{fig:E=F*s}
\end{figure}
\begin{equation}
