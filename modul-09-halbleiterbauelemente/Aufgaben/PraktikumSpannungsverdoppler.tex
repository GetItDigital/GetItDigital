\subsection{Analyse einer Spannungsverdoppler-Schaltung}
...
\Aufgabe{
    ...
    \begin{figure}[H]
        \centering
        \input{Tikz/src/FigPraktikumSpannungsverdoppler}\alttext{Das Bild zeigt einen elektrischen Schaltplan mit einer Wechselspannungsquelle auf der linken Seite, gekennzeichnet als \(u_{\mathrm{E}_i}(t)\). Von der Quelle führen zwei Leitungen nach rechts. Auf der oberen Leitung befindet sich eine Diode \(D_1\), die in Richtung der rechten Seite zeigt. Nach \(D_1\) sind zwei Kondensatoren \(C_1\) und \(C_2\) parallel zueinander geschaltet, wobei \(C_1\) oben und \(C_2\) unten angeordnet ist. Unterhalb von \(C_2\) befindet sich eine zweite Diode \(D_2\), die nach links zeigt und mit der unteren Leitung verbunden ist. Rechts neben den Kondensatoren ist ein Widerstand \(R_\mathrm{L}\) parallel zu den Kondensatoren geschaltet. Ganz rechts sind zwei Anschlusspunkte ohne weitere Bauteile. Die Spannung über dem Widerstand ist mit \(U_\mathrm{A}\) bezeichnet und durch einen Pfeil von oben nach unten dargestellt.}
...
\Loesung{
    \begin{itemize}
        ...
        \item[b)] Berechnung der Leerlauf-Ausgangsspannung $U_\mathrm{A}$
              ...
              $$ U_\mathrm{A} = 2 \cdot (\hat{U}_\mathrm{E} - U_\mathrm{D}) = 2 \cdot (\sqrt{2} \cdot U_\mathrm{E} - 0{,}7\,\mathrm{V}) $$
