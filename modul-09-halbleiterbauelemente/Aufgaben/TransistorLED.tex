...
\Aufgabe{
    ...
    \begin{figure}[H]
        ...
        \alttext{Die Abbildung zeigt eine Transistorschaltung mit einer Leuchtdiode.
        Links befindet sich eine Spannungsquelle, die eine Spannung \(U\) liefert. Von dieser Quelle aus fließt der Strom durch einen Vorwiderstand, der als \(R_\mathrm{V}\) gekennzeichnet ist weiter zur Basis eines Transistors. Der Basisstrom ist in rot als Pfeil in Stromrichtung mit \(I_\mathrm{B}\) gekennzeichnet.  
        Die Spannung zwischen Basis und Ermitter wird mit \(U_{\mathrm{BE}}\) gekennzeichnet und zwischen Kollektror und Ermitter mit \(U_{\mathrm{CE}}\)
        Auf der rechten Seite des Transistors ist ein Widerstand \(R_2\) angeschlossen, hinter dem sich eine Leuchtdiode \(D\) befindet, die in Richtung des Stromflusses betrieben wird. Der Strom ist mit \(I_\matrhm{D}\) gekennzeichnet. Die Spannung zwischen dem Eingang der Diode und dem Massepotential wird mit \(U_\mathrm{L}\) gekennzeichnet.}
...
\Loesung{
    ...
    \textbf{Antwort:}
    Die Basisspannung muss \( 2{,}72\,\mathrm{V} \) betragen, damit der Transistor durchschaltet.
