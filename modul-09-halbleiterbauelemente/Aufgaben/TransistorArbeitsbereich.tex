\subsection{Analyse des Arbeitsbereichs eines Transistors}
% Alt: Aufgabe 16
\Aufgabe{
    \label{Aufg16}
    ...
    \begin{figure}[H]
        \centering
        \input{Tikz/src/FigTransistorArbeitsbereich.tex}\alttext{Die Abbildung zeigt einen elektrischen Schaltkreis mit einem Transistor in der Mitte. Oben links befindet sich eine Leitung, an der zwei Widerstände hintereinander an die Versorgungsspannung geschaltet sind, zuerst \(R_1\) und darunter \(R_2\).  Zwischen diesen beiden Widerständen ist ein Abgriff zur Basis des Transistors geführt. Vom oberen Anschluss des Transistors (Kollektor) führt eine Leitung nach oben mit einem Pfeil, der den Strom mit \(I_\mathrm{C}\) gekennzeichnet nach unten anzeigt. Vom unteren Anschluss des Transistors (Emitter) gehen zwei parallele Leitungen ab, die jeweils über einen Widerstand führen: links durch \(R_3\) und rechts durch \(R_\mathrm{L}\). Beide Widerstände sind mit Masse verbunden, also dem Bezugspunkt für die elektrische Spannung. Rechts der Widerstände ist eine Spannung eingezeichnet die als \(U_\mathrm{A}\) gekennzeichnet wird. Der Spannungspfeil verläuft dabei von oben nach unten. Links neben den ersten beiden Widerständen verläuft eine Spannung \(U_\mathrm{B}\) ebenfalls von oben nach unten entlang der Leiterbahn bis zur Masse.}
