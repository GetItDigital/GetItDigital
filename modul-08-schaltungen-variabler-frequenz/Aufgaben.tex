...
\begin{align*}
    -6\,\mathrm{dB} &= 20\cdot \lg\left(\frac{U_a}{U_e}\right)\,\mathrm{dB} \\
...
Gegeben ist unten dargestellte Schaltung mit $L = 20\,\mathrm{mH}$, einem Widerstand $R = 150\,\Omega$ und einem Kondensator mit $C = 1\,\mu \mathrm{F}$.\\
\begin{minipage}{\textwidth}\centering
    \includegraphics[alt={Schaltbild: RLC-Serienschwingkreis mit Spannungen u1 über RLC und u2 über C.}]{Tikz/pdf/Uebungen/circ_uebung_rlcs_schwingkreis.pdf}% RLC-Reihenschaltung, u1 über RLC, u2 über C, Vgl. Prof. Ackers Übung 4.4
\end{minipage}
\begin{enumerate}
...
\end{enumerate}\vspace{-0.5cm}
\begin{align*}
    \left| \underline{F}(\mathrm{j}\omega) \right| = A(\omega) &= \frac{1}{\sqrt{(1-\omega^2 LC)^2 + (\omega R C)^2 }}
\end{align*}
Der Betrag des Frequenzganges ist in folger Abbildung dargestellt.
\begin{figure}[H]\centering
    \includegraphics[alt={Amplitudengang mit linearer Amplituden- und logarithmischer Frequenzachse. Amplitude steigt von 1 bis zu einem Maximum und fällt dann wieder ab.}]{Tikz/pdf/Uebungen/plot_uebung_rlcs_schwingkreis_amplitudengang.pdf}
\end{figure}
Die zugehörigen Werte sind:\\[5pt]
% A(100)=1.000
% A(1000)=1.009
% A(2000)=1.033
% A(3500)=1.087
...
\end{tabular}
\begin{enumerate}
    \item[d)] Die Güte eines Reihenschwingkreises gibt die Spannungsüberhöhung über $L$ beziehungsweise $C$ im Resonanzfall an.
\end{enumerate}\vspace{-0.5cm}
\begin{align*}
...
Die unten dargestellte Schaltung soll untersucht werden. 
Hierbei sollen für die Bauelemente die allgemeinen Werte $\mathrm{R}_1$, $\mathrm{R}_2$, L, C verwendet werden.
%
\begin{figure}[H]\centering% alt: u1 über R1+(L||C)+R2 und u2 über R2
    \includegraphics[alt={Schaltbild: Vierpol bestehend aus R1 und R2 in Reihe mit der Parallelschaltung aus L und C. Spannungen u1 über Gesamtschaltung und u2 über R2.}]{Tikz/pdf/Uebungen/circ_uebung_vierpol_r_lcp_r.pdf}%  u1 über R1+(L||C)+R2, u2 über R2, Vgl. Prof. Ackers Übung 4.5
\end{figure}

\begin{enumerate}
    \item[a)] Bestimmen Sie das folgende Spannungsteilerverhältnis.
\end{enumerate}\vspace{-0.5cm}
\begin{align*}
    \underline{F}(\mathrm{j}\omega) = \frac{\underline{U}_2}{\underline{U}_1}
\end{align*}
\begin{enumerate}
    \item[b)] Berechnen und skizzieren Sie den Betrag und die Phase von $\underline{F}(\mathrm{j}\omega)$. Kennzeichnen Sie charakteristische Punkte.
...
\begin{figure}[H]\centering
    \includegraphics[alt={Bodediagramm mit Amplituden- und Phasengang. Amplitudengang doppeltlogarithisch dargestellt mit Lücke (Polstelle) bei Resonanzfrequenz. Phasengang halblogarithmisch dargestellt. Phase bei niedrigen Frequenzen null Grad bis -45 Grad bei Resonanzfrequenz. Phase macht Sprung bei Resonanzfrequenz (Sprung) auf +45 Grad und fällt für hohe Frequenzen auf null Grad.}]{Tikz/pdf/Uebungen/plot_uebung_vierpol_r_lcp_r_bodediagramm.pdf}
\end{figure}
\begin{enumerate}
...
\begin{minipage}[t][0.5cm][t]{0.3\textwidth}%
    \,\\\rlap{% space, linebreak and rlap for top alignment of image
    \includegraphics[alt={Schaltbild: RLC-Parallelschwingkreis.}]{Tikz/pdf/Uebungen/circ_uebung_rlcp_schwingkreis.pdf}% RLC-Parallelschwingkreis, Ackers Übung 5.4
    }%
\end{minipage}
...
\begin{enumerate}%\itemsep1mm
    ...
    \item[b)] Skizzieren Sie den Betrag der komplexen Impedanz in halblogarithmischer Darstellung und kennzeichnen Sie charakteristische Punkte.
\end{enumerate}
\end{minipage}
\begin{enumerate}%\itemsep1mm
...
\end{enumerate}\vspace{-0.5cm}
\begin{figure}[H]\centering%
    \includegraphics[alt={Impedanzkurve in halblogarithmischer Darstellung. Die Kurve zeigt den Impedanzbetrag in abhängigkeit der Frequenz mit einem Maximum bei der Resonanzfrequenz. Für sehr niedrige und sehr hohe Frequnzen fällt der Impedanzbetrag auf null ab.}]{Tikz/pdf/Uebungen/plot_uebung_rlcp_schwingkreis_gesamtimpedanz.pdf}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[alt={Schaltbild: Zwei hintereinandergeschaltete RC-Tiefpassglieder erster Ordnung mit gleichen Widerständen und Kapazitäten.}]{Tikz/pdf/Uebungen/circ_uebung_rc_filter_2_ord.pdf}% 2x RC-Tiefpass 1. Ordnung in Reihe
\end{figure}
\begin{enumerate}%\itemsep1mm
...
\end{enumerate}\vspace{-0.5cm}
\begin{figure}[H]\centering
    \includegraphics[alt={Bodediagramm: Amplituden- und Phasengang von RC-Tiefpassfiltern erster und zweiter Ordnung. Vergleich eines einzelnen RC Gliedes mit zwei hintereinandergeschalteten RC Gliedern. Einmal mit Entkopplung und einmal ohne. Die entkoppelte Variante verhält sich im Bereich der Grenzfrequenz schärfer.}]{Tikz/pdf/Uebungen/plot_uebung_rc_filter_2_ord_bodediagramm.pdf}% RC-Tiefpass Vgl. 1. Ordnung, 2. Ordnung
\end{figure}
Die Dämpfung im Sperrbereich beträgt bei 2. Ordnung (blau) das doppelte ($40\,\mathrm{dB}/\mathrm{Dek}$)
im Vergleich zur 1. Ordnung (rot) ($20\,\mathrm{dB}/\mathrm{Dek}$).
...
\begin{minipage}{0.38\textwidth}
    \includegraphics[alt={Impedanzortskurve (erster Quadrant) und Admittanzortskurve (vierter Quadrant) zusammen dargestellt. Impedanzortskurve als Gerade parallel zur Im-Achse. Admittanzortskurve als Halbkreis auf der Re-Achse. Winkel für Zeiger von Z und Y bei gleicher Frequenz einfach negiert. }]{Tikz/pdf/Uebungen/plot_uebung_impedanzortskurve_admittanzortskurve_rls.pdf}
\end{minipage}
}
...
\begin{minipage}{0.7\textwidth}
    Die Admittanz der hier gezeigten Schaltung soll untersucht werden.\\[10pt]
    Parallel zu einer Reihenschaltung aus dem Widerstand $R_1$ und der Kapazität $C$
    befindet sich ein zweiter Widerstand $R_2$.\\[10pt]
    Die Bauteilwerte sind gegeben durch:\\[2pt] % R1=p*R0, R0=5 Ohm, R2=20 Ohm, omega*C=5 Ohm
    $R_1(p) = p \cdot R_0$,\quad $R_0 = 5\,\Omega$,\quad $R_2 = 20\,\Omega$\quad und \quad $\omega C = 0,2\,\mathrm{S}$
\end{minipage}%
\begin{minipage}{0.3\textwidth}
    % Schaltbild: R1 in Reihe zu C, zusammen parallel zu R2
    \includegraphics[alt={Schaltbild: Reihenschaltung aus R1 und Kapazität C. Beide zusammen parallel zu R2.}]{Tikz/pdf/Uebungen/circ_uebung_admittanzortskurve_rcs_rp.pdf}%
\end{minipage}
\begin{enumerate}
...
\item[] \includegraphics[alt={Admittanzortskurve der gegebenen Schaltung für p zwischen 0 und unendlich. Kurve als Halbkreis auf einer Parallelen zur Im-Achse im ersten Quadranten. B=0 für p gegen unendlich. B=maximal für p=0. Y bei 45 Grad als Zeiger eingezeichnet. }]{Tikz/pdf/Uebungen/plot_uebung_admittanzortskurve_rcs_rp.pdf}
...
\end{minipage}\hspace{0.05\textwidth}%
\begin{minipage}[t][0.5cm][t]{0.45\textwidth}%
    \,\\\rlap{% Schaltbild der Wechselstrommessbrücke, top aligned (space, linebreak and rlap in minipage)
    \includegraphics[alt={Schaltbild: Wechselstrommessbrücke mit Impedanzen Z1 bis Z4 an idealer Spannungsquelle U0. Erster Strang der Messbrücke Z1 in Reihe mit Z2. Parallel dazu zweiter Strang mit Z3 in Reihe mit Z4. Messspannung UB zwischen den Mittelpunkten der beiden Stränge.}]{Tikz/pdf/Uebungen/circ_uebung_wechselstrommessbruecke_allgemein.pdf}%
    }%
\end{minipage}
