...
\begin{frame}\ftx{\secname}
    \begin{Lernziele}{Video - Grundlagen [1/2]}
        ...
        \end{itemize}
    \end{Lernziele}
\speech{2Grundlagen1F1Videoziele}{1}{%
 Willkommen zu diesem Video über den ersten Teil der Grundlagen zum Modul Schaltvorgänge.
 ...
 Da beide Methoden auf dem Lösen von Differenzialgleichungen, kurz DGL en basieren, wird ein Verfahren zum Aufstellen von Differenzialgleichungen vorgestellt.
 Anhand von Eigenschaften und Topologie einer Schaltung werden sie lernen, Eigenschaften und Form der zugehörigen Differenzialgeichung abzuleiten.
...
\begin{table}[h]
    ...
    \s{\caption{Gruppierte Begriffe mit Definitionen in Kurzform}}
        ...
        \begin{tabular}{@{}crl@{}} % remove inter/border column space with @{}: https://tex.stackexchange.com/questions/490181/actual-split-cell-in-tabular
            ...
            & homogen$^2$     & Form gewöhnlich: $\sum a_i(t) \frac{\mathrm{d}^{i}}{\mathrm{d}t^{i}}\, f_i(y(t)) \overset{!}{=} 0$ \quad mit Störfunktion $b=0$ $\vphantom{\Big|}$\\
            & linear$^2$      & Form gewöhnlich: $\sum a_i(t) \frac{\mathrm{d}^{i}}{\mathrm{d}t^{i}}\, y(t) = b(t)$\qquad d.h. mit $\frac{\mathrm{d}^{i}}{\mathrm{d}t^{i}}\, y(t)$ linear $\vphantom{\Big|}$\\
            & Ordnung$^2$     & \makebox[7mm][l]{$n$} $[1]$ d.h. höchstens Ableitung der Ordnung $n$ in DGL \\
            \s{%
            &&\\[-4pt]%\midrule
            \multirow{ 5}{*}{\rotatebox[origin=c]{90}{\textbf{Größen}}} 
            & Abklingkonst.     & \makebox[7mm][l]{$\delta$} $[\mathrm{s}^{-1}]$ Maß für die Dämpfung linear gedämpfter Schwingungssysteme\\ %\todo{Bezug DGL} 
            & Dämpfungskonst.   & \makebox[7mm][l]{$\delta$} $[\mathrm{s}^{-1}]$ synonym zu Abklingkonstante\\ %\todo{Bezug DGL} 
    ...
    \end{table}
\speech{2Grundlagen1F2Definitionen}{1}{%
...
An dieser Stelle sei darauf hingewiesen, dass es für die verschiedenen hier gelisteten Vorgänge leicht unterschiedliche Definitionen in der Fachliteratur gibt.
...
\s{%
    ...
    \fu{\includegraphics[alt={Gezeigt ist ein Flussdiagramm zur Berechnung von Schaltvorgängen im Zeit- und Bildbereich. Ausgangspunkt ist eine DGL für f(t). Im Zeitbereich wird die DGL gelöst um den Zeitverlauf f(t) zu bestimmen. Alternativ wird die DGL in den Bildbereich transformiert und bildet dort eine algebraische Gleichung. Durch algebraisches lösen wird die Bildfunktion F(s) bestimmt. Durch Rücktransformation in den Zeitbereich wird f(t) bestimmt.}]{Tikz/pdf/plot_flussdiagramm_dgl_berechnungsmethoden.pdf}}{Flussdiagramm, Berechnungsmethoden für Schaltvorgänge\label{fig:plot:flussdiagrammbildbereich}}
...
\b{%
    ...
    \begin{center}% Gleiche Bild wie im Skript, nur kleiner skaliert
        \includegraphics[width=0.75\textwidth, alt={Gezeigt ist ein Flussdiagramm zur Berechnung von Schaltvorgängen im Zeit- und Bildbereich. Im Zeitbereich von DGL durch lösen zum Zeitverlauf. Im Bildbereich durch transformierte DGL in Form einer algebraischen Gleichung durch algebraisches lösen zur Bildfunktion und über Rücktransformation zum Zeitverlauf.}]{Tikz/pdf/plot_flussdiagramm_dgl_berechnungsmethoden.pdf}
    \end{center}
    Beide Methoden basieren auf dem Lösen von Differentialgleichungen.
}%
\speech{2Grundlagen1F3Berechnungsmethoden}{1}{%
...
Ausgangspunkt der Methode ist die Differenzialgleichung der Schaltung für die Systemgröße f.
Durch Lösen der Differenzialgleichung lässt sich der gesuchte Zeitverlauf F von Tee bestimmen.
...
\silence{2}%
\speech{2Grundlagen1F3Berechnungsmethoden}{1}{%
Bei der zweiten Methode wird die gleiche Differenzialgleichung als Ausgangspunkt genommen, allerdings wird diese im Bildbereich gelöst. 
Vorraussetzung ist, dass die Differenzialgleichung für T größer gleich 0 gilt, was bei Schaltvorgängen gegeben ist. 
...
Wie links zu sehen ist, wird die Differenzialgleichung durch die Laplace-Transformation in eine algebraische Form gebracht. 
Das bedeutet, dass Integrale und Differenziale durch die Transformation entfallen und die Gleichung, wie unten zu sehen ist, 
algebraisch nach der Bildfunktion groß F von S aufgelöst werden kann. Durch Rücktransformation wird so der Zeitverlauf klein F von Tee bestimmt. 
}%
\silence{4}%
\speech{2Grundlagen1F3Berechnungsmethoden}{1}{%
Der große Vorteil ist, dass die algebraische Umformung im Bildbereich in der Regel einfacher zu berechnen ist als das Lösen der Differenzialgleichung im Zeitbereich. 
Für die Transformation und Rücktransformation gibt es entsprechende Korrespondenztabellen.
Vereinfacht wird das Verfahren zusätzlich, bei verschwindenden Anfangsbedingungen, das heißt wenn F von Tee oder deren Ableitungen zum Schaltzeitpunkt gleich Null sind.
Andernfalls müssen die Anfangsbedingungen im Bildbereich mit eingearbeitet werden. 
Ähnliche Ansätze im Bildbereich zu rechnen finden wir auch bei der komplexen Wechselstromrechnung oder beim Aufstellen von Differenzialgleichungen.
}% Speech Ende
\end{frame}
...
\s{% Anfang Skript only
    ...
    \begin{align}
        &\text{Widerstand:}  & \ecv{u_R}(t) &= R \cdot \eci{i_R}(t) \vphantom{\frac{\mathrm{d}}{\mathrm{d}t}} &&\label{eq:grundlagen:bauteilgleichung:widerstand}\\
        &\text{Induktivität:}& \ecv{u_L}(t) &= L \cdot \frac{\mathrm{d}}{\mathrm{d}t}\, \eci{i_L}(t)          &&\label{eq:grundlagen:bauteilgleichung:induktivitaet}\\
        &\text{Kapazität:}   & \eci{i_C}(t) &= C \cdot \frac{\mathrm{d}}{\mathrm{d}t}\, \ecv{u_C}(t)          &&\label{eq:grundlagen:bauteilgleichung:kapazitaet}
    \end{align}
    ...
    \fu{\includegraphics[alt={Gezeigt ist ein Flussdiagramm zur Aufstellung von DGLen im Zeit- und Bildbereich. Im Zeitbereich werden Kirchhoffsche Regeln und Bauteilgleichungen direkt verwendet. Im Bildbereich wird die komplexe Wechselstromrechnung mit Impedanzen und Admittanzen. Bei Rücktransformation wird j omega zu d/dt.}]{Tikz/pdf/plot_flussdiagramm_dgl_aufstellen.pdf}}{Flussdiagramm, Aufstellungsmethoden für Differentialgleichungen\label{fig:plot:dglaufstellen}}
...
\b{% Folie: Schaltungselemente + Kirchhoff + Komponentengleichungen
    \includegraphics[alt={Übersichtsgrafik: Links sind Schaltungssymbole für passive und aktive Bauelemente. Passiv sind R L C und Leitungen. Aktiv sind Spannungs- und Stromquellen. Rechts sind Gleichungen. Komponentengleichungen. Für R: u*R=i. Für L: u*L=di/dt. Für C: i*C=du/dt. Darunter sind die Kirchhoffschen Regeln. Für Knoten: Summe der Ströme ist null. Für Maschen: Summe der Spannungen ist null. Schriftzug: Energiespeicher in Schaltungen führen zu Schaltvorgängen und DGLen.}]{Tikz/pdf/slide_aktive_passive_bauele2.pdf}
}%
\speech{2Grundlagen1F4DGL}{1}{%
Wie erhalten wir nun die erforderlichen Differenzialgleichungen? 
Die Schaltungen die wir in diesem Modul betrachten werden, bestehen aus den links gezeigten Komponenten. 
Das sind zum Einen die passiven Komponenten: Elektrische Widerstände R, Induktivitäten L, Kapazitäten C und Leitungen, 
zum Anderen die aktiven Komponenten: Spannungsquellen U und Stromquellen I. 
}% Speech Ende
\silence{1}%
\speech{2Grundlagen1F4DGL}{1}{%
Das Vorhandensein von Energiespeichern, das heißt von Induktivitäten oder Kapazitäten, führt zu Schaltvorgängen und Differenzialgleichungen. 
Wie rechts bei den Komponenten Gleichungen zu sehen ist, ist die Spannung U L an Induktivitäten proportional zur zeitlichen Ableitung des Stromes I L. 
...
Die Differenzialgleichung erhalten wir durch Anwendung der Komponentengleichung und der Kirchhoffschen Regeln, also der Knoten- und Maschenregel. 
}% Speech Ende
\end{frame}
...
\b{% Folie: Aufstellungsmethoden für DGL (Zeitbereich, Bildbereich)
    ...
    \includegraphics[width=\textwidth, alt={Gezeigt ist ein Flussdiagramm zur Aufstellung von DGLen im Zeit- und Bildbereich. Im Zeitbereich werden Kirchhoffsche Regeln und Bauteilgleichungen direkt verwendet. Im Bildbereich wird die komplexe Wechselstromrechnung mit Impedanzen und Admittanzen. Bei Rücktransformation wird j omega zu d/dt.}]{Tikz/pdf/plot_flussdiagramm_dgl_aufstellen.pdf}
}
\speech{2Grundlagen1F5DGLAufstellen}{1}{%
Das hier gezeigte Flussdiagramm stellt prinzipiell zwei Vorgehensweisen dar, um die gesuchte Differenzialgleichung - oben rechts im Bild - auf zu stellen.
...
Bei der zweiten Methode wird wie in der unteren Bildhälfte dargestellt, quasi die komplexe Wechselstromrechnung verwendet, um die DGL algebraisch im Bildbereich aufzustellen.
Bei der Umformung in den Bildbereich und zurück wird der Differenzialoperators d nach d T durch J Omega getauscht und umgekehrt
...
\begin{frame}[t]\ftx{\subsecname aufstellen, Beispiel}
    ...
    \begin{minipage}{\textwidth}\centering\vspace{5pt}%
        \includegraphics[alt={Schaltbild: RC Reihenschaltung mit Schalter. Dieser schaltet vom Kurzschluss zum Anschluss einer Gleichspannungsquelle Uq.}]{Tikz/pdf/circ_switchable_rc_charge_dc.pdf}
    \end{minipage}
    ...
    \begin{minipage}{\textwidth}%
        ...
        \begin{split}
            &t\geq0&\textbf{Kirchhoff:}&&&&&\textbf{Komponenten:}\\
                &\text{Knoten:}& \eci{i_R} = \eci{i_C} &= \eci{i} &&&& \eci{i_C} = C \cdot \frac{\mathrm{d}}{\mathrm{d}t}\, \ecv{u_C}\\
                &\text{Masche:}& \ecv{u_R} + \ecv{u_C} &= \ecv{u} &&&& \ecv{u_R} = R \cdot \eci{i_R}\\[+5pt]
            \hline\\[-10pt]%
            \onslide<2->{% start overlay
                && R \cdot \eci{i} + \ecv{u_C} &= \ecv{U_q} \\
                &\textbf{DGL: }& RC \cdot \frac{\mathrm{d}}{\mathrm{d}t} \ecv{u_C} + \ecv{u_C} &= \ecv{U_q} &&&&\quad\text{1. Ord.}
            }% end overlay
        ...
        \begin{align*}
            ...
            \textbf{DGL}:\quad RC\cdot \frac{\mathrm{d}}{\mathrm{d}t}\, \ecv{u_C} + \ecv{u_C} &= \ecv{U_q} \vphantom{\bigg|}
        \end{align*}
        }%
    \end{minipage}
    }% Beamer only Beispiel, Ende
\speech{2Grundlagen1F6DGLAufstellenBeispiel}{1}{%
...
oder ab dem Schaltzeitpunkt T gleich null an eine ideale Gleichspannungsquelle U Kuh angeschlossen. 
Gesucht sei die Spannung U C an der Kapazität für T größer gleich Null.
...
\speech{2Grundlagen1F6DGLAufstellenBeispiel}{2}{%
...
Dadurch erhalten wir die DGL erster Ordnung: R C mal der ersten Ableitung von U C plus U C ist gleich U Kuh. 
}% Speech Ende
\speech{2Grundlagen1F6DGLAufstellenBeispiel}{3}{%
Zum Vergleich stellen wir nun die DGL für die gleiche Schaltung mit der zweiten Methode auf. 
Da wir die DGL für U C bestimmen wollen, beginnen wir mit dem komplexen Spannungsteiler für U C. 
U C ist gleich U Kuh mal Z C geteilt durch R plus Z C. 
In dieser Form haben wir bereits nur unsere gewünschte Zielgröße U C, 
die Spannungsquelle U Kuh als unsere Störquelle und die Variablen R und C, die unsere konstanten Koeffizienten bilden.
Durch algebraische Umformung stellen wir die Gleichung um. 
Auf die linke Seite kommen erstens alle J Omega Terme, zweitens unsere Zielgröße U C in komplexer Form. 
Wichtig ist, dass die J Omega Terme im Zähler stehen, damit diese am Ende zu Differentialoperatoren umgewandelt werden können.
Auf die rechte Seite kommt unser komplexes U Kuh als Störterm. 
Durch Rücktransformation in den Zeitbereich erhalten wir unsere Differenzialgleichung. 
}% Speech Ende
\end{frame}
...
\b{%
    \centering%
    \includegraphics[alt={Links ist eine Übersicht über von Schaltungselementen gezeigt. Passive Elemente sind R L C und Leitungen. Aktive sind Strom- und Spannungsquellen. Rechts sind Systemeigenschaften zu DGL-Eigenschaften zugeordnet. Energiespeicher führen zu DGL. Lineares System zu linearer DGL. Zeitinvariantes System zu konstanten Koeffizienten. Unten ist eine DGL gegeben. Summe für i=0 bis i=n von a_i mal i-te Ableitung von y(t) ist gleich b(t). Mit n als Ordnung. a_i als konstante Koeffizienten. y(t) als Schaltungsgröße. b(t) als Störgröße.}]{Tikz/pdf/slide_aktive_passive_bauele.pdf}
}%
\speech{2Grundlagen1F7DGLEigenschaften}{1}{%
Hier sehen wir eine kurze Zusammenfassung von Schaltungskomponenten, System- und DGL Eigenschaften und der Form der DGL. 
Links sind die linearen, zeitinvarianten Schaltungskomponenten, aus denen sich unsere Schaltungen zusammen setzen. 
Oben rechts sehen wir, welche Eigenschaften der DGL sich aus den Systemeigenschaften der Schaltung ableiten lassen. 
Beinhaltet die Schaltung Energiespeicher, das heißt Induktivitäten oder Kapazitäten, so erhalten wir eine DGL. 
}% Speech Ende
\silence{1}%
\speech{2Grundlagen1F7DGLEigenschaften}{1}{%
Die Zusammenhänge von Schaltungstopologie und Eigenschaften und Form der DGL zu verstehen hilft uns zum Beispiel 
beim Aufstellen der DGL, da wir wissen, welche Form diese annehmen muss. 
Außerdem kann uns dieses Wissen später helfen das Systemverhalten zum Teil direkt aus der Form der DGL abzuleiten.
Wenn alle Komponenten einer Schaltung linear sind, ist auch die zugehörige DGL linear. 
Bei allen Komponenten gehen wir von Zeitinvarianz aus, das heißt zum Beispiel, dass der elektrische Widerstand R sich nicht über die Zeit verändert.
Daraus lässt sich ableiten, die Konstanten A I unserer Differenzialgleichung sind konstant, da diese nur von den Werten für R, L und oder C abhängen. 
}% Speech Ende
\silence{2}%
\speech{2Grundlagen1F7DGLEigenschaften}{1}{%
Unten sehen wir die allgemeine Form einer gewöhnlichen, linearen, inhomogenen Differenzialgleichung der Ordnung N mit konstanten Koeffizienten.
Die DGL ist gewöhnlich, weil sie nur von einer Variable, hier der Zeit T, abhängt. 
Die DGL ist linear, weil nur lineare Terme enthalten sind. 
Die DGL ist inhomogen, da sie eine Störgröße B von Tee auf der rechten Seite der Gleichung enthält. 
...
Ohne Anregung, zum Beispiel beim Entladen einer Kapazität über einen Widerstand, entfällt die Störgröße auf der rechten Seite und wir erhalten eine homogene DGL.
Die DGL hat die Ordnung N, da die Ente Ableitung die höchst auftretende Ableitung der Schaltungsgröße Ypsilon ist.
}% Speech Ende
\end{frame}
...
\s{%
    ...
    \begin{figure}[H]\centering% Bilder vertikal angeordnet
        \begin{subfigure}{0.24\textwidth}\centering
            \includegraphics[alt={Schaltbild: Zweitor aus RC-Serienglied. Eingang über R und C. Ausgang über C.}]{Tikz/pdf/circ_example_ordnung_01.pdf}
        \end{subfigure}
        \caption*{1 Energiespeicher $\rightarrow$ DGL 1. Ord.}
        \begin{subfigure}{0.24\textwidth}\centering
            \includegraphics[alt={Schaltbild: RLC-Parallelschwingkreis.}]{Tikz/pdf/circ_example_ordnung_02.pdf}
        \end{subfigure}
        \caption*{2 Energiespeicher $\rightarrow$ DGL 2. Ord.}
        \begin{subfigure}{0.24\textwidth}\centering
            \includegraphics[alt={Schaltbild: Serienschaltung aus R und L1. Parallel dazu L2.}]{Tikz/pdf/circ_example_ordnung_03.pdf}
        \end{subfigure}
        \caption*{2 Energiespeicher $\rightarrow$ DGL 2. Ord.}
        \begin{subfigure}{0.24\textwidth}\centering
            \includegraphics[alt={Schaltbild: Parallelschaltung aus R1 und L. In Reihe mit Parallelschaltung aus R2 und C.}]{Tikz/pdf/circ_example_ordnung_04.pdf}
        \end{subfigure}
        \caption*{2 Energiespeicher $\rightarrow$ DGL 2. Ord.}
...
\b{%
    \begin{figure}[H]\centering% Bilder horizontal angeordnet
        \begin{subfigure}{0.24\textwidth}\centering
            \includegraphics[width=\textwidth, alt={Schaltbild: Zweitor aus RC-Serienglied. Eingang über R und C. Ausgang über C.}]{Tikz/pdf/circ_example_ordnung_01.pdf}
            \caption*{1 En.sp. $\rightarrow$ 1. Ord.}
        \end{subfigure}
        \begin{subfigure}{0.24\textwidth}\centering
            \includegraphics[width=\textwidth, alt={Schaltbild: RLC-Parallelschwingkreis.}]{Tikz/pdf/circ_example_ordnung_02.pdf}
            \caption*{2 En.sp. $\rightarrow$ 2. Ord.}
        \end{subfigure}
        \begin{subfigure}{0.24\textwidth}\centering
            \includegraphics[width=\textwidth, alt={Schaltbild: Serienschaltung aus R und L1. Parallel dazu L2.}]{Tikz/pdf/circ_example_ordnung_03.pdf}
            \caption*{2 En.sp. $\rightarrow$ 2. Ord.}
        \end{subfigure}
        \begin{subfigure}{0.24\textwidth}\centering
            \includegraphics[width=\textwidth, alt={Schaltbild: Parallelschaltung aus R1 und L. In Reihe mit Parallelschaltung aus R2 und C.}]{Tikz/pdf/circ_example_ordnung_04.pdf}
            \caption*{2 En.sp. $\rightarrow$ 2. Ord.}
        \end{subfigure}
        \label{fig:plot:beispielordnungdgl}
    \end{figure}
...
\speech{2Grundlagen1F8DGLOrdnung}{1}{%
Die Ordnung einer Differenzialgleichung entspricht exakt der Menge an nicht zusammenfassbarer Energiespeicher in der Schaltung.
Auf dieser Folie sehen Sie einige Beispielschaltungen und die Anzahl der Energiespeicher und resultierenden Ordnung der DGL.
Sind beispielsweise zwei Induktivitäten unmittelbar in Serie oder zwei Kapazitäten unmittelbar parallel zueinander, 
...
Bei den hier gezeigten Beispielen haben die rechten drei Beispiele alle zwei Energiespeicher und somit eine DGL zweiter Ordnung.
...
\begin{frame}\ftx{\subsecname}
    ...
    \end{itemize}
    \end{Lernziele}
\speech{2Grundlagen2F01Lernziele}{1}{%
Willkommen zum zweiten Grundlagen Video des Moduls Schaltvorgänge.
In diesem Video behandeln wir das letzte Grundlagen Unterkapitel Berechnungsverfahren im Zeitbereich.
...
\speech{2Grundlagen2F02Anfangsbedingungen}{1}{%
Differenzialgleichungen haben prinzipiell unendlich viele Lösungen. 
}% Speech Ende
\speech{2Grundlagen2F02Anfangsbedingungen}{2}{%
Um das Zeitverhalten einer gesuchten Schaltungsgröße zu bestimmen, müssen wir die allgemeine Lösung einer DGL auf eine konkrete, distinktive Lösung eingrenzen. 
Zur Eingrenzung auf diese eindeutige Lösung bedarf es für eine DGL n ter Ordnung exakt n Anfangsbedingungen.
}% Speech Ende
\speech{2Grundlagen2F02Anfangsbedingungen}{3}{%
Eine Anfangsbedingung ist der Wert einer beliebigen Schaltungsgröße Ypsilon von Tee
...
\speech{2Grundlagen2F02Anfangsbedingungen}{4}{%
Da Kapazitäten Energie in Form elektrischer Energie speichern, ist die Spannung 
an einer Kapazität stetig und kann sich nicht schlagartig ändern.
Da Induktivitäten Energie in Form magnetischer Energie speichern, ist der Strom 
durch eine Induktivität stetig und kann sich ebenfalls nicht schlagartig ändern. 
}% Speech Ende
\speech{2Grundlagen2F02Anfangsbedingungen}{5}{%
...
\s{% Zerlegung: flüchtig, eingeschwungen
    ...
    \begin{figure}[H]\centering
        \includegraphics[alt={Plot: Zeitverlauf einer Größe y(t) zerlegt in ye und yf. Dabei nähert sich y asymptotisch dem konstanten Wert ye an. Die Differenz der beiden yf geht gegen null.}]{Tikz/pdf/plot_example_fluechtig_eingeschwungen.pdf}
        \caption{Ausgleichsvorgang zerlegt in eingeschwungenen und flüchtigen Zustand}
        \label{plot:example:fluechtig_eingeschwungen}
    \end{figure}%
}%
\b{%
    \includegraphics[alt={Plot: Zeitverlauf einer Größe y(t) zerlegt in ye und yf. Dabei nähert sich y asymptotisch dem konstanten Wert ye an. Die Differenz der beiden yf geht gegen null.}]{Tikz/pdf/plot_example_fluechtig_eingeschwungen_def.pdf}%
    \pause%
}%
\s{%
    ...
    %\begin{align*}
    %    \sum_{i=0}^{n} a_i \cdot \frac{\mathrm{d}^{i}}{\mathrm{d}t^{i}}\, y_{\mathrlap{\mathrm{h}}\hphantom{h}}(t) &= 0 \\
    %    \sum_{i=0}^{n} a_i \cdot \frac{\mathrm{d}^{i}}{\mathrm{d}t^{i}}\, y_{\mathrlap{\mathrm{p}}\hphantom{h}}(t) &= b(t) \\    
    %\end{align*}
...
\b{%
    ...
    \begin{split}
        \sum a_i\cdot \frac{\mathrm{d}^{i}}{\mathrm{d}t^{i}}\ y_{\hphantom{e}} &= b(t) \\
    \mathllap{\huge\mathbf - \hspace{5pt}} 
        \ec[blue]{\sum a_i \cdot \frac{\mathrm{d}^{i}}{\mathrm{d}t^{i}}\ y_{\mathrlap{\mathrm{e}}\hphantom{e}}} &\ec[blue]{{}= b(t)}\\[+5pt]
    \hline\\[-15pt]
    \mathllap{\huge\mathbf = \hspace{5pt}} 
        \ec[red]{\sum a_i \cdot \frac{\mathrm{d}^{i}}{\mathrm{d}t^{i}}\ y_{\mathrlap{\mathrm{f}}\hphantom{e}}} &\ec[red]{{}= 0}
    \end{split}\end{aligned}
    \end{equation*}
}%
\end{columns}
\speech{2Grundlagen2F03Zerlegung}{1}{%
Um die Differenzialgleichung zu lösen hilft es, die gesuchte Systemgröße während des Schaltvorganges 
in einen eingeschwungenen Zustand und einen flüchtigen Zustand zu zerlegen.
Der eingeschwungene Zustand Ypsilon E ist der stationäre Zustand, den unsere Systemgröße Ypsilon während des Schaltvorganges anstrebt. 
Subtrahieren wir Ypsilon E von Ypsilon, bleibt der zeitlich abklingende flüchtige Zustand Ypsilon F übrig.
}% Speech Ende
\speech{2Grundlagen2F03Zerlegung}{2}{%
In diesem Beispiel Schaltvorgang nähert sich Ypsilon exponentiell einem konstanten Grenzwert an. 
Dieser durch die DC Anregung bedingte konstante Endwert von Ypsilon ist unser eingeschwungener Zustand Ypsilon E. 
Der abklingende Exponenzialverlauf Ypsilon F ist unser flüchtiger Zustand. 
Wie wir gleich sehen werden, lassen sich beide Zustände einzeln gut berechnen,
sodass wir Ypsilon durch die Überlagerung beider Zustände bestimmen können.
}% Speech Ende
\speech{2Grundlagen2F03Zerlegung}{3}{%
Mathematisch entspricht diese Zerlegung genau der Zerlegung von Ypsilon 
in eine partikuläre Lösung Ypsilon P der DGL und die allgemeine Lösung der homogenen DGL Ypsilon H, 
im Folgenden kurz homogene Lösung genannt.
Da Ypsilon für große T gleich Ypsilon E ist, muss auch Ypsilon E die DGL erfüllen und zwar für alle Tee.
Der eingeschwungene Zustand ist deshalb eine partikuläre Lösung der DGL. 
}% Speech Ende
\speech{2Grundlagen2F03Zerlegung}{4}{%
Nehmen wir nun die allgemeine DGL und subtrahieren von dieser die DGL mit eingesetzter Lösung Ypsilon E,
so erhalten wir die homogene DGL für Ypsilon F ohne Störterm.
Deshalb ist unser Ypsilon E die partikuläre Lösung unserer DGL und Ypsilon F die homogene Lösung unserer DGL.
}% Speech Ende
\end{frame}
...
\b{%
    \begin{align*}
        &\textbf{Homogene DGL:}& 
            \sum_{i=0}^{n} a_i \cdot \frac{\mathrm{d}^{i}}{\mathrm{d}t^{i}}\, y_{\mathrm{h}} &= 0 &
...
\s{%
    ...
    %\begin{align*}
    %    \frac{\mathrm{d}}{\mathrm{d}t} y_{\mathrm{h}} &= \lambda \cdot y_{\mathrm{h}} \\
    ...
    \begin{align}
        %\sum_{i=0}^{n} a_i \cdot \frac{\mathrm{d}^{i}}{\mathrm{d}t^{i}}\, y_{\mathrm{h}}(t) &= 0 \nonumber\\
        \sum_{i=0}^{n} a_i \cdot \frac{\mathrm{d}^{i}}{\mathrm{d}t^{i}}\, K \cdot \mathrm{e}^{\lambda t} &= 0 \nonumber\\[2pt]
    ...
    \begin{bsp}{Linear unabhängige Lösungen einer DGL}{grundlagen:linearunabhaengige_loesungen_homo_dgl}
        ...
        \begin{equation*}
            %\frac{\mathrm{d}^{5}}{\mathrm{d}t^{5}} y(t) + 7 \frac{\mathrm{d}^{4}}{\mathrm{d}t^{4}} y(t) + 26 \frac{\mathrm{d}^{3}}{\mathrm{d}t^{3}} y(t) + 62 \frac{\mathrm{d}^{2}}{\mathrm{d}t^{2}} y(t) + 8 \frac{\mathrm{d}}{\mathrm{d}t} y(t) + 75 y(t) = 0
            \frac{\mathrm{d}^{\ecr{5}}}{\mathrm{d}t^{\ecr{5}}} y(t) + \ecb{7} \frac{\mathrm{d}^{\ecr{4}}}{\mathrm{d}t^{\ecr{4}}} y(t) + \ecb{26} \frac{\mathrm{d}^{\ecr{3}}}{\mathrm{d}t^{\ecr{3}}} y(t) + \ecb{62} \frac{\mathrm{d}^{\ecr{2}}}{\mathrm{d}t^{\ecr{2}}} y(t) + \ecb{8} \frac{\mathrm{d}}{\mathrm{d}t} y(t) + \ecb{75} y(t) = 0
        \end{equation*}
...
\speech{2Grundlagen2F04HomogeneLoesung}{1}{%
Kommen wir zur Berechnung der homogenen Lösung. 
Die homogene DGL erhalten wir, indem wir in unserer DGL den Störterm gleich Null setzen. 
}% Speech Ende
\speech{2Grundlagen2F04HomogeneLoesung}{2}{%
Als erster Schritt bestimmen wir die Eigenwerte Lambda i unserer DGL.
Dafür benötigen wir das charakteristische Polynom.
Dieses erhalten wir, indem wir den Exponenzialansatz K mal E hoch Lambda T für Ypsilon h einsetzen. 
Durch i-faches Differenzieren ergeben sich die i-fachen Potenzen von Lambda als Koeffizienten vor dem Exponenzialansatz. 
Der Exponenzialterm wird ausgeklammert und eliminiert.  
Als Abkürzung kann das charakteristische Polynom auch direkt aus der DGL abgelesen werden, 
hierfür wird jede i-te Ableitung von Ypsilon h durch den Term Lambda hoch i ersetzt.
Die Eigenwerte erhalten wir durch Bestimmung der Nullstellen des charakteristischen Polynoms.
}% Speech Ende
\speech{2Grundlagen2F04HomogeneLoesung}{3}{%
Die homogene Lösung ist eine Linearkombination von exakt N linear unabhängigen Lösungen der homogenen DGL.
...
Hierfür wird der Exponenzialansatz jeder weiteren gleichen Nullstelle mit dem Faktor T hoch J minus eins multipliziert. 
Also T hoch null für die erste Nullstelle, T hoch eins für die zweite Nullstelle und so weiter.
Achtung Komplexe Eigenwerte sind möglich, können aber nur paarweise komplex-konjugiert auftreten. 
...
\speech{2Grundlagen2F04HomogeneLoesung}{4}{%
...
\speech{2Grundlagen2F05HomogeneLoesungBeispiel}{1}{%
Auf dieser Folie sehen wir exemplarisch ein Beispiel für eine homogene DGL fünfter Ordnung. 
}% Speech Ende
\speech{2Grundlagen2F05HomogeneLoesungBeispiel}{2}{%
Aus den konstanten Koeffizienten lesen wir direkt die Koeffizienten für das charakteristische Polynom ab.
}% Speech Ende
\speech{2Grundlagen2F05HomogeneLoesungBeispiel}{3}{%
Hier sind zum Verdeutlichen die Koeffizienten in blau und die Potenzen von Lambda beziehungsweise der Ableitungen von Ypsilon von Tee in rot hervorgehoben. 
Die Eigenwerte Lambda eins bis Lambda fünf erhalten wir nun durch Bestimmung der Nullstellen des charakteristischen Polynoms.
}% Speech Ende
\speech{2Grundlagen2F05HomogeneLoesungBeispiel}{4}{%
...
Für die einfache Nullstelle Lambda 1 wählen wir den ersten Lösungsansatz mit dem einfachen Exponenzialterm K eins mal E hoch Lambda eins mal t. 
Für die zwei doppelten Nullstellen Lambda zwei und drei, sowie Lambda vier und fünf wählen wir den Exponenzialansatz mit Variation der Konstanten.
}% Speech Ende
\speech{2Grundlagen2F05HomogeneLoesungBeispiel}{5}{%
Hier sehen wir die resultierende allgemeine Lösung mit eingefärbten Koeffizienten K eins bis K fünf. 
Für die doppelten Nullstellen erhalten wir K zwei plus K drei mal Tee, beziehungsweise K vier plus K fünf mal Tee als Koeffizienten für Exponenzialtermen.
}% Speech Ende
\speech{2Grundlagen2F05HomogeneLoesungBeispiel}{6}{%
...
Der Realteil der komplex konjugierten Nullstellen, hier die in orange hervorgehobene minus eins, 
bestimmt die Abklingrate des Exponenzialterms, E hoch Realteil von Lambda mal Tee, hier E hoch minus Tee. 
Der Imaginärteil der komplex konjugierten Nullstellen, hier die in magenta hervorgehobene plus beziehungsweise minus zwei, 
bestimmt die Kreisfrequenz des Sinus- und des Kosinusterms, hier Sinus beziehungsweise Kosinus von zwei Tee.
...
\s{%
    ...
    \begin{align*}
        y(t) &= \hat{Y} \cdot \cos(\omega t + \varphi) = \Re\left\{ \underline{\hat{Y}} \cdot \mathrm{e}^{\mathrm{j} \omega t} \right\} &
            ...
            \vphantom{\bigg|}\\
    \end{align*}
    Ableitungen nach der Zeit $\frac{\mathrm{d}}{\mathrm{d}t}$ werden im komplexen Zahlenraum durch $\mathrm{j} \omega$ ersetzt:
    \begin{align*}
    % for preview comment out \intertext{}
    %\intertext{Ableitungen nach der Zeit $\frac{\mathrm{d}}{\mathrm{d}t}$ werden im komplexen Zahlenraum durch $\mathrm{j} \omega$ ersetzt:}
        \frac{\mathrm{d}}{\mathrm{d}t} y(t) &= \hat{Y} \cdot \omega \cdot \cos(\omega t + \varphi + \frac{\pi}{2}) &
            &\Longleftrightarrow& 
    ...
    \begin{equation}
        \sum_{i=0}^n a_i \cdot \left(\frac{\mathrm{d}}{\mathrm{d}t}\right)^i y_{ac} = b_{ac}
...
\speech{2Grundlagen2F06PartikulaereLoesung}{1}{%
Kommen wir zur Berechnung der partikulären Lösung Ypsilon P, das heißt zur Berechnung des eingeschwungenen Zustandes Ypsilon E.
Im eingeschwungenen Zustand verhält sich unsere Schaltung stationär. 
In linearen, zeitinvarianten Systemen bedeutet das, dass unser Ypsilon P 
bei DC-Anregung eine Gleichgröße mit konstantem Wert ist und
bei AC-Anregung eine sinusförmige Wechselgröße mit gleicher Frequenz wie die Anregung.
Zur Bestimmung wenden wir die bekannten Methoden zur Netzwerkberechnung an. 
Dazu gehören zum Beispiel Knoten- und Maschenregeln, daraus abgeleitet Spannungs- und Stromteiler in realer oder komplexer Form. 
...
\speech{2Grundlagen2F07Zusammenfassung}{1}{%
Unsere bisherigen Kenntnisse lassen sich nun zu einem Berechnungsverfahren zur Bestimmung von Differenzialgleichungen im Zeitbereich zusammen fassen. 
Mit dem gezeigten Verfahren in fünf Schritten werden wir alle Schaltvorgänge im folgenden Kapitel berechnen.
Als erster Schritt wird die Differenzialgleichung aufgestellt. 
...
Handelt es sich bei Schritt eins um eine homogene Differenzialgleichung entfallen die Schritte drei und vier mit Ypsilon E gleich Null.
}% Speech Ende
\end{frame}
