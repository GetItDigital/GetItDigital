...
\begin{Lernziele}{\v{Video - }Einführung}
	...
	\end{itemize}
\end{Lernziele}
\speech{1Einfuehrung1Lernziele}{1}{%
...
\silence{1}
\speech{1Einfuehrung1Lernziele}{1}{%
Zunächst werden wir den Begriff Schaltvorgang näher erläutern und Analogien herausarbeiten.
Hierfür werden wir uns Ausgleichsvorgänge im Allgemeinen mit einfachen Beispielen aus der Physik anschauen.
Im weiteren Verlauf werden wir Ursachen, den typischen Ablauf und eine grobe Definition von Schaltvorgängen kennenlernen.
Anhand von Beispielen für Schaltvorgänge, werden Sie Herausforderungen und Möglichkeiten kennenlernen, die sich aus Schaltvorgängen ergeben.
Zudem lernen Sie das Schaltverhalten idealer und realer Schalter kennen und zu unterscheiden.
}% Speech Ende
\end{frame}
...
\s{%
	...
	\begin{figure}[H]\centering
		...
		\includegraphics[width=0.3\textwidth, alt={Gemäldeausschnitt eines Kochtopfes über einem Lagerfeuer}]{Bilder/Ausschnitt_A_Peasant_Family_Cooking_over_a_Campfire_by_Bartolomeo_Pinelli_CC0_1.0.png} % 320x319px
...
\b{%
	\begin{minipage}{\textwidth}% Obere Hälfte
		\begin{minipage}{0.48\textwidth}\centering
			\includegraphics[width=0.9\textwidth, alt={Schaltbild: R C Reihenschaltung mit Schalter, der von Kurzschluss zum Anschluss einer Gleichspannungsquelle Uq schaltet.}]{Tikz/pdf/circ_switchable_rc_charge_dc.pdf}
		\end{minipage}%
		\begin{minipage}{0.48\textwidth}\centering
			\includegraphics[alt={Zeitverlauf der Spannung uC an der Kapazität während des Ladevorgangs. Exponentielle Näherung an Spannungswert.}]{Tikz/pdf/plot_rc_charge_uc_transition.pdf}
		\end{minipage}
	\end{minipage}
	\vspace{0.5cm}
	\pause
	...
	\begin{minipage}{\textwidth}% Untere Hälfte
		\begin{minipage}[t]{0.48\textwidth}\centering
			...
			\includegraphics[width=0.4\textwidth, alt={Gemäldeausschnitt eines Kochtopfes über einem Lagerfeuer}]{Bilder/Ausschnitt_A_Peasant_Family_Cooking_over_a_Campfire_by_Bartolomeo_Pinelli_CC0_1.0.png}%
			\pause
		\end{minipage}%
		\begin{minipage}[t]{0.48\textwidth}%\centering
...
\speech{1Einfuehrung2Ausgleichsvorgaenge}{1}{%
...
Von der angeschlossenen Spannungsquelle fließt solange Energie in die Kapazität, bis sich die Spannungen angeglichen haben.
Am Ende fließt kein Strom mehr und die Kapazität hat die Spannung U q erreicht.
}% Speech Ende
\speech{1Einfuehrung2Ausgleichsvorgaenge}{2}{%
...
Die zugeführte Energie entspricht dann exakt der abgeführten Energie zum Beispiel durch Verdunstung.
}%
\speech{1Einfuehrung2Ausgleichsvorgaenge}{3}{%
...
\begin{frame}\ftx{\subsecname\,- Beispiele}
	\includegraphics[width=\textwidth, alt={Abbildung zweier Beispiele von Ausgleichsvorgängen. Erstens eine Roboterhand, die eine Glühbirne hält. Die Sprungantwort zeigt entweder ein aperiodisches Einschwingen oder eines mit Überschwingen, wobei die Glühbirne zerbricht. Zweitens ein Motorquerschnitt. Die Sprungantwort ist entweder eine schnelle oder eine langsame exponnentielle Annäherung, wie sie bei Fahrzeugen vorkommt.}]{Bilder/Ausgleichsvorgaenge_Beispiele_ET3_S_Exknowski_FH_SWF.png}% Provisorisch mit Genehmigung von Prof. Exknowski
\speech{1Einfuehrung2AusgleichsvorgaengeBeispiele}{1}{%
...
Im obigen Beispiel sehen wir ein Beispiel aus der Robotik.
Links zu sehen ist eine Roboterhand, die eine Glühbirne hält. 
Zum Halten muss der Fingerdruck auf einen festen Wert geregelt werden. 
...
\fu{\includegraphics[alt={Das Bild zeigt einen Ausgleichsvorgang bei DC und einen bei AC Anregung. In beiden Fällen ist die Systemgröße s zunächst null. Im Übergangsbereich (transient) nähert sich s einem stationären Zustand (persistent). Im DC Fall nähert sich s asymptotisch einem konstanten Wert an. Im AC Fall nähert sich s asymptotisch einer Sinuskurve an. Der transiente Zustand entspricht im AC Fall der Überlagerung einer exponentiell abklingenden Kurve und der eingeschwungenen Sinuskurve.}]{Tikz/pdf/plot_transient_persistent.pdf}}%
	{Vergleich: Ausgleichsvorgang DC, AC\label{fig:plot:transient_persistent}}
\s{
...
\speech{1Einfuehrung3Schaltvorgaenge}{1}{%
...
\silence{1}%
\speech{1Einfuehrung3Schaltvorgaenge}{1}{%
Mit den gepunkteten Verläufen wird angedeutet, wie sich der resultierende Zeitverlauf während des Schaltvorganges aus der
Überlagerung einer Sinusschwingung wie im eingeschwungenen Zustand und einer Exponenzialkurve wie im DC Fall ergibt. 
Der Schaltvorgang bezeichnet den Zustand des Überganges bei dem ein stationärer Zustand angestrebt wird. 
Stationär heißt im DC Fall ein konstanter Wert oder im AC Fall eine konstante Amplitude, Frequenz und Phase.
...
\begin{minipage}{0.38\textwidth}\centering
	\includegraphics[width=\textwidth, alt={Schaltbild eines Tiefsetzstellers: LC Reihenschaltung mit U2 über C. Die LC Reihenschaltung ist bei gezeigter Schalterstellung kurzgeschlossen während U1 an offenen Klemmen anliegt. In der anderen Schalterstellung liegt U1 über der LC Reihenschaltung an.}]{Bilder/Tiefsetzsteller_Schaltung_GET2_J_Böcker_Uni_Paderborn_cut.png}
	Tiefsetzsteller mit Glättungskondensator\footnotemark
\end{minipage}%
\begin{minipage}{0.58\textwidth}\centering
	\includegraphics[width=0.9\textwidth, alt={Zeitverläufe von Spannungen und Strömen am Tiefsetzsteller.}]{Bilder/Tiefsetzsteller_Zeitverläufe_GET2_J_Böcker_Uni_Paderborn_cut_edit.png}
\end{minipage}
...
\speech{1Einfuehrung3SchaltvorgaengeBeispiele}{1}{%
...
in der unteren Schalterstellung entlädt die Induktivität näherungsweise linear. 
...
\begin{figure}[H]\centering
	\includegraphics[alt={Schaltungsbild: Lineare Gleichspannungsquelle an idealen Schalter angeschlossen. Ausgangsspannung über Schalter.}]{Tikz/pdf/circ_switch_ideal.pdf}
...
\begin{figure}[H]\centering
	\begin{subfigure}{0.48\textwidth}\centering
		\includegraphics[alt={Spannungsverlauf bei idealem Schalter: Spannung wechselt instantan zwischen zwei Zuständen ohne Übergangsphase.}]{Tikz/pdf/plot_switch_ideal.pdf}
		\s{\caption{Spannung bei idealem Schalter}}\b{\par Ideal: verlustlos, instantan}
		\label{fig:plot:schalter:ideal}
	\end{subfigure}\pause
	\begin{subfigure}{0.48\textwidth}\centering
		\includegraphics[alt={Spannungsverlauf bei realem Schalter: Spannung ändert sich nicht instantan. Übergangsphasen mit Verlusten und Latenzen.}]{Tikz/pdf/plot_switch_real.pdf}
...
%	U1_	|__/ ....... \______/.......    ... = Strom
%		|___________________________> t
\speech{1Einfuehrung4SchalterIdealVsReal1}{1}{%
...
Im Durchlassbetrieb wird eine unendlich große Leitfähigkeit wie bei einem Kurzschluss angenommen und
im Sperrbetrieb wird ein unendlich großer elektrischer Widerstand wie bei offenen Klemmen angenommen.
}% Speech Ende
\speech{1Einfuehrung4SchalterIdealVsReal1}{2}{%
...
\begin{figure}[H]\centering
	\begin{subfigure}{0.48\textwidth}\centering
		\includegraphics[alt={Schaltungsbild: MOSFET als idealer Schalter ohne parasitäre Kapazitäten.}]{Tikz/pdf/circ_mosfet_ideal.pdf}
		\s{\caption{MOSFET, ohne Kapazitäten (ideal)}}\b{Ideal: keine parasitären Effekte}
		\label{fig:circ:mosfet:ideal}
	\end{subfigure}%
	\onslide<2->{%
	\begin{subfigure}{0.48\textwidth}\centering
		\includegraphics[alt={Schaltungsbild: MOSFET als realer Schalter mit parasitären Kapazitäten zwischen allen drei Anschlüssen.}]{Tikz/pdf/circ_mosfet_capacitive_effects.pdf}
...
\speech{1Einfuehrung5SchalterIdealVsReal2}{1}{%
Links sehen wir das Schaltbild eines MOSFET-Schalters mit Gate-, Drain- und Source-Anschlüssen.
Idealerweise treten keine parasitären Effekte auf.
}% Speech Ende
\speech{1Einfuehrung5SchalterIdealVsReal2}{2}{%
Real betrachtet treten kapazitive Effekte zwischen allen drei Anschlüssen auf.
