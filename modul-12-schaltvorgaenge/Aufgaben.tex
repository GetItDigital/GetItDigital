...
\begin{minipage}[b]{0.5\textwidth}% rechts
    \begin{figure}[H]\centering\vspace{0pt}% empty spacing für vertical alignment for tikzpicture
        \includegraphics[alt={Schaltbild: Eine ideale Gleichspannungsquelle Uq wird über einen Schalter an eine Serienschaltung aus R1 und R2 und C angeschlossen. Spannung u1 über allen drei Bauteilen. Spannung u2 nur über R2 und C.}]{Tikz/pdf/Uebungen/circ_uebung_einschaltverhalten_rs_rcs_dc.pdf}
    \end{figure}
\end{minipage}
\begin{enumerate}
...
\item[a)] DGL für $u_C(t \geq 0)$:
    \begin{align*}
        ...
        \underbrace{(R_1 + R_2) \cdot C}_{\ =\tau} \cdot \frac{\mathrm{d}}{\mathrm{d}t}\, \ecv{u_C} + \ecv{u_C} &= \ecv{U_q}
    \end{align*}
...
\begin{align*}
    ...
    &= C \cdot R_2 \cdot \frac{\mathrm{d}}{\mathrm{d}t}\, \ecv{u_C} + \ecv{u_C} \\
    &= C \cdot R_2 \cdot \frac{\mathrm{d}}{\mathrm{d}t}\, \left(1 - \mathrm{e}^{-\frac{t}{\tau}}\right) \cdot \ecv{U_q} + \left(1 - \mathrm{e}^{-\frac{t}{\tau}}\right) \cdot \ecv{U_q} \\
...
\begin{minipage}{0.5\textwidth}
\begin{figure}[H]
    \includegraphics[alt={Plot: Zeitverlauf der Spannungen uC und u2 und uR2 bei Einschaltvorgang einer Serienschaltung aus R1 und R2 und C. Die Spannung uC nähert sich von null exponentiell Uq an. Die Spannung uR2 springt von null auf Uq*R2/(R1+R2) und fällt dann exponentiell auf null ab. Die Spannung u2 springt auf den selben Wert und nähert sich dann exponentiell der Spannung Uq. Alle drei Exponentialverläufe haben die gleiche Zeitkonstante tau. Deren Tangenten schneiden nach exakt ihren jeweiligen Endwert nach exakt einem tau.}]{Tikz/pdf/Uebungen/plot_uebung_einschaltverhalten_rs_rcs_dc.pdf}
\end{figure}
\end{minipage}
...
\Aufgabe{
    ...
    \begin{figure}[H]\centering
        \includegraphics[alt={Schaltbild: Eine RC Schaltung mit zwei Widerständen wird mit Schalter vom Kurzschluss zum Anschluss an eine Gleichspannungsquelle Uq geschaltet. Die RC Schaltung besteht aus R1 in Reihe mit der Parallelschaltung aus R2 und C.}]{Tikz/pdf/Uebungen/circ_uebung_einschaltverhalten_rc_schaltung.pdf}% U_q an R1 in Reihe mit (R2 parallel zu C) ab t=0
    \end{figure}
...
\Loesung{
 ...
 \begin{align*}
    ...
    R_1 \cdot (i_C + i_{R2}) + u_C &= U_q &  i_C &= C \cdot \frac{\mathrm{d}}{\mathrm{d}t}\, u_C \qquad i_{R2} = \frac{u_C}{R_2}\\
    C \cdot R_1 \cdot \frac{\mathrm{d}}{\mathrm{d}t}\, u_C + \frac{R_1}{R_2} \cdot u_C + u_C &= U_q &&\Big| \cdot R_2\\
    C \cdot R_1 \cdot R_2 \cdot  \frac{\mathrm{d}}{\mathrm{d}t}\, u_C + \left(R_1 + R_2 \right) \cdot u_C &= U_q \cdot R_2 &&\Big| : (R_1+R_2)\\
    \underbrace{C \cdot \frac{R_1 \cdot R_2}{R_1+R_2}}_{\tau} \cdot \frac{\mathrm{d}}{\mathrm{d}t}\, u_C + u_C &= U_q \cdot \frac{R_2}{R_1+R_2}
 \end{align*}
...
\begin{minipage}[t]{0.48\textwidth}
    ...
    \begin{align*}
        ...
        i_C(t) &= C \cdot \frac{\mathrm{d}}{\mathrm{d}t}\, u_C(t) \\
...
\begin{minipage}[t]{0.48\textwidth}\centering
    Zeitverläufe:\\
    \includegraphics[alt={Plot: Gezeigt sind die Zeitverläufe für uC und für uR1 und für iC während des Schaltvorganges. uC nähert sich von null exponentiell dem Endwert Uq*R2/(R1+R2) an. uR1 springt von null auf Uq und fällt dann exponentiell ab auf den Wert Uq*R1/(R1+R2). iC springt von null auf Uq/R1 und fällt dann exponentiell auf null ab. Für alle drei Exponentialverläufe sind Tangenten eingezeichnet die nach der gleichen Zeitkonstante tau den jeweiligen Endwert schneiden.}]{Tikz/pdf/Uebungen/plot_uebung_einschaltverhalten_rc_schaltung.pdf}% Skizzen zu Aufgabenteil b) uC, uR1, iC
\end{minipage}
\begin{enumerate}
...
\begin{minipage}{\textwidth}\centering
    ...
    \includegraphics[alt={Schaltbild: Gezeigt ist eine RL Reihenschaltung mit Spannung u1 über R und L.}]{Tikz/pdf/Uebungen/circ_uebung_7.2.pdf}% Vgl. ET3 Judith Ackers, Übung 7.2
    \end{minipage}
    \quad\quad
    \begin{minipage}{0.25\textwidth}\centering
    \includegraphics[alt={Plot: Gezeigt ist der rechteckförmige Zeitverlauf der Spannung u1. Diese springt bei t=0 von 0 auf Uq. Bei t=t1 springt sie zurück auf 0.}]{Tikz/pdf/Uebungen/plot_uebung_7.2.pdf}% Vgl. ET3 Judith Ackers, Übung 7.2
    \end{minipage}
\end{minipage}
    ...
    \begin{enumerate}
        ...
        \begin{enumerate}
        \item[c1)] Nach welcher Zeit $t_0$ ist der Einschwingvorgang abgeschlossen?
        \item[c2)] Wie groß sind der Spulenstrom $i_L$ und die Spulenspannung $u_L$ nach $t=50\,ms$?
        \end{enumerate}
...
\Loesung{
    % Übernommen von Prof. Exknowski, FH SWF Hagen, 2024, ET3 (Übung 7, Aufgabe 1), leicht verändert
    \begin{align*}
        \text{DGL (allgemein):}&& \frac{L}{R}\cdot\frac{\mathrm{d}}{\mathrm{d}t}\, i &= \frac{U_1}{R_1} \quad\text{(Einschalten)}\\
    ...
    c) Zeit bis zum Abschluss des Einschwingvorgangs, Spulen -Strom und -Spannung nach $50\,ms$.\\[-1em]
        ...
        u_L&=L\cdot\frac{\mathrm{d}}{\mathrm{d}t} i_L = \frac{L}{R}\cdot U_0 \cdot (-\frac{1}{\tau})\cdot(-e^{-\frac{t}{\tau}})\\
    ...
    d) Entladevorgang, Spulen -Strom und -Spannung nach $t = t_1 + 50\,ms$.\\[-1em]
        \begin{align*}
            \text{DGL (Entladevorgang):}&& \frac{L}{R}\cdot\frac{\mathrm{d}}{\mathrm{d}t}\, i+i &=0\\
            \text{Lösungsansatz:}&& i &= \frac{U_0}{R}\cdot e^{-\frac{t}{\tau}}\quad\text{mit}\quad\tau=\frac{L}{R}\\
        \end{align*}
        \begin{align*}
            u_L&=L\cdot\frac{\mathrm{d}}{\mathrm{d}t} i = \frac{L}{R}\cdot u_0\cdot (-\frac{1}{\tau})\cdot e^{-\frac{t}{\tau}}\\
            &=-U_0\cdot e^{-\frac{t}{\tau}}\\
    ...
    \begin{center}
        \includegraphics[alt={Plot: Zeitverläufe für u1 und für uL und für iL. Der Zeitverlauf von u1 ist rechteckförmig. Bei t=0 springt u1 von 0 auf U0. Bei t=t1=10*tau springt u1 zurück auf 0. Die Spannung uL springt bei t=0 von null auf U0 und fällt dann mit Zeitkonstante tau exponentiell auf null ab. Bei t=t1 springt uL auf minus U0 und nähert sich dann mit dem gleichen tau exponentiell null an. Der Strom iL fängt bei 0 an und nähert sich ab t=0 exponentiell dem Wert U0/R. Bei t=t1 fällt iL exponentiell auf null ab. Die Zeitkonstanten sind gleich.}]{Tikz/pdf/Uebungen/plot_uebung_7.1_a.pdf}
    \end{center}
}
...
\Loesung{%
    ...
    \begin{align*}
        \text{DGL:}&&\quad\frac{L}{R}\cdot\frac{\mathrm{d}}{\mathrm{d}t} i+i&=\frac{U_0}{R}\\
        \text{Lösung:}&&i&=\frac{U_0}{R}\cdot(1-e^{-\frac{t}{\tau}})\quad\text{mit}\quad \tau = \frac{L}{R}
    \end{align*}
    \begin{align*}
        u_L &= L\cdot\frac{\mathrm{d}}{\mathrm{d}t} i =U_0\cdot e^{-\frac{t}{\tau}}\\
...
\Aufgabe{
    ...
    \begin{minipage}{0.38\textwidth}
        \centering
        \includegraphics[alt={Schaltbild: Zu sehen ist eine H-Brückenschaltung mit Anschluss an eine ideale Gleichspannungsquelle Uq. Die Brückenschaltung besteht aus drei Widerständen und einer Kapazität. Der linke Strang besteht aus R1 oben und C unten. Der rechte Strang besteht aus R2 oben und R3 unten. In der Mitte des H sind beide Stränge jeweils mittig über einen Schalter miteinander verbunden. }]{Tikz/pdf/Uebungen/circ_uebung_8.pdf}
    \end{minipage}
}
\Loesung{
...
\begin{align*}
    ...
    &= R_{12} \cdot (\eci{i_C} + \eci{i_3}) + \ecv{u_C} &&\text{mit}\quad \eci{i_{C}}=C\cdot\frac{\mathrm{d}}{\mathrm{d}t}\,\ecv{u_C},& \eci{i_3}&=\frac{\ecv{u_C}}{{R_3}}\\
    &= R_{12} \cdot (C \cdot \frac{\mathrm{d}}{\mathrm{d}t}\,\ecv{u_C} + \frac{\ecv{u_C}}{{R_3}}) + \ecv{u_C}\\
    &= C \cdot R_{12} \cdot \frac{\mathrm{d}}{\mathrm{d}t}\,\ecv{u_C} + \left( \frac{R_{12}}{{R_3}} + 1 \right) \cdot \ecv{u_C}& &\bigg|\ \cdot R_3\\
    \ecv{U_q} \cdot R_3
    &= C \cdot R_{12} \cdot R_3 \cdot \frac{\mathrm{d}}{\mathrm{d}t}\,\ecv{u_C} + \left(R_{12}+R_3\right) \cdot \ecv{u_C}& &\bigg| :\left(R_{12}+R_3\right)\\
    \ecv{U_q} \cdot \frac{R_3}{R_{12}+R_3}
    &= \underbrace{C \cdot \frac{R_{12} \cdot R_3}{R_{12} + R_3}}_{\tau} \cdot \frac{\mathrm{d}}{\mathrm{d}t}\,\ecv{u_C} + \ecv{u_C}& &\Longrightarrow\quad \tau=C \cdot \frac{R_{12} \cdot R_3}{R_{12} + R_3} 
\end{align*}
...
\includegraphics[alt={Plot: Gezeigt ist der Zeitverlauf i3 während eines Schaltvorganges. Der Strom ist zunächst konstant bei 100 mA und springt dann 500 mA. Von dort fällt er exponentiell auf 200 mA ab mit einer Zeitkonstante von 300 us.}]{Tikz/pdf/Uebungen/plot_uebung_8.pdf}
\end{minipage}
}
...
\Aufgabe{
    ...
    \begin{figure}[H]\centering
        \includegraphics[alt={Schaltbild: Gezeigt ist eine Schaltung aus R1 in Reihe mit der Parallelschaltung aus R2 und L. Die ganze Schaltung ist zunächst kurzgeschlossen und schaltet von dort auf den Anschluss einer idealen Wechselspannungsquelle u1.}]{Tikz/pdf/Uebungen/circ_uebung_einschaltverhalten_rs_rlp.pdf}
    \end{figure}
    \begin{enumerate}
...
\Loesung{
    ...
    \begin{align*}
        ...
        &= R_1 \cdot (\eci{i_2} + \eci{i_L}) + L \cdot \frac{\mathrm{d}}{\mathrm{d}t}\, \eci{i_L}\\
        &= R_1 \cdot \left( \frac{\ecv{u_L}}{R_2} + \eci{i_L} \right) + L \cdot \frac{\mathrm{d}}{\mathrm{d}t}\, \eci{i_L}\\
        &= \frac{R_1}{R_2} \cdot L \cdot \frac{\mathrm{d}}{\mathrm{d}t}\, \eci{i_L} + R_1 \cdot \eci{i_L} + L \cdot \frac{\mathrm{d}}{\mathrm{d}t}\, \eci{i_L}  \\
        &= L \cdot \left( \frac{R_1}{R_2} + 1 \right) \cdot \frac{\mathrm{d}}{\mathrm{d}t}\, \eci{i_L} + R_1 \cdot \eci{i_L} \qquad \bigg| \cdot \frac{1}{R_1}\\
        \frac{\ecv{u_1}}{R_1} &= \underbrace{L \cdot \left( \frac{1}{R_1} + \frac{1}{R_2} \right)}_{\ =\tau} \cdot \frac{\mathrm{d}}{\mathrm{d}t}\, \eci{i_L} + \eci{i_L}\\ 
