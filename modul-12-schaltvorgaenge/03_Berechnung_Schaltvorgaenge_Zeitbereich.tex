...
\speech{3Schaltvorgaenge1F01DC1Lernziele}{1}{%
...
\b{%
    ...
    \begin{minipage}{0.48\textwidth}%\centering % linksbündig
        ...
        \begin{equation*}
            a_1 \cdot \frac{\mathrm{d}}{\mathrm{d}t}\, y(t) + a_0 \cdot y(t) = b
        \end{equation*}
    \end{minipage}%
    \begin{minipage}{0.48\textwidth}\centering
        \pause
        \includegraphics[width=0.8\textwidth, alt={Plot: Zeitverlauf von Spannungen u(t) und uC(t) sowie von Strom i(t) bei einem RC-Glied. Spannung u schaltet von 0 auf konstanten Wert und später auf 0 zurück. Spannung uC steigt exponentiell annäherend an Endwert und später exponentiell abfallend auf 0 ab. Strom i springt von null auf Maximalwert und fällt exponentiell auf null ab. Später springt i auf negativen Maximalwert und nähert sich dann exponentiell null an.}]{Tikz/pdf/plot_rc_cycle_seperate_u.pdf}
        Schaltvorgänge bei RC-Glied (DC)
    \end{minipage}
}%

\speech{3Schaltvorgaenge1F02DC1}{1}{%
Bei Schaltvorgängen bei Gleichspannung kann es sich prinzipiell um Ladevorgänge oder um Entladevorgänge handeln.
}% Speech Ende
\speech{3Schaltvorgaenge1F02DC1}{2}{%
...
\speech{3Schaltvorgaenge1F02DC1}{3}{%
Wie aus den Grundlagen bekannt ist, ergibt sich für ein einfaches R C oder ein einfaches R L Glied 
jeweils eine DGL erster Ordnung, da in beiden Fällen nur ein Energiespeicher vorhanden ist (C oder L).
Die DGL wird daher die hier gezeigte Form: A eins mal erste Ableitung von Ypsilon von Tee plus A null mal Ypsilon von Tee ist gleich B annehmen.
Und wie sehen die Zeitverläufe prinzipiell aus?
}% Speech Ende
\speech{3Schaltvorgaenge1F02DC1}{4}{%
Bei einer DGL erster Ordnung erhalten wir einen einzelnen Eigenwert Lambda. 
...
Die Exponenzialform resultiert aus dem flüchtigen Zustand, das heißt der homogenen Lösung K mal E hoch Lambda Tee für einfache Nullstellen. 
}% Speech Ende
\end{frame}
...
\s{% skript only, tau
    ...
    \begin{minipage}[b]{0.48\textwidth}\centering
        \fu{\includegraphics[alt={Plot: Zeitverlauf von Spannung uC. Zunächst uC=0 und ab t=0 exponentielle Annäherung von uC an Uq. Tangente im Ursprung schneidet nach einem Tau Uq.}]{Tikz/pdf/plot_rc_charge_uc_tau.pdf}}{Exp. Annäherung und $\tau$\label{fig:plot:rc:tau}}%
    \end{minipage}%
    \begin{minipage}[b]{0.48\textwidth}\centering
    ...
    \begin{equation}\label{eq:dgl:tau}
        \tau \cdot \frac{\mathrm{d}}{\mathrm{d}t}\,y(t) + y(t) = b(t)
    \end{equation}
...
\begin{minipage}[t][][t]{0.48\textwidth}\centering\vspace{0cm}% vspace: input empty space before tikzpicture for top alignment%
    \includegraphics[alt={Schaltung: RC Reihenschaltung mit Schalter. Schaltet vom Kurzschluss zum Anschluss einer Gleichspannungsquelle Uq.}]{Tikz/pdf/circ_switchable_rc_charge_dc.pdf}\\[4pt]
    \textbf{DGL 1. Ordnung} $\Rightarrow$ Exponentialverlauf.
\end{minipage}%
\pause%
% rechts
\begin{minipage}[t][][t]{0.48\textwidth}\centering\vspace{0cm}% vspace: input empty space before tikzpicture for top alignment%
    \includegraphics[alt={Plot: Zeitverlauf von Spannung uC. Zunächst uC=0 und ab t=0 exponentielle Annäherung von uC an Uq. Tangente im Ursprung schneidet nach einem Tau Uq.}]{Tikz/pdf/plot_rc_charge_uc_tau.pdf}\\
...
\speech{3Schaltvorgaenge1F03DC1Zeitkonstante}{1}{%
...
Wegen des einen Energiespeichers C, hat die DGL erste Ordnung und die Zeitverläufe haben die Form von Exponenzialverläufen.
}% Speech Ende
\speech{3Schaltvorgaenge1F03DC1Zeitkonstante}{2}{%
Rechts sehen wir im Zeitverlaufsdiagramm die Spannung U C an der Kapazität. 
Die Spannung U C ist zu Beginn null und steigt ab dem Schaltzeitpunkt Tee gleich null exponentiell bis zum Endwert U q.
Die dazugehörige Gleichung ist: U C von Tee ist gleich U q (der Endwert) mal Klammer auf eins minus e hoch minus Tee durch Tau Klammer zu. 
Der Term: Eins minus e hoch minus Tee durch Tau, entspricht einer exponentiellen Annäherung von null an eins. 
}% Speech Ende
\speech{3Schaltvorgaenge1F03DC1Zeitkonstante}{3}{%
...
\speech{3Schaltvorgaenge1F03DC1Zeitkonstante}{4}{%
...
Beide Verläufe näheren sich dem Endwert eins oder null, erreichen diesen aber nie.
Deshalb gilt als Faustregel, dass ein Exponenzialverlauf nach fünf Tau eingeschwungen ist.
Dann hat der Wert ca. neunundneunzig Prozent der Differenz zum Endwert überbrückt.
Nach einem Tau sind es ca. dreiundsechzig Prozent, nach drei Tau circa fünfundneunzig Prozent.
...
\s{% RC-Laden, skript only without frames
    ...
    \begin{bsp}{RC-Glied DC-Ladevorgang}{rc:chargedc}
        ...
        \begin{minipage}{\textwidth}\centering
        \begin{minipage}[t][][t]{0.48\textwidth}\centering\vspace{0cm}% vspace: input empty space before tikzpicture for top alignment
            \includegraphics[alt={Schaltung: RC Reihenschaltung mit Schalter. Schaltet vom Kurzschluss zum Anschluss einer Gleichspannungsquelle Uq.}]{Tikz/pdf/circ_switchable_rc_charge_dc.pdf}%
        ...
        \end{minipage}
        \begin{minipage}[t][][t]{0.48\textwidth}\centering\vspace{0cm}% vspace: input empty space before tikzpicture for top alignment
            \includegraphics[alt={Plot: Zeitverlauf von uC und i beim laden eines RC Gliedes. Beide Größen sind vor t=0 null. Spannung uC nähert sich dann exponentiell Uq an. Strom i springt auf den Maximalwert Uq/R und fällt dann exponentiell auf 0 ab.}]{Tikz/pdf/plot_rc_charge_uc_i_seperate.pdf}
        \end{minipage}
        \end{minipage}
        ...
        \begin{align*}
            ...
            &&&&
                ...
                &\text{mit}\quad \eci{i} = C \cdot \frac{\mathrm{d}}{\mathrm{d}t} \ecv{u_C} \\
            &\text{DGL}:&&&
                R \cdot C \cdot \frac{\mathrm{d}}{\mathrm{d}t} \ecv{u_C}  + \ecv{u_C}   &= \ecv{U_q}
        ...
        \begin{align*}%
            &\text{DGL}_{h}:&&&
                RC \cdot \frac{\mathrm{d}}{\mathrm{d}t} \ecv{u_{C,\mathrm{h}}} + \ecv{u_{C,\mathrm{h}}} &= 0 &&&
                    &\text{Ansatz:}\quad
        ...
        \begin{align*}%
            &\text{DGL}_{p}:&&&
                \redcancel{RC \cdot \frac{\mathrm{d}}{\mathrm{d}t} \ecv{u_{C,p}}} + \ecv{u_{C,p}} &= \ecv{U_q} &&&
...
\s{% RC-Entladen, skript only without frames
    \begin{bsp}{RC-Glied Entladevorgang}{rc:discharge}
        ...
        \begin{minipage}{\textwidth}\centering
            \begin{minipage}[t][][t]{0.48\textwidth}\centering\vspace{0cm}% vspace: input empty space before tikzpicture for top alignment
                \includegraphics[alt={Schaltbild: RC Reihenschaltung mit Schalter. Schaltet von Anschluss einer Gleichspannungsquelle Uq zum Kurzschluss.}]{Tikz/pdf/circ_switchable_rc_discharge.pdf}%
                \begin{equation*}\begin{aligned}
            ...
            \end{minipage}
            \begin{minipage}[t][][t]{0.48\textwidth}\centering\vspace{0cm}% vspace: input empty space before tikzpicture for top alignment
                \includegraphics[alt={Plot: Zeitverlauf von Spannung uC und Strom i beim Entladen eines RC-Glieds. Spannung uC startet bei Uq und fällt exponentiell auf 0 ab. Strom i startet bei null und springt dann auf minus Uq/R und nähert sich dann exponentiell 0 an.}]{Tikz/pdf/plot_rc_discharge_uc_i_seperate.pdf}
            \end{minipage}
        ...
        \textbf{1.) DGL aufstellen:} von $\ecv{u_C}(t)$ für $t>0$, analog zu Beispiel \ref{bsp:rc:chargedc}
            ...
            &\text{Masche}:&&&
                ...
                &\text{mit}\quad \ecv{u_R} = R \cdot \eci{i} \\
            &\text{DGL}:&&&
                RC \cdot \frac{\mathrm{d}}{\mathrm{d}t} \ecv{u_C}  + \ecv{u_C}   &= 0 &&&
                &\text{mit}\quad \eci{i} = C \cdot \frac{\mathrm{d}}{\mathrm{d}t} \ecv{u_C}
            \\[-15pt]&\hphantom{R1ooooooo}&&&\hphantom{L3oooooooooooooooooooo}&\hphantom{R3ooo}&&&&\hphantom{R5oooooooooooooooooooooo}\nonumber% Dummy-Zeile A für horizontales Alignment zwischen align-Umgebungen [15pt hoch]
        \end{align*}%
...
\begin{minipage}{\textwidth}\centering
\begin{minipage}[t][][t]{0.48\textwidth}\centering\vspace{0cm}% vspace: input empty space before tikzpicture for top alignment
    \includegraphics[alt={Schaltbild: RC Reihenschaltung mit Schalter. Schaltet vom Kurzschluss zum Anschluss einer Gleichspannungsquelle Uq.}]{Tikz/pdf/circ_switchable_rc_charge_dc.pdf}%
    \begin{equation*}\begin{aligned}
        &\text{gegeben:}&       &\ecv{u_C}(0),\ \ecv{U_q},\ R,\ C \\
        &\text{gesucht:}&       &\ecv{u_C}(t)
    \end{aligned}\end{equation*}
\end{minipage}
\begin{minipage}[t][][t]{0.48\textwidth}\centering\vspace{0cm}% vspace: input empty space before tikzpicture for top alignment
    \includegraphics[width=0.8\textwidth, alt={Plot: Zeitverlauf von uC und i beim laden eines RC Gliedes. Beide Größen sind vor t=0 null. Spannung uC nähert sich dann exponentiell Uq an. Strom i springt auf den Maximalwert Uq/R und fällt dann exponentiell auf 0 ab.}]{Tikz/pdf/plot_rc_charge_uc_i_seperate.pdf}
\end{minipage}
\end{minipage}
% Untere Hälfte
...
\begin{align*}
&\textbf{1. DGL}&\hspace{15pt}\ecv{U_q} &= RC \cdot \frac{\mathrm{d}}{\mathrm{d}t} \ecv{u_C} + \ecv{u_C} \vphantom{\Big|}\\
...
\speech{3Schaltvorgaenge1F04DC1RCLadevorgang1}{1}{%
Kommen wir zur eigentlichen Berechnung des Ladevorganges beim gerade gezeigten R C Glied. 
Oben links sehen wir die gegebenen Größen, das ist U C unmittelbar vor dem Schalten, die Quellenspannug U Kuh und die Komponenten R und C. 
Gesucht ist U C während des Schaltvorganges für Tee größer gleich null. 
...
Unten sehen wir die fünf Schritte des Berechnungsverfahrens in Kurzform und die dazugehörigen Teillösungen die wir im folgenden Einzeln durch gehen.
}% Speech Ende
\end{frame}
...
&&
    R \cdot {\color{red} i} + \ecv{u_C}    &= \ecv{U_q}&
        &\text{mit}\quad \ecv{u_R} = R \cdot \eci{i} \vphantom{\frac{\mathrm{d}}{\mathrm{d}t}}
}\\[2pt]%
\onslide<4->{% <--- Overlay
&\text{DGL}:&
    R \cdot C \cdot \frac{\mathrm{d}}{\mathrm{d}t}\, \ecv{u_C}  + \ecv{u_C}   &= \ecv{U_q}&
...
\speech{3Schaltvorgaenge1F05DC1RCLadevorgang2}{1}{%
Schritt eins ist das Aufstellen der DGL für unsere gesuchte Größe U C von Tee für den Zeitraum Tee größer gleich null, 
das heißt ab unmittelbar nach dem Schaltzeitpunkt Tee gleich null.
Wie im Beispiel in den Grundlagen beginnen wir mit der Maschengleichung:
}% Speech Ende
\speech{3Schaltvorgaenge1F05DC1RCLadevorgang2}{2}{%
U R plus U C ist gleich U Kuh.
}% Speech Ende
\speech{3Schaltvorgaenge1F05DC1RCLadevorgang2}{3}{%
Dann ersetzen wir U R durch R mal I.
}% Speech Ende
\speech{3Schaltvorgaenge1F05DC1RCLadevorgang2}{4}{%
Und ersetzen I durch C mal der zeitlichen Ableitung von U C. 
Wenn wir unsere DGL mit der allgemeinen Form oben rechts vergleichen, erkennen wir:
R C entspricht unserem konstanten Koeffizienten a eins. 
U C von Tee entspricht unserer Systemgröße Ypsilon von T. 
Und die Quellenspannung U Kuh entspricht der Störgröße B.
}% Speech Ende
\end{frame}
...
\begin{align*}
&\text{DGL}_{h}:&
    RC \cdot \frac{\mathrm{d}}{\mathrm{d}t} \ecv{u_{C,h}} + \ecv{u_{C,h}} &= 0 &
        &\text{Ansatz: }
...
&\text{DGL}_{p}:&
    \redcancel{RC \cdot \frac{\mathrm{d}}{\mathrm{d}t} \ecv{u_{C,p}}} + \ecv{u_{C,p}} &= \ecv{U_q}
        &&\text{mit}\quad \left.\frac{\mathrm{d}}{\mathrm{d}t}\right|_{t \to \infty} = 0  \\
&\text{part. Lsg.}:&
    \ecv{u_{C,p}} &= \ecv{U_q} %
\\[-15pt]&\hphantom{R1oooooooooo}&\hphantom{L2oooooooooooooooooooooo}&\hphantom{R2oooo}&&\hphantom{R3oooooooooooooooooo}% Dummy-Zeile RL-DC für horizontales Alignment zwischen align-Umgebungen [15pt hoch]
\end{align*}% new alignments

\speech{3Schaltvorgaenge1F06DC1RCLadevorgang3}{1}{%
Um die flüchtige Lösung zu bestimmen, bestimmen wir als nächsten Schritt die Lösung der homogenen DGL, das heißt für die DGL ohne Störterm. 
Aus der DGL können wir direkt das charakteristische Polynom ableiten: R C mal Lambda plus eins.
...
Für eine bessere Lesbarkeit der Eigenschaften schreiben wir den Exponenzialansatz in Abhängigkeit der Zeitkonstante auf. 
Unsere homogene Lösung ist K mal E hoch minus Tee durch Tau. 
...
\speech{3Schaltvorgaenge1F06DC1RCLadevorgang3}{2}{%
Die partikuläre Lösung, das heißt den eingeschwungenen Zustand für Tee gegen unendlich, erhalten wir entweder durch Netzwerkberechnung oder leiten ihn aus der DGL ab.
Für beide Herangehensweisen gilt, dass die eingeschwungenen Größen Gleichgrößen sind, weil die Anregung eine Gleichspannung ist. Also der DC Fall vorliegt.
Aus der Gleichstrom Netzwerkberechnung wissen wir, dass die Kapazität wie eine offene Klemme wirkt. Es fließt kein Strom und die gesamte Spannung U Kuh liegt über C an.
Alternativ können wir wie hier gezeigt ist die DGL für die partikuläre Lösung herannehmen.
Da es sich um Gleichgrößen handelt, ist die zeitliche Ableitung von U C P gleich Null und der erste Teil der DGL verschwindet.
Übrig bleibt, dass U C P gleich der Anregung U Kuh ist.
}% Speech Ende
\end{frame}
...
&\text{Strom: }&
    ...
    &\text{mit}\quad \eci{i_C} = C \cdot \frac{\mathrm{d}}{\mathrm{d}t} \ecv{u_C}
\\[-15pt]&\hphantom{R1oooooooooo}&\hphantom{L2oooooooo}&\hphantom{R2oooooooooooooooooo}&&\hphantom{R3oooooooooooooooooo}% Dummy-Zeile RL-DC für horizontales Alignment zwischen align-Umgebungen [15pt hoch]
\end{align*}%

\speech{3Schaltvorgaenge1F07DC1RCLadevorgang4}{1}{%
Die allgemeine Lösung der inhomogenen DGL erhalten wir durch Überlagerung unserer homogenen und unserer partikulären Lösung.
U c ist gleich U c h plus U c p. Oder alternativ ist gleich U c F plus U c e. 
Eingesetzt ergibt sich daraus die allgemeine Lösung U c von Tee ist gleich K mal e hoch minus T durch Tau plus U q. 
}% Speech Ende
\speech{3Schaltvorgaenge1F07DC1RCLadevorgang4}{2}{%
...
Der Exponenzialterm entfällt für T gleich null.
Durch Umformung erhalten wir K ist gleich Minus U Kuh.
Durch Einsetzen von K und Ausklammern von U Kuh erhalten wir die spezielle allgemeine Lösung:
U C von Tee ist gleich U Kuh mal Klammer auf eins minus e hoch minus T durch Lambda Klammer zu. 
Den Strom können wir aus der Beziehung I gleich C mal der zeitlichen Ableitung von U C an der Kapazität herleiten. 
Hier mit tau gleich R C eingesetzt.
Durch kürzen erhalten wir den Stromverlauf I C von Tee ist gleich U Kuh durch R mal e hoch minus T durch Tau. 
Der maximale Anfangsstrom I C null ist also U Kuh durch R. 
}% Speech Ende
\end{frame}
...
\speech{3Schaltvorgaenge2F01DC2RCEntladevorgangLernziele}{1}{%
...
\begin{minipage}{\textwidth}\centering
\begin{minipage}[t][][t]{0.48\textwidth}\centering\vspace{0cm}% vspace: input empty space before tikzpicture for top alignment
    \includegraphics[alt={Schaltbild: RC Reihenschaltung mit Schalter. Schaltet vom Anschluss einer Gleichspannungsquelle Uq zum Kurzschluss.}]{Tikz/pdf/circ_switchable_rc_discharge.pdf}%
    \begin{equation*}\begin{aligned}
        &\text{gegeben:}&       &\ecv{u_C}(0),\ \ecv{U_q},\ R,\ C \\
        &\text{gesucht:}&       &\ecv{u_C}(t)
    \end{aligned}\end{equation*}
\end{minipage}
\begin{minipage}[t][][t]{0.48\textwidth}\centering\vspace{0cm}% vspace: input empty space before tikzpicture for top alignment
    \includegraphics[width=0.8\textwidth, alt={Plot: Zeitverlauf von Spannung uC und Strom i beim Entladen eines RC-Glieds. Spannung uC startet bei Uq und fällt exponentiell auf 0 ab. Strom i startet bei null, springt dann auf negativ Uq/R und nähert sich dann exponentiell 0 an.}]{Tikz/pdf/plot_rc_discharge_uc_i_seperate.pdf}
\end{minipage}
\end{minipage}
% Untere Hälfte
\resizebox{0.9\textwidth}{!}{% to fit on slide
\begin{minipage}{\textwidth}%
\begin{align*}
&\textbf{1. DGL}&\hspace{15pt} 0 &= RC \cdot \frac{\mathrm{d}}{\mathrm{d}t} \ecv{u_C} + \ecv{u_C}\\
...
\speech{3Schaltvorgaenge2F02DC2RCEntladevorgang1}{1}{%
...
Der Strom beginnt bei null, macht dann einen Sprung auf Minus U q durch R und nähert sich dann exponentiell der Null.
...
\textbf{1.) DGL aufstellen:} für $\ecv{u_C}(t)$ ($t\geq 0$)
...
&\text{DGL}:&
    RC \cdot \frac{\mathrm{d}}{\mathrm{d}t} \ecv{u_C}  + \ecv{u_C}   &= 0
        &&\text{mit}\quad
        \eci{i} = \eci{i_C} = C \cdot \frac{\mathrm{d}}{\mathrm{d}t} \ecv{u_C}
\end{align*}%

\speech{3Schaltvorgaenge2F03DC2RCEntladevorgang2}{1}{%
Die DGL wird im gleichen Verfahren aufgestellt wie beim Ladevorgang. 
Da die Masche nicht mehr durch die Spannungsquelle, sondern durch den Kurzschluss geschlossen wird, entfällt allerdings der Störterm. 
Dadurch erhalten wir die homogene DGL aus dem Ladevorgang.
}% Speech Ende
\end{frame}
...
\speech{3Schaltvorgaenge2F04DC2RCEntladevorgang3}{1}{%
...
Die speziefische allgemeine Lösung lautet damit uc von Tee ist gleich U q mal e hoch minus T durch Tau. 
Den Strom können wir über die Komponentengeich für C direkt aus U C bestimmen. 
Daraus ergibt sich für den Strom I C von Tee ist gleich Minus U q durch R mal e hoch minus T durch Tau.
}% Speech Ende
\end{frame}
...
% Obere Hälfte
\begin{minipage}[t][][t]{0.48\textwidth}\centering\vspace{0cm}% vspace: input empty space before tikzpicture for top alignment%
    \includegraphics[alt={Schaltbild: RC Reihenschaltung mit Schalter. Schaltet vom Kurzschluss zum Anschluss einer Gleichspannungsquelle Uq.}]{Tikz/pdf/circ_switchable_rc_charge_dc.pdf}\\[1em]% Leerzeile
    {\flushleft% left alignment
    Rechenwege prinzipiell gleich für (DC) \\ Ladevorgang und Entladevorgang.%
    }%
\end{minipage}
\begin{minipage}[t][][t]{0.48\textwidth}\centering\vspace{0cm}% vspace: input empty space before tikzpicture for top alignment
    \includegraphics[width=0.6\textwidth, alt={Plot: Zeitverlauf von Spannung uC und i beim Lade- und Entladezyklus eines RC-Glieds. Beide Größen sind vor t=0 null. Spannung uC nähert sich dann exponentiell Uq an und danach exponentiell null an. Strom i springt auf den Maximalwert Uq/R und fällt dann exponentiell auf 0 ab. Danach springt i auf negativ Uq/R und nähert sich exponentiell 0 an.}]{Tikz/pdf/plot_rc_cycle_seperate_u.pdf}%
\end{minipage}
\end{minipage}\hfill%
% Untere Hälfte
\begin{minipage}{\textwidth}\centering%
...
\speech{3Schaltvorgaenge2F05DC2RCVergleich}{1}{%
...
Mathematisch ausgedrückt: U c von Tee minus t0 ist gleich U c von Tee null, der Startwert, plus Klammer auf, die Differenz U c e minus U c von Tee null, Klammer zu, mal klammer auf Eins minus Exponent minus T minus T null durch Tau Exponent Ende, die exponentielle Annäherung, Klammer zu. 
Die Rechenwege zur Berechnung von Lade- und Entladevorgängen bei DC Fällen sind prinzipiell identisch
...
\begin{bsp}{RL-Glied DC-Ladevorgang - Über DGL von $i$}{rl:chargedc:dgli}
    ...
    \begin{minipage}{\textwidth}\centering
        \begin{minipage}[t][][t]{0.48\textwidth}\centering\vspace{0cm}% vspace: input empty space before tikzpicture for top alignment
            \includegraphics[alt={Schaltbild: RL Serienschaltung an Schalter. Schaltet vom Kurzschluss zum Anschluss einer Gleichspannungsquelle Uq.}]{Tikz/pdf/circ_switchable_rl_charge_dc.pdf}%
            \begin{equation*}\begin{aligned}
        ...
        \end{minipage}
        \begin{minipage}[t][][t]{0.48\textwidth}\centering\vspace{0cm}% vspace: input empty space before tikzpicture for top alignment
            \includegraphics[alt={Plot: Zeitverlauf von uL und i beim Laden eines RL Gliedes. Beide Größen sind vor t=0 null. Spannung uL springt auf den Maximalwert Uq und fällt dann exponentiell auf 0 ab. Strom i nähert sich exponentiell Uq/R an.}]{Tikz/pdf/plot_rl_charge_ul_i_seperate.pdf}
        \end{minipage}
    \end{minipage}
    ...
    \begin{align*}
        &\text{Masche}:&&&
            ...
            &\text{mit}\quad \ecv{u_R} = R \cdot \eci{i} \\
        &&&&
            R \cdot \eci{i} + L \cdot \frac{\mathrm{d}}{\mathrm{d}t} \eci{i} &= \ecv{U_q} &&&
            &\text{mit}\quad \ecv{u_L} = L \cdot \frac{\mathrm{d}}{\mathrm{d}t} \eci{i} \\
        &\text{DGL}:&&&
        \frac{L}{R} \cdot \frac{\mathrm{d}}{\mathrm{d}t} \eci{i}  + \eci{i}  &= \frac{\ecv{U_q}}{R}
    ...
    \begin{align*}%
        &\text{DGL}_{h}:&&&
            \frac{L}{R} \cdot \frac{\mathrm{d}}{\mathrm{d}t} \eci{i_{\mathrm h}} + \eci{i_{\mathrm h}} &= 0 &&&
                &\text{Ansatz:}\quad
    ...
    \begin{align*}%
        &\text{DGL}_{p}:&&&
            \redcancel{\frac{L}{R} \cdot \frac{\mathrm{d}}{\mathrm{d}t} \eci{i_{\mathrm p}}} + \eci{i_{\mathrm p}} &= \frac{\ecv{U_q}}{R} &&&
...
\begin{minipage}{\textwidth}\centering%
\begin{minipage}[t][][t]{0.48\textwidth}\centering\vspace{0cm}% vspace: input empty space before tikzpicture for top alignment%
    \includegraphics[alt={Schaltbild: RL Serienschaltung an Schalter. Schaltet vom Kurzschluss zum Anschluss einer Gleichspannungsquelle Uq.}]{Tikz/pdf/circ_switchable_rl_charge_dc.pdf}\\[1em]% Leerzeile
    {\flushleft% left alignment
...
\end{minipage}
\begin{minipage}[t][][t]{0.48\textwidth}\centering\vspace{0cm}% vspace: input empty space before tikzpicture for top alignment
    \includegraphics[width=0.6\textwidth, alt={Plot: Zeitverlauf von Strom i und Spannung uL beim Lade- und Entladezyklus eines RL-Glieds. Beide Größen sind vor t=0 null. Spannung uL springt auf einen Maximalwert und fällt dann exponentiell auf 0 ab. Strom i nähert sich exponentiell einem Endwert an und fällt dann exponentiell auf 0 ab. Danach springt i auf den negativen Maximalwert und nähert sich exponentiell 0 an.}]{Tikz/pdf/plot_rl_cycle_seperate_i.pdf}%
\end{minipage}
\end{minipage}\hfill%
% Untere Hälfte
\begin{minipage}{\textwidth}%
...
\speech{3Schaltvorgaenge2F06DC2RLVergleich}{1}{%
Kommen wir nun zum Lade- und Entladevorgang eines RL Seriengliedes bei Gleichspannung. 
Die Schaltung ist identisch zu den Beispielen mit dem RC Glied aufgebaut, nur dass die Kapazität mit einer Induktivität ersetzt wurde.
Da es sich zum eine Schaltung mit nur einem Energiespeicher handelt, erhalten wir wieder eine DGL erster Ordnung. 
...
Als Vermutung können wir aufstellen, dass die Zeitverläufe prinzipiell ähnlich aussehen wie bei den DC Schaltvorgängen beim RC Glied.
...
\end{minipage}
\begin{minipage}[t][][t]{0.48\textwidth}\centering\vspace{0cm}% vspace: input empty space before tikzpicture for top alignment
    \includegraphics[width=0.8\textwidth, alt={Plot: Zeitverlauf von uL und i beim Laden eines RL Gliedes. Beide Größen sind vor t=0 null. Spannung uL springt auf den Maximalwert Uq und fällt dann exponentiell auf 0 ab. Strom i nähert sich exponentiell Uq/R an.}]{Tikz/pdf/plot_rl_charge_ul_i_seperate.pdf}
\end{minipage}
\end{minipage}
% Untere Hälfte
\resizebox{0.9\textwidth}{!}{% to fit on slide
\begin{minipage}{\textwidth}%
\begin{align*}
&\textbf{1. DGL}&\hspace{15pt}\frac{\ecv{U_q}}{R} &= \frac{L}{R} \cdot \frac{\mathrm{d}}{\mathrm{d}t}\, \eci{i} + \eci{i}\\[-2pt]
...
\speech{3Schaltvorgaenge2F07DC2RLLadevorgang1}{1}{%
Beginnen wir beim Ladevorgang. Oben links sehen Sie das Schaltbild.
Zu Beginn ist das R L Glied kurzgeschlossen und ab T gleich null ist das R L Glied an die Spannungsquelle U Kuh geschlossen.
Zu Beginn fließt kein Strom, U Kuh, R und L sind bekannt, gesucht ist der Strom I für T größer gleich null.
Oben rechts sehen Sie die Zeitverläufe für U L und I. Die Spannung U L springt bei T gleich null von null auf U Kuh.
...
Der Strom nähert sich ab Tee gleich null exponentiell seinem Endwert U Kuh durch R, da der Widerstand R den maximalen Strom begrenzt.
...
&&
    R \cdot \eci{i} + L \cdot \frac{\mathrm{d}}{\mathrm{d}t} \eci{i} &= \ecv{U_q} &
        &\text{mit}\quad \ecv{u_L} = L \cdot \frac{\mathrm{d}}{\mathrm{d}t} \eci{i} \\
&&
    L \cdot \frac{\mathrm{d}}{\mathrm{d}t} \eci{i} + R \cdot \eci{i} &= \ecv{U_q} &
        &\text{mit}\quad \ecv{u_L} = L \cdot \frac{\mathrm{d}}{\mathrm{d}t} \eci{i} \\
&\text{DGL}:&
    \frac{L}{R} \cdot \frac{\mathrm{d}}{\mathrm{d}t} \eci{i}  + \eci{i}  &= \frac{\ecv{U_q}}{R}
\end{align*}%

\speech{3Schaltvorgaenge2F08DC2RLLadevorgang2}{1}{%
Die DGL stellen wir am Besten ausgehend von der Maschengleichung auf.
U R und U L ist gleich U Kuh. 
...
L mal der zeitlichen Ableitung von I plus R mal I ist gleich U Kuh.
Theoretisch könnten wir nun bereits weiter rechnen, allerdings bietet es sich an, 
die DGL weiter umzuformen, dass unsere gesuchte Größe I ohne Koeffizienten einzeln steht.
Also teilen wir durch R und erhaltene unten gezeigte DGL, 
...
\speech{3Schaltvorgaenge2F09DC2RLLadevorgang3}{1}{%
Durch Aufstellen des charakteristschen Polnoms können wir den Eigenwert Lambda bestimmen.
Da die Lösung in Exponenzialform auftritt, schreiben wir direkt Lambda ist gleich minus eins durch Tau ist gleich minus R durch L.
Dadurch können wir die homogene Lösung direkt in die Form K mal e hoch minus Tee durch Tau bringen mit Tau ist gleich L durch R.
}% Speech Ende
\speech{3Schaltvorgaenge2F09DC2RLLadevorgang3}{2}{%
Für die partikuläre Lösung bestimmen wir den eingeschwungenen Zustand. 
Wir können annehmen, dass I P konstant ist, wodurch der Differenzialterm entfällt. 
Daraus folgt I P ist gleich U Kuh durch R.
}% Speech Ende
\end{frame}
...
\speech{3Schaltvorgaenge2F10DC2RLLadevorgang4}{1}{%
Die Allgemeine Lösung ergibt sich durch Überlagerung beider Lösungen.
}% Speech Ende
\speech{3Schaltvorgaenge2F10DC2RLLadevorgang4}{2}{%
Die Konstante K erhalten wir durch einsetzen der Anfangsbedingung.
Für diese gilt, dass der Anfangsstrom i zum Zeitpunkt null gleich null ist.
Daraus folgt K ist gleich minus U Kuh durch R.
Eingesetzt in unsere allgemeine Lösung ergibt sich die eindeutige Lösung.
Durch etwas Umformung erhalten wir die gezeigte Form. 
Diese entspricht einer exponentiellen Annäherung des Stromes an den Endwert U Kuh durch R.
Die Spannung an der Induktivität lässt sich über die Komponentengleichung für L bestimmen.
Durch einsetzen der Zeitkonstante und Vereinfachung erhalten wir die gezeigte Form.
}% Speech Ende
\end{frame}
}% Ende Folien only Multi Frame
...
\begin{bsp}{RL-Glied DC-Ladevorgang - Über DGL von $u_L$}{rl:chargedc:dglu}
    ...
    \begin{align*}
        ...
        &&&&
            ...
            &\left| \cdot \frac{\mathrm{d}}{\mathrm{d}t} \right. \\
        &&&&
        R \cdot \frac{\ecv{u_L}}{L}  + \frac{\mathrm{d}}{\mathrm{d}t} \ecv{u_L}  &= \frac{\mathrm{d}}{\mathrm{d}t} \ecv{U_q} &&&
            &\text{mit}\quad \ecv{u_L} = L \cdot \frac{\mathrm{d}}{\mathrm{d}t} \eci{i} \\
        &\text{DGL}:&&&
            \frac{L}{R} \cdot \frac{\mathrm{d}}{\mathrm{d}t} \ecv{u_L} + \ecv{u_L}  &= 0
            \\[-15pt]&\hphantom{R1ooooooo}&&&\hphantom{L3oooooooooooooooooooo}&\hphantom{R3ooo}&&&&\hphantom{R5oooooooooooooooooooooo}\nonumber% Dummy-Zeile A für horizontales Alignment zwischen align-Umgebungen [15pt hoch]
        \end{align*}%
    \textbf{2.) Homogene Lösung, allg.:} (flüchtig) (ohne Störterm)%
    \begin{align*}%
        &\text{DGL}_{h}:&&&
            \frac{L}{R} \cdot \frac{\mathrm{d}}{\mathrm{d}t} \ecv{u_{L,h}} + \ecv{u_{L,h}} &= 0 &&&
                &\text{Ansatz:}\quad
    ...
    \begin{align*}%
        &\text{DGL}_{p}:&&&
            \redcancel{\frac{L}{R} \cdot \frac{\mathrm{d}}{\mathrm{d}t} \ecv{u_{L,p}}} + \ecv{u_{L,p}} &= 0 &&&
...
\s{%
    ...
    \begin{figure}[H]\centering
        \begin{subfigure}{0.48\textwidth}\centering
            \includegraphics[alt={Schaltbild: RL Serienschaltung an Schalter. Schaltet vom Anschluss einer Gleichspannungsquelle Uq zum Kurzschluss.}]{Tikz/pdf/circ_switchable_rl_discharge.pdf}
            \caption{Schaltbild}
            \label{fig:circ:rl:discharge}
        \end{subfigure}
        \begin{subfigure}{0.48\textwidth}\centering
            \includegraphics[alt={Plot: Zeitverlauf von uL und i beim Entladen eines RL Gliedes. Die Spannung uL springt von null auf den negativen Maximalwert Uq und nähert sich dann exponentiell null an. Der Strom i ist maximal bei Uq/R und fällt dann exponentiell auf null ab.}]{Tikz/pdf/plot_rl_discharge_ul_i_seperate.pdf}
    ...
    \begin{figure}[H]\centering
        \begin{subfigure}{0.48\textwidth}\centering
            \includegraphics[alt={Schaltbild: RL Glied mit zwei Widerständen schaltet vom Kurzschluss zum Anschluss einer Gleichspannungsquelle Uq. R1 in Serie mit L und R2 parallel zu L.}]{Tikz/pdf/circ_switchable_rrl_charge_detail.pdf}
            \caption{Schaltbild}
            \label{fig:circ:rl:charge}
        \end{subfigure}
        \begin{subfigure}{0.48\textwidth}\centering
            \includegraphics[alt={Plot: Zeitverlauf für Ströme und Spannungen beim Laden des realen RL-Gliedes. Spannung uL springt von null auf Maximalwert Uq*R2/(R1+R2) und fällt dann exponentiell ab. Die Spannung uR1 springt von null auf Uq*R1/(R1+R2) und nähert sich dann exponentiell Uq an. Der Strom iL nähert sich von null exponentiell dem Endwert Uq/R1. Der Strom iR2 springen von null auf Uq/(R1+R2) und fällt dann auf null ab. iR1 ist die Summe aus iL und iR2.}]{Tikz/pdf/plot_rrl_charge_dc.pdf}
            \caption{Spannungs- und Stromverlauf}
...
\end{minipage}
\begin{minipage}[t][][t]{0.48\textwidth}\centering\vspace{0cm}% vspace: input empty space before tikzpicture for top alignment
    \includegraphics[width=0.8\textwidth, alt={Plot: Zeitverlauf von uL und i beim Entladen eines RL Gliedes. Die Spannung uL springt von null auf minus Uq und nähert sich dann exponentiell null an. Der Strom i ist maximal bei Uq/R und fällt dann exponentiell auf null ab.}]{Tikz/pdf/plot_rl_discharge_ul_i_seperate.pdf}
\end{minipage}
\end{minipage}
% Untere Hälfte
\resizebox{0.9\textwidth}{!}{% to fit on slide
\begin{minipage}{\textwidth}%
\begin{align*}
&\textbf{1. DGL}&\hspace{15pt} 0 &= \frac{L}{R} \cdot \frac{\mathrm{d}}{\mathrm{d}t}\, \eci{i} + \eci{i} \vphantom{\frac{L}{R}}\\[-2pt]
...
\speech{3Schaltvorgaenge2F11DC2RLEntladevorgang}{1}{%
...
\speech{3Schaltvorgaenge3F01ACLernziele}{1}{%
...
Schaltvorgänge bei einfachen RC und RL Serienschaltungen bei Anregung mit Wechselspannung.
Sie lernen:
Schaltvorgänge bei AC Anregung zu berechnen, 
...
Möglichkeiten zur Vermeidung von Schaltvorgängen bei AC Anregung kennen.
}% Speech Ende
\end{frame}%
}% Video Only Frame Ende
...
\begin{minipage}{0.48\textwidth}\centering
    \pause
    \includegraphics[width=0.8\textwidth, alt={Plot: Zeitverlauf von uC bei unterschiedlichen Schaltzeitpunkten t0 und t1 zerlegt in eingeschwungenen und flüchtigen Zustand. uC ist zunächst null. Beim Schaltzeitpunkt t0 zum Maximum des eingeschwungenen Zustandes entsteht ein Einschwingvorgang. Beim Schaltzeitpunkt t1 zum Nulldurchgang des eingeschwungenen Zustandes ist uC sofort eingeschwungen. }]{Tikz/pdf/plot_rc_charge_uc_ac_different_t0.pdf}
    Vergleich: Schaltzeitpunkte $t_0$ und $t_1$
\end{minipage}
}%

\speech{3Schaltvorgaenge3F02AC}{1}{%
Fangen wir mit den größten Unterschieden von Schaltvorgängen bei AC und bei DC Anregung an.
Schaltvorgänge bei AC Anregung sind frequenzabhängig und schaltzeitpunktabhängig.
}% Speech Ende
\speech{3Schaltvorgaenge3F02AC}{2}{%
Wir leiten uns beide Merkmale anhand zweier einfacher Beispiele her.
Die Berechnung erfolgt analog zu den DC Beispielen mit dem bekannten Berechnungsverfahren. 
Zunächst für eine Serienschaltung aus C und R und dann für eine Serienschaltung aus L und R. 
}% Speech Ende
\speech{3Schaltvorgaenge3F02AC}{3}{%
In beiden Beispielen erhalten wir wieder eine DGL erster Ordnung.
Im Vergleich zu den DC Beispielen unterscheiden sich die DGL en hier nur durch einen sinusförmigen Störterm b. 
...
\speech{3Schaltvorgaenge3F02AC}{4}{%
...
In der Mitte ist der Schaltvorgang für die Spannung u C abgebildet, wenn bei T null geschaltet wird.
Unten ist der Schaltvorgang für die Spannung u C abgebildet, wenn bei T eins geschaltet wird.
...
\s{%
    ...
    \begin{figure}[H]\centering
        \includegraphics[alt={Schaltbild: RC Serienschaltung an Schalter. Schaltet vom Kurzschluss zum Anschluss einer idealen Wechselspannungsquelle uq.}]{Tikz/pdf/circ_switchable_rc_charge_ac.pdf}%
    ...
    \begin{align}
        RC \cdot \frac{\mathrm{d}}{\mathrm{d}t} \ecv{u_C}(t) + \ecv{u_C}(t) ={}& \ecv{u_q}(t) \\
    ...
    \begin{figure}[H]\centering
        \begin{subfigure}{0.48\textwidth}\centering
            \includegraphics[alt={Plot: Zeitverlauf von uC bei unterschiedlichen Schaltzeitpunkten t0 und t1 zerlegt in eingeschwungenen und flüchtigen Zustand. uC ist zunächst null. Beim Schaltzeitpunkt t0 zum Maximum des eingeschwungenen Zustandes entsteht ein Einschwingvorgang. Beim Schaltzeitpunkt t1 zum Nulldurchgang des eingeschwungenen Zustandes ist uC sofort eingeschwungen.}]{Tikz/pdf/plot_rc_charge_uc_ac_different_t0.pdf} % u_C bei verschiedenen Schaltzeitpunkten
            \caption{Verschiedene Schaltzeitpunkte $t_0$}
            \label{fig:plot:rc:charge:ac:uc:vergleicht0}
        \end{subfigure}%
        \begin{subfigure}{0.48\textwidth}\centering
            \includegraphics[alt={Plot: Zeitverlauf von uC bei großer und bei kleiner Zeitkonstante jeweils mit Schaltzeitpunkt zum Maximum des eingeschwungenen Zustandes. In beiden Fällen schwing uC sich von null in den eingeschwungenen Zustand ein. Bei großer Zeitkonstante entstehen Überspannungen mit fast dem doppelten Scheitelwert.}]{Tikz/pdf/plot_rc_charge_uc_ac_different_tau.pdf} % u_C bei verschiedenen Zeitkonstanten
...
\begin{minipage}{\textwidth}\centering%
\begin{minipage}[t][][t]{0.48\textwidth}\centering\vspace{0cm}% vspace: input empty space before tikzpicture for top alignment%
    \includegraphics[alt={Schaltbild: RC Reihenschaltung mit Schalter. Schaltet vom Kurzschluss zum Anschluss einer idealen Wechselspannungsquelle uq.}]{Tikz/pdf/circ_switchable_rc_charge_ac.pdf}
    \begin{equation*}\begin{aligned}
...
\end{minipage}\pause%
\begin{minipage}[t][][t]{0.48\textwidth}\centering\vspace{0cm}% vspace: input empty space before tikzpicture for top alignment%
    \includegraphics[width=0.75\textwidth, alt={Plot: Zeitverlauf von uC während Einschwingvorgang bei AC Anregung zerlegt in flüchtigen und eingeschwungenen Zustand. Der flüchtige Zustand nähert sich null an.}]{Tikz/pdf/plot_rc_charge_uc_ac_different_t0_short.pdf}
\end{minipage}%
...
\begin{align*}
&\textbf{1. DGL}&                   \ecv{u_q} &= RC \cdot \frac{\mathrm{d}}{\mathrm{d}t} \ecv{u_C}(t) + \ecv{u_C}(t) = \ecv{\hat{U}_q} \cdot \cos(\omega t + \varphi_q)\\
...
\speech{3Schaltvorgaenge3F03ACbeiRC1}{1}{%
Gehen wir das Beispiel Schritt für Schritt durch. 
Oben links ist unsere bekannte Schaltung aus den DC Beispielen. 
Zum Schaltzeitpunkt T null wird ein zuvor kurzgeschlossenes R C Serienglied an eine ideale Wechselspannungsquelle U Kuh geschlossen. 
Gegeben ist die Spannung U C zum Schaltzeitpunkt T null, die Wechselspannung U Kuh, die Komponenten R, C und die Kreisfrequenz omega. 
Gesucht ist U C für T größer gleich T null.
}% Speech Ende
\speech{3Schaltvorgaenge3F03ACbeiRC1}{2}{%
...
\speech{3Schaltvorgaenge3F03ACbeiRC1}{3}{%
Hier sehen wir kurz zur Übersicht die fünf Schritte des Berechnungsverfahrens und die zugehörigen Teillösungen die wir im Folgenden ausführlich berechnen werden.
}% Speech Ende
\end{frame}
...
\speech{3Schaltvorgaenge3F04ACbeiRC2}{1}{%
Der erste Schritt, das Aufstellen der DGL ist im Prinzip identisch zu den DC Beispielen. 
Der Unterschied ist nur, dass unser Störterm keine Gleichgröße ist, sondern eine Wechselspannung.
Hier ist unser Störterm U Kuh von Tee gleich dem Scheitelwert U Kuh Dach mal Kosinus von omega T plus Phi Q.
...
\speech{3Schaltvorgaenge3F05ACbeiRC3}{1}{%
Im dritten Schritt bestimmen wir die Partikuläre Lösung, das heißt den eingeschwungenen Zustand. 
Aufgrund der sinusförmigen Eingangsspannung U Kuh erhalten wir in unserer linearen zeitinvarianten Schaltung 
auch eine sinusförmige Ausgangsspannung U C P mit gleicher Kreisfrequenz omega wie bei der Anregung U Kuh im eingeschwungenen Zustand. 
}% Speech Ende
\silence{1}%
\speech{3Schaltvorgaenge3F05ACbeiRC3}{1}{%
Im Zeitbereich dargestellt ist unsere partikuläre Lösung U C P gleich dem Scheitelwert U C P Dach mal Kosinus von omega T plus Vieh Zeh P.
Die Amplitude U C P Dach und den Phasenwinkel Vieh Zeh P können wir nun mittels komplexer Wechselstromrechnung bestimmen.
Zur Erinnerung, unser U C P von Tee im Zeitbereich ist der Realteil des komplexen Amplitudenzeigers U C P Dach komplex mal e hoch J Omega t. 
Der komplexe Amplitudenzeiger U C P Dach komplex ist gleich U C P Dach reell mal e hoch J Vieh Zeh P. 
}% Speech Ende
\speech{3Schaltvorgaenge3F05ACbeiRC3}{2}{%
Durch den Komplexen Spannungsteiler können wir U C P komplex in Abhängigkeit von U Kuh komplex und den Impedanzen Z C und R bestimmen. 
Durch etwas Umformung kommen wir auf die Form: 
U C P Dach komplex ist gleich U Kuh Dach komplex durch eins plus J Omega C R. 
}% Speech Ende
\speech{3Schaltvorgaenge3F05ACbeiRC3}{3}{%
Durch Betragsbildung erhalten wir die Amplitude U C P Dach mit der Lösung unten links.
Durch Winkelbestimmung erhalten wir den Lastphasenwinkel Vieh Zeh P mit der Lösung unten rechts.
Der Winkel Vieh Zeh P ist die Phasendifferenz der Quellenspannung U Kuh und der eingeschwungenen Spannung U C P. 
}% Speech Ende
\end{frame}
...
\speech{3Schaltvorgaenge3F06ACbeiRC4}{1}{%
Im vierten Schritten erhalten wir die allgemeine Lösung der inhomogenen DGL durch Überlagerung.
Die Lösung besteht wie wir sehen aus einer abklingenden Exponentialkurve und einer sinusförmigen Schwingung.
...
Der Exponenzialterm reduziert sich zu e hoch null gleich eins und kann gekürzt werden. 
Durch Umstellen erhalten wir unsere Konstante K. Wie wir sehen können, entspricht unser K mit
K ist gleich Minus U C P Dach mal Kosinus von omega mal T null plus Vieh Zeh P, 
...
\speech{3Schaltvorgaenge3F06ACbeiRC4}{2}{%
...
\begin{frame}\ftx{Schaltvorgänge \subsecname\ \subsubsecname}% Vergleich verschiedene tau und maximale Auslenkung (Überschwinger)
    ...
    \end{minipage}%
    \begin{minipage}{0.48\textwidth}\centering
        \includegraphics[width=0.8\textwidth, alt={Plot: Zeitverlauf von uC bei unterschiedlichen Zeitkonstanten tau. Bei großem Tau werden Überspannungen bis fast zum doppelten Scheitelwert erreicht. Schaltzeitpunkt jeweils zum Maximum des eingeschwungenen Zustandes. uC zu Beginn null.}]{Tikz/pdf/plot_rc_charge_uc_ac_different_tau.pdf}
        Vergleich: kleines und großes $\tau$
    \end{minipage}%

\speech{3Schaltvorgaenge3F07ACUeberspannungenundstroeme}{1}{%
Insbesondere bei großen Zeitkonstanten sind Überspannungen und -ströme möglich!
}% Speech Ende
\speech{3Schaltvorgaenge3F07ACUeberspannungenundstroeme}{2}{%
...
Der dreifache Scheitelwert ist möglich, wenn die Anfangsbedingung ist, dass die Spannung während des Schaltzeitpunktes im Scheitelpunkt liegt. 
}% Speech Ende
\speech{3Schaltvorgaenge3F07ACUeberspannungenundstroeme}{3}{%
...
\speech{3Schaltvorgaenge3F07ACUeberspannungenundstroeme}{4}{%
...
Beim kleinen Tau kommt es bereits zu einer leichten Überspannung, die schnell abklingt.
...
\s{%
    ...
    \begin{align*}
        &\text{DGL$_{\mathrm p}$:}&
            \frac{L}{R} \cdot \frac{\mathrm{d}}{\mathrm{d}t} \eci{i_{\mathrm p}} + \eci{i_{\mathrm p}} &= \frac{\ecv{u_q}}{R} &
    ...
    \begin{bsp}{RL-Glied, Schaltvorgang mit Wechselspannungsquelle}{rl:chargeac}
        ...
        \begin{minipage}{\textwidth}\centering
            \begin{minipage}[t][][t]{0.48\textwidth}\centering\vspace{0cm}% vspace: input empty space before tikzpicture for top alignment
                \includegraphics[alt={Schaltbild: RL Reihenschaltung mit Schalter. Schaltet vom Kurzschluss zum Anschluss einer idealen Wechselspannungsquelle uq.}]{Tikz/pdf/circ_switchable_rl_charge_ac.pdf}%
                \begin{equation*}\begin{aligned}
            ...
            \end{minipage}
            \begin{minipage}[t][][t]{0.48\textwidth}\centering\vspace{0cm}% vspace: input empty space before tikzpicture for top alignment
                \includegraphics[alt={Plot: Zeitverlauf von iL während Einschwingvorgang bei AC Anregung zerlegt in flüchtigen und eingeschwungenen Zustand. Der flüchtige Zustand nähert sich null an.}]{Tikz/pdf/plot_rl_charge_il_ac_different_t0_short.pdf}
            \end{minipage}
        \end{minipage}
        ...
        \begin{align*}
            ...
            \\[-15pt]&\hphantom{R1oooooooooo}&&&\hphantom{L3oooooo}&\hphantom{R3ooooooooooooooooooooooooooooooooooooooooooo}&&&&\hphantom{R5}\nonumber% Dummy-Zeile RL-AC Step 4-5 für horizontales Alignment zwischen align-Umgebungen [15pt hoch]
        \end{align*}
        Spannung $\ecv{u_L}$ über der Induktivität $L$ über $\ecv{u_L} = L \cdot \frac{\mathrm{d}}{\mathrm{d}t} \eci{i}$ berechnen.
        \begin{align*}
            &\text{Spannung:}&&&
...
\end{minipage}\pause%
\begin{minipage}[t][][t]{0.48\textwidth}\centering\vspace{0cm}% vspace: input empty space before tikzpicture for top alignment%
    \includegraphics[width=0.75\textwidth, alt={Plot: Zeitverlauf von iL während Einschwingvorgang bei AC Anregung zerlegt in flüchtigen und eingeschwungenen Zustand. Der flüchtige Zustand nähert sich null an.}]{Tikz/pdf/plot_rl_charge_il_ac_different_t0_short.pdf}%
\end{minipage}%
\end{minipage}\pause\hfill%
% Untere Hälfte
\resizebox{0.95\textwidth}{!}{% to fit on slide
\begin{minipage}{\textwidth}\vspace{-0.75cm}%
\begin{align*}
&\textbf{1. DGL}&                   \frac{\ecv{u_q}}{R} &= \frac{L}{R} \cdot \frac{\mathrm{d}}{\mathrm{d}t}\, \eci{i} + \eci{i} =\frac{\ecv{\hat{U}_q}}{R} \cdot \cos(\omega t + \varphi_q)\\
...
\speech{3Schaltvorgaenge3F08ACbeiRL1}{1}{%
...
Gesucht ist I von Tee ab dem Schaltzeitpunkt T null. Zum Zeitpunkt T null fließt noch kein Strom, die Wechselspannung, R und L sind gegeben.
Die Berechnung erfolgt analog zum vorigen Beispiel am R C Glied. 
}% Speech Ende
\speech{3Schaltvorgaenge3F08ACbeiRL1}{2}{%
Oben rechts sind die Zeitverläufe von der Anregung U q der gesuchten Größe I von Tee und für einen Beispiel Schaltzeitpunkt T Null dargestellt. 
}% Speech Ende
\speech{3Schaltvorgaenge3F08ACbeiRL1}{3}{%
...
\speech{3Schaltvorgaenge3F09ACbeiRL2}{1}{%
Das Aufstellen der DGL erfolgt wieder analog zum DC Beispiel. 
Genau wie beim R C Beispiel ist unsere Störgröße sinusförmig. Hier ist unsere Störgröße der Strom U q durch R
mit U q gleich U q Dach mal Kosinus von omega T plus Vieh Kuh.
}% Speech Ende
\speech{3Schaltvorgaenge3F09ACbeiRL2}{2}{%
Die homogene Lösung können wir wieder aus unserem DC Beispiel übernehmen, hier mit allgemeinem Schaltzeitpunkt T null.
...
\speech{3Schaltvorgaenge3F10ACbeiRL3}{1}{%
Für die partikuläre Lösung wenden wir wieder die komplexe Wechselstromrechnung an. 
Statt eines komplexen Spannungsteilers, transformieren wir die DGL in den Bildbereich und ersetzen dabei den Differenzialoperator d nach d Tee mit J Omega. 
Durch Ausklammern und Umstellen erhalten wir unseren komplexen Amplituden Zeiger 
I P Dach komplex gleich U Kuh Dach komplex durch R mal eins im Zähler, geteilt durch eins plus J Omega mal L durch R im Nenner.
Jetzt wieder den Betrag und die Phasenlage für I P bestimmen. 
Achtung: Vieh Kuh ist beliebig wählbar, da Tee null variabel ist, dadurch kann Vieh Kuh zur Vereinfachung auch null gesetzt werden.
}% Speech Ende
\end{frame}
...
\begin{frame}\ftx{Schaltvorgänge \subsecname \, \subsubsecname}
    ...
    Spannung $\ecv{u_L}$ mit $\ecv{u_L} = L \cdot \frac{\mathrm{d}}{\mathrm{d}t} \eci{i}$ berechnen:
    \begin{align*}
        &\text{Spannung:}&&&
            \ecv{u_L}(t) &= R \cdot \eci{\hat{I}_{\mathrm p}} \cdot \cos(\omega t_0 + \varphi_I) \cdot \mathrm{e}^{-\frac{t-t_0}{\tau}} - \omega L \cdot \eci{\hat{I}_{\mathrm p}} \cdot \sin(\omega t + \varphi_I)
    \end{align*}%

\speech{3Schaltvorgaenge3F11ACbeiRL4}{1}{%
Die allgemeine Lösung erhalten wir wieder durch Überlagerung der homogenen und der partikulären Lösung. 
}% Speech Ende
\speech{3Schaltvorgaenge3F11ACbeiRL4}{2}{%
Als Anfangsbedingung sei angenommen, dass I von Tee null gleich null ist, das heißt zum Einschaltzeitpunkt kein Strom fließt. 
...
\speech{3Schaltvorgaenge3F11ACbeiRL4}{3}{%
Die Spannung U L an der Induktivität erhalten wir mit U L ist gleich L mal d nach d T von I. 
Achtung. Im Gegensatz zum Strom kann die Spannung U L springen! 
Das sehen wir an der Gleichung, da sich der Exponenzialterm und der sinusförmige Term zum Schaltzeitpunkt nicht gegenseitig aufheben. 
Zur Übung können Sie.
Erstens den Zeitverlauf für die Spannung U L in diesem Beispiel skizzieren.
Zweitens den Zeitverlauf für den Strom beim R C Beispiel skizzieren.
Und drittens überlegen, welche Zeitverläufe sich beim Zurückschalten von Wechselspannungsquelle zu Kurzschluss ergeben.
}% Speech Ende
\end{frame}
...
\newvideofile{SchaltvorgaengeRLC1}{Schaltvorgänge bei RLC-Serienschwingkreisen [1/2]}
\v{% Video only Frame
\begin{frame}\ftx{Entladevorgang \subsubsecname}
    \begin{Lernziele}{Video - Schaltvorgänge bei RLC-Serienschwingkreisen [1/2]}
        Sie lernen in diesem Video:
        \begin{itemize}\b{\setlength\itemsep{0.7em}}
            \item Schaltvorgänge bei RLC-Gliedern bei Gleichspannung im Zeitbereich zu berechnen,
            \item die Fallunterscheidung bei RLC-Gliedern zu verstehen und anzuwenden und
            \item Einschwingvorgänge bei RLC-Gliedern zu analysieren.
        \end{itemize}
    \end{Lernziele}
\speech{3Schaltvorgaenge4F01RLC1Lernziele}{1}{%
Willkommen zum siebten Video der Videoreihe für das Modul Schaltvorgänge. 
In diesem Video steigen wir ein in die Berechnung von Schaltvorgängen bei RLC-Gliedern im Zeitbereich bei Gleichspannung. 
Wir lernen die Fallunterscheidung für periodische und aperiodische Einschwingvorgänge bei Schwingkreisen kennen und anwenden.
Und wir analysieren Einschwingvorgänge bei RLC-Gliedern.
}% Speech Ende
\end{frame}
}% Video only Frame Ende
...
\s{%
    ...
    \begin{figure}[H]\centering
        \begin{subfigure}{0.48\textwidth}\centering
            \includegraphics[alt={Schaltbild: RLC Reihenschaltung mit Schalter. Schaltet vom Kurzschluss zum Anschluss einer Gleichspannungsquelle Uq.}]{Tikz/pdf/circ_switchable_rlc_charge_detail.pdf}
            \caption{Schaltbild}
            \label{fig:circ:rlc:charge}
        \end{subfigure}
        \begin{subfigure}{0.48\textwidth}\centering
            \includegraphics[alt={Plot: Zeitverlauf von uC bei Ladevorgang eines RLC-Gliedes mit Gleichspannung Uq. Darstellung der drei Fälle. Aperiodisch ist langsam. Aperiodischer Grenzfall ist schneller. Periodisch ist am schnellsten mit Überschwingen.}]{Tikz/pdf/plot_rlcs_charge_dc_uc.pdf}
    ...
    \begin{align}
        ...
        L \cdot \frac{\mathrm{d}}{\mathrm{d}t} \eci{i} + R \cdot \eci{i} + \ecv{u_C}&= \ecv{U_q} \nonumber\\
        LC \cdot \frac{\mathrm{d}^{2}}{\mathrm{d}t^{2}} \ecv{u_C} + RC \cdot \frac{\mathrm{d}}{\mathrm{d}t} \ecv{u_C} + \ecv{u_C} &= \ecv{U_q} \label{eq:dgl:rlc:dc}
    \end{align}
    ...
    \begin{align}
        &\text{hom. DGL:}&
            LC \cdot \frac{\mathrm{d}^{2}}{\mathrm{d}t^{2}} \ecv{u_{C,\mathrm{h}}} + RC \cdot \frac{\mathrm{d}}{\mathrm{d}t} \ecv{u_{C,\mathrm{h}}} + \ecv{u_{C,\mathrm{h}}} &= 0
...
\begin{minipage}{\textwidth}\centering
    \begin{minipage}[t][3.5cm][]{0.48\textwidth}\centering
        \includegraphics[width=\textwidth, alt={Schaltbild: RLC Reihenschaltung mit Schalter. Schaltet vom Kurzschluss zum Anschluss einer Gleichspannungsquelle Uq.}]{Tikz/pdf/circ_switchable_rlc_charge_detail.pdf}%
        \vspace{5pt}

        Wegen $L$ und $C$ $\Rightarrow$ Schwingkreis
    \end{minipage}%
    \begin{minipage}[t][3.5cm][]{0.48\textwidth}\centering
        \includegraphics[width=\textwidth, alt={Plot: Zeitverlauf von uC bei Ladevorgang eines RLC-Gliedes mit Gleichspannung Uq. Darstellung der drei Fälle. Aperiodisch ist langsam. Aperiodischer Grenzfall ist schneller. Periodisch ist am schnellsten mit Überschwingen.}]{Tikz/pdf/plot_rlcs_charge_dc_uc.pdf}%
    \end{minipage}
\end{minipage}\vspace{2pt}

    %\textbf{DGL}: inhomogen, linear, gewöhnlich, 2. Ordnung, mit konst. Koeffizienten
    \begin{align*}
    &\text{\textbf{DGL} (für $\ecv{u_C}$ für $t \geq 0$):}&    LC \cdot \frac{\mathrm{d}^{2}}{\mathrm{d}t^{2}} \ecv{u_C} + RC \cdot \frac{\mathrm{d}}{\mathrm{d}t} \ecv{u_C} + \ecv{u_C} &= \ecv{U_q}\\[-20pt]
...
\speech{3Schaltvorgaenge4F02RLC1Allgemein1}{1}{%
Gegeben sei der RLC-Serienschwingkreis wie im Schaltbild oben links dargestellt.
Durch die zwei verschiedenen Energiespeicher ist das System prinzipiell schwingungsfähig.
Das Systemverhalten kann durch eine DGL zweiter Ordnung beschrieben werden.
Abhängig von den Werten der Bauelemente R, L und C ergeben sich verschiedene Einschwingverhalten bei Anregung mit Gleichspannung U Kuh.
Dabei sind drei Fälle möglich, diese sind rechts im Zeitverlaufsdiagramm dargestellt und unten in der Tabelle gegenübergestellt.
Möglich ist ein aperiodischer Fall, ein aperiodischer Grenzfall und ein periodischer Fall.
Die Fälle können anhand verschiedener Merkmale wie der Diskriminante D, welche wir im Folgenden genauer betrachten, oder anhand der Schwingkreisgüte Q bestimmt werden.
}% Speech Ende
\end{frame}%
...
\begin{align*}
    &\textbf{homogene DGL:}&
        LC \cdot \frac{\mathrm{d}^{2}}{\mathrm{d}t^{2}} \ecv{u_{C,\mathrm{h}}} + RC \cdot \frac{\mathrm{d}}{\mathrm{d}t} \ecv{u_{C,\mathrm{h}}} + \ecv{u_{C,\mathrm{h}}} &= 0  %\label{eq:dgl:rlc:homo}
        \\[-20pt]
        &\hphantom{R1oooooooooooooooooooo}&\hphantom{L2oooooooooooooooooooooooooooooo}&\hphantom{R2ooo} % Dummy-Zeile für horizontales Alignment zwischen Frames
        \\[6pt]
    &\text{char. Polynom:}&
...
\speech{3Schaltvorgaenge4F03RLC1Allgemein2}{1}{%
Um Schaltvorgänge bei RLC Gliedern zu berechnen, gehen wir nach dem bekannten Verfahren vor.
Im ersten Schritt stellen wir die DGL auf, hier für die Kondensatorspannung U C, und bestimmen anhand des charakteristischen Polynoms die Eigenwerte Lambda eins und Lambda zwei.
An dieser Stelle ergibt sich eine Besonderheit. Der Term unter der Wurzel, die sogenannte Diskriminante D, kann je nach Werten der Bauelemente R, L und C entweder positiv, null oder negativ sein.
Dadurch ergeben sich für die Eigenwerte drei verschiedene Fälle.
}% Speech Ende
\end{frame}
...
\speech{3Schaltvorgaenge4F04RLC1Fallunterscheidung}{1}{%
Je nach Diskriminante D ergeben sich unterschiedliche Eigenwerte Lambda eins und Lambda zwei.
Bei einer positiven Diskriminante sind die Eigenwerte verschieden und reell.
Bei einer Diskriminante von null sind die Eigenwerte identisch und reell.
Bei einer negativen Diskriminante sind die Eigenwerte komplex konjugiert.
}% Speech Ende
\speech{3Schaltvorgaenge4F04RLC1Fallunterscheidung}{2}{%
Wie wir wissen, bestimmt die Art der Eigenwerte die Form der homogenen Lösung.
Rechts sehen sie die allgemeine Form der homogenen Lösung für zwei unterschiedliche Eigenwerte beziehungsweise für zwei identische Eigenwerte.
}% Speech Ende
\speech{3Schaltvorgaenge4F04RLC1Fallunterscheidung}{3}{%
Hier sehen Sie die drei Fälle der Fallunterscheidung noch einmal in tabellarischer Form gegenübergestellt.
Die drei Fälle werden wir im Folgenden nacheinander einzeln betrachten. 
Wie wir sehen werden, lässt sich die homogene Lösung für den aperiodischen Fall und den periodischen Fall noch weiter spezifizieren.
}% Speech Ende
\end{frame}
...
%}%
\speech{3Schaltvorgaenge4F05RLC1FallunterscheidungAperiodisch1}{1}{%
Beginnen wir beim aperiodischen Fall, auch Kriechfall genannt, mit positiver Diskriminante D.
Wie in der Gleichung für Lambda eins und zwei oben rechts zu erkennen ist, ergeben sich in diesem Fall zwei verschiedene, reelle Eigenwerte.
Diese sind stets negativ, da der Term vor der Wurzel betragsmäßig größer ist als der Wurzelterm.
}% Speech Ende
\speech{3Schaltvorgaenge4F05RLC1FallunterscheidungAperiodisch1}{2}{%
Die homogene Lösung besteht aus der Überlagerung zweier Exponentialfunktionen mit den Eigenwerten Lambda eins und Lambda zwei.
Die Konstanten K eins und K zwei müssen beide reell sein, damit die Lösung reellwertig bleibt.
Der flüchtige Zustand entspricht damit der Addition zweier Exponentialfunktionen.
Dabei dominiert die langsamer Exponenzialfunktion, das heißt die mit der größeren Zeitkonstante.
Die partikuläre Lösung, das heißt der eingeschwungene Zustand, entspricht in diesem Beispiel der Gleichspannung U q.
}% Speech Ende
\end{frame}%
%
\begin{frame}\ftx{\subsubsecname}% Aperiodischer Fall: Zweiter Frame
    ...
    \s{Sei $\ecv{u_C}(0)=0$ und $\eci{i}(0)=0 \Rightarrow \frac{\mathrm{d}}{\mathrm{d}t}\, \ecv{u_C}(0) = 0 $ als Anfangsbedingungen (AB) gegeben, so folgt:}%
    \b{\vspace{4pt}Anfangsbedingungen (AB): $\ecv{u_C}(0)=0$ und $\eci{i}(0)=0 \Rightarrow \frac{\mathrm{d}}{\mathrm{d}t}\, \ecv{u_C}(0) = 0 $}
    ...
    \begin{align*}
        ...
        &\text{2. AB:}&
            \frac{\mathrm{d}}{\mathrm{d}t} \ecv{u_C}(0) &= K_1 \cdot \lambda_1 \cdot \redcancel{\mathrm{e}^{\lambda_1 0}} + K_2 \cdot \lambda_2 \cdot \redcancel{\mathrm{e}^{\lambda_2 0}} + \redcancel{0} \overset{!}{=} 0 &  \Rightarrow\quad K_1 &= - K_2 \frac{\lambda_2}{\lambda_1} \nonumber\\
        &\Longrightarrow&
            K_1 &=  \frac{\ecv{U_q}}{\frac{\lambda_1}{\lambda_2}-1} \quad\text{und}\quad
                K_2 = \frac{\ecv{U_q}}{\frac{\lambda_2}{\lambda_1}-1}
    \end{align*}

\speech{3Schaltvorgaenge4F06RLC1FallunterscheidungAperiodisch2}{1}{%
Die allgemeine Lösung ergibt sich durch Überlagerung von homogener und partikulärer Lösung.
}% Speech Ende
\speech{3Schaltvorgaenge4F06RLC1FallunterscheidungAperiodisch2}{2}{%
Die Konstanten K eins und K zwei können wie gewohnt durch Einsetzen der Anfangsbedingungen bestimmt werden.
Als Anfangsbedingungen seien die Kondensatorspannung U C von null Volt und der Strom I von null Ampere gegeben.
Durch Einsetzen in die allgemeine Lösung und deren erste Ableitung ergeben sich zwei Gleichungen mit zwei Unbekannten.
Durch Auflösen nach K eins und K zwei ergeben sich die hier dargestellten Ausdrücke.
}% Speech Ende
\end{frame}
...
\begin{align*}
    ...
    \\[-15pt] &\hphantom{1rrrrrrrrrrrrrrrr} &   \hphantom{2llllll}&\hphantom{\mathrel{\phantom{=}}2rrrrrrrrrrrrrrrrrrrrrrrrrrr}     &&\hphantom{3rrrrrrrrrrrrrrrrrrrrrrrr} \nonumber
    \end{align*}
\speech{3Schaltvorgaenge4F07RLC1FallunterscheidungGrenzfall1}{1}{%
Beim aperiodischen Grenzfall ist die Diskriminante D exakt null. Dadurch sind beide Eigenwerte identisch, reell und negativ.
}% Speech Ende
\speech{3Schaltvorgaenge4F07RLC1FallunterscheidungGrenzfall1}{2}{%
Die homogene Lösung ergibt sich durch Überlagerung der zwei Ansätze, einmal mit dem Exponentialterm und einmal mit dem Exponenzialterm multipliziert mit der Zeit t.
Die partikuläre Lösung entspricht wie in den anderen beiden Fällen der Gleichspannung U q.
}% Speech Ende
\end{frame}
...
\begin{frame}\ftx{\subsubsecname}% Aperiodischer Grenzfall: Zweiter Frame
    ...
    \begin{align*}
        ...
        &\text{2. AB:}&
            \frac{\mathrm{d}}{\mathrm{d}t} \ecv{u_C}(0) &= \left( K_1 + \redcancel{K_2 \cdot 0} \right) \cdot \lambda \cdot \redcancel{\mathrm{e}^{\lambda 0}} + K_2 \cdot \redcancel{\mathrm{e}^{\lambda 0}} \overset{!}{=} 0 & \Rightarrow\quad K_2 &= -K_1 \cdot \lambda \nonumber\\
            &\Longrightarrow&
        K_1 &= -\ecv{U_q} \quad\text{und}\quad
                K_2 = +\ecv{U_q} \cdot \lambda
    \end{align*}%
\speech{3Schaltvorgaenge4F08RLC1FallunterscheidungGrenzfall2}{1}{%
Durch Überlagerung von homogener und partikulärer Lösung ergibt sich die allgemeine Lösung.
Und durch Einsetzen der Anfangsbedingungen können die Konstanten K eins und K zwei bestimmt werden.
Achtung: In diesem Fall ergeben sich unterschiedliche Einheiten für K eins und K zwei.
}% Speech Ende
\end{frame}
...
\speech{3Schaltvorgaenge4F09RLC1FallunterscheidungPeriodisch1}{1}{%
Der periodische Fall ist durch eine negative Diskriminante D gekennzeichnet.
Dadurch ergeben sich zwei komplex konjugierte Eigenwerte mit negativem Realteil.
Durch Umformung mit der imaginären Einheit J lässt sich der Real- und Imaginärteil ablesen.
Hierfür wird die Wurzel aus dem negativen Term als J mal die Wurzel aus dem positiven Term dargestellt.
Der Betrag des negativen Realteils wird als Abklingkonstante Delta bezeichnet.
Der Betrag des Imaginärteils wird als gedämpfte Eigenkreisfrequenz Omega D bezeichnet.
}% Speech Ende
\speech{3Schaltvorgaenge4F09RLC1FallunterscheidungPeriodisch1}{2}{%
Im periodischen Fall entspricht die homogene Lösung einer exponentiell abklingenden sinusförmigen Schwingung mit der Abklingkonstante Delta und der gedämpften Eigenkreisfrequenz Omega D.
Die zu bestimmenden Konstanten sind die Amplitude C null und die Phasenverschiebung Phi null.
Eine genaue Herleitung findet sich im Video über Exkurse zu RLC Schwingkreisen.
Kurz gesagt, entspricht die homogene Lösung bei unterschiedlichen Eigenwerten allgemein der Überlagerung zweier Exponenzialfunktionen.
Durch Anwendung der Eulerschen Formel lassen sich diese in die hier gezeigte Form umwandeln.
}% Speech Ende
\end{frame}
...
\begin{frame}\ftx{\subsubsecname}% Periodischer Fall: Zweiter Frame
    ...
    \begin{minipage}{\textwidth}\centering
        ...
        \begin{minipage}[t][3cm][]{0.5\textwidth}\centering
            \vspace{10pt}
            \includegraphics[alt={Plot: Gezeigt ist der Einschwingvorgang eines RLC-Gliedes im periodischen Fall. Der Einschwingvorgang der Spannung uC entspricht einer exponentiell abklingenden Sinusschwingung um den angestrebten Endwert Uq herum. Der Verlauf beginnt bei uC=0. Als Hilfslinien sind eine ungedämpfte Sinusschwingung mit gleicher initalier Amplitude C0 und gleichem initialem Phasenwinkel phi0 eingezeichnet sowie die Einhüllende der abklingenden Schwingung von uC. Die Zeitkonstante der Einhüllenden entspricht 1/delta und die Periodendauer der Schwingung entspricht 2pi/omega d.}]{Tikz/pdf/plot_rlcs_charge_dc_uc_periodisch.pdf}
        \end{minipage}
    ...
    \begin{align*}
        ...
        &\text{2. AB:}&
            \frac{\mathrm{d}}{\mathrm{d}t} \ecv{u_C}(0) &= -\delta \cdot \redcancel{\mathrm{e}^{-\delta 0}} \cdot C_0 \cdot \sin(\varphi_0) + \mathrlap{\redcancel{\mathrm{e}^{-\delta 0}} \cdot C_0 \cdot \omega_d \cdot \cos(\varphi_0) \overset{!}{=} 0 }\nonumber\\
        &\Longrightarrow&
    ...
    \begin{figure}[H]\centering%
        \includegraphics[width=0.7\textwidth, alt={Plot: Gezeigt ist der Einschwingvorgang eines RLC-Gliedes im periodischen Fall. Der Einschwingvorgang der Spannung uC entspricht einer exponentiell abklingenden Sinusschwingung um den angestrebten Endwert Uq herum. Der Verlauf beginnt bei uC=0. Als Hilfslinien sind eine ungedämpfte Sinusschwingung mit gleicher initalier Amplitude C0 und gleichem initialem Phasenwinkel phi0 eingezeichnet sowie die Einhüllende der abklingenden Schwingung von uC. Die Zeitkonstante der Einhüllenden entspricht 1/delta und die Periodendauer der Schwingung entspricht 2pi/omega d.}]{Tikz/pdf/plot_rlcs_charge_dc_uc_periodisch.pdf}%
...
\speech{3Schaltvorgaenge4F10RLC1FallunterscheidungPeriodisch2}{1}{%
Die partikuläre Lösung, die allgemeine Lösung und die Bestimmung der Konstanten erfolgen wie gewohnt.
Oben rechts ist der zeitliche Verlauf der Kondensatorspannung U C im periodischen Fall dargestellt.
Wie zu erkennen ist, entspricht die Konstante C null der Schwingung der Amplitude zum Zeitpunkt null.
Die Phasenverschiebung Phi null gibt an, wie weit die Schwingung zu Beginn verschoben ist.
Die Abklingkonstante delta bestimmt die Geschwindigkeit des Abklingens der Schwingung und entspricht damit der Zeitkonstante der einhüllenden Exponenzialfunktion.
Die gedämpfte Eigenkreisfrequenz omega d bestimmt die Frequenz der Schwingung.
}% Speech Ende
\end{frame}
...
\newvideofile{SchaltvorgaengeRLC2Exkurse}{Schaltvorgänge bei RLC-Serienschwingkreisen (Exkurse) [2/2]}
\v{% Video only Frame
\begin{frame}\ftx{Entladevorgang \subsubsecname}
    \begin{Lernziele}{Video - Schaltvorgänge bei RLC-Serienschwingkreisen [2/2]}
        Sie lernen in diesem Video:
        \begin{itemize}\b{\setlength\itemsep{0.7em}}
            \item die Herleitung der homogenen Lösung im periodischen Fall zu verstehen,
            \item komplexer Eigenwerte im Ortskurvendiagramm zu analysieren, und
            \item die Zusammenhänge zwischen $\delta$, $\omega_d$, $\omega_0$ und $Q_S$ zu verstehen.
        \end{itemize}
    \end{Lernziele}
\speech{3Schaltvorgaenge5F01RLC2ExkurseLernziele}{1}{%
Willkommen zum letzten Video der Videoreihe für das Modul Schaltvorgänge. 
In diesem Video vertiefen wir einige Aspekte im Bezug auf Schaltvorgängen bei RLC-Gliedern. 
Wir lernen die Herleitung der homogenen Lösung im periodischen Fall zu verstehen.
Außerdem führen wir eine Nullstellenbetrachtung für komplexe Eigenwerte durch und wenden diese an.
Abschließend betrachten wir die Zusammenhänge zwischen der Abklingkonstante delta, der gedämpften Eigenkreisfrequenz omega d, der Resonanzkreisfrequenz omega null und der Schwingkreisgüte Q S.
}% Speech Ende
\end{frame}%
}% Video only Frame Ende
%
...
\end{align}%
\speech{3Schaltvorgaenge5F02RLC2ExkursePeriodisch1}{1}{%
Die homogene Lösung im periodischen Fall von RLC Schwingkreisen ergibt sich durch Einsetzen der komplexen Eigenwerte in die allgemeine Form der homogenen Lösung.
Durch Ausklammern des Exponentialterms e hoch minus delta T ergibt sich die obige Gleichung.
Damit die homogene Lösung, und damit auch die gesamte Lösung, reell ist, müssen die Konstanten K eins und K zwei komplex konjugiert sein.
Durch Einsetzen der komplex konjugierten Konstanten und Anwendung der Eulerschen Formel lassen sich die Exponenzialfunktionen in Sinus- und Kosinusfunktionen umwandeln.
Mit etwas Umformung, Sortieren und Kürzen ergibt sich die unten gezeigte Form der homogenen Lösung.
Diese entspricht der exponentiell abklingenden Überlagerung einer Sinus- und einer Kosinusschwingung bei gleicher Frequenz omega d.
}% Speech Ende
\end{frame}%
%
...
\begin{figure}[H]\centering
    \includegraphics[alt={Plot: Gezeigt ist die Überlagerung zweier sinusförmiger Schwingungen gleicher Frequenz links im Zeitverlaufsdiagramm und rechts im Zeigerdiagramm. Die Schwingungen z1 und z2 sind um 90 Grad phasenverschoben. Die Schwingung z0 resultiert aus der Überlagerung von z1 und z2. Im Zeigerdiagramm werden hierfür die komplexen Amplitudenzeiger für z1 und z2 vektoriell addiert. Der resultierende Phasenwinkel für z0 liegt nun zwischen dem für z1 und dem für z2. Die Amplitude für z0 ist größer als für die um 90 grad verschobenen z1 und z2.}]{Tikz/pdf/plot_zeigerdiagramm_schwingungen_addieren.pdf}\hfill% C1*cos(wt) - C2*sin(wt) = C0*sin(wt + phi0)
...
\b{% Anfang Folien only
    ...
    \end{align*}%
    \includegraphics[alt={Plot: Gezeigt ist die Überlagerung zweier sinusförmiger Schwingungen gleicher Frequenz links im Zeitverlaufsdiagramm und rechts im Zeigerdiagramm. Die Schwingungen z1 und z2 sind um 90 Grad phasenverschoben. Die Schwingung z0 resultiert aus der Überlagerung von z1 und z2. Im Zeigerdiagramm werden hierfür die komplexen Amplitudenzeiger für z1 und z2 vektoriell addiert. Der resultierende Phasenwinkel für z0 liegt nun zwischen dem für z1 und dem für z2. Die Amplitude für z0 ist größer als für die um 90 grad verschobenen z1 und z2.}]{Tikz/pdf/plot_zeigerdiagramm_schwingungen_addieren.pdf}% C1*cos(wt) - C2*sin(wt) = C0*sin(wt + phi0)%
    \vfill%
    \begin{align*}
        &&
...
\speech{3Schaltvorgaenge5F03RLC2ExkursePeriodisch2}{1}{%
Die Überlagerung zweier Schwingungen gleicher Frequenz kann zu einer einzelnen Schwingung mit gleicher Frequenz, aber anderer Amplitude und Phasenverschiebung zusammengefasst werden.
In der Abbildung sind links die Zeitverläufe und rechts die komplexen Amplitudenzeiger im Zeigerdiagramm dargestellt.
In diesem Beispiel wird die blaue Schwingung z 1 und die rote Schwingung z 2 überlagert. 
Die Amplitude und Phasenverschiebung der resultierenden Schwingung z null können durch geometrische Addition der Amplitudenzeiger im Zeigerdiagramm bestimmt werden.
Die zu bestimmenden Konstanten sind entweder C eins und C zwei in der Schreibweise oben oder C null und Phi null in der Schreibweise unten.
}% Speech Ende
\end{frame}%
%
...
\speech{3Schaltvorgaenge5F04RLC2ExkursePeriodisch3}{1}{%
Hier ist die selbe Überlagerung noch einmal in mathematischer Form dargestellt.
Links sind die Zeitfunktionen der einzelnen Schwingungen und der resultierenden Schwingung gezeigt.
In der Mitte sind die komplexen Amplitudenzeiger in Polarform und rechts in kartesischer Form dargestellt.
Die Amplitude C null der resultierenden Schwingung ergibt sich aus dem Satz des Pythagoras.
Die Phasenverschiebung Psi der resultierenden Schwingung ergibt sich aus dem Arkustangens des Verhältnisses der Amplituden C zwei und C eins.
Welche Konstanten letztlich zu bestimmen sind, hängt von der gewählten Form der homogenen Lösung ab.
In der unten gezeigten Form ist dabei am deutlichsten zu erkennen, dass der flüchtige Zustand im periodischen Fall einer abklingenden sinusförmigen Schwingung entspricht.
}% Speech Ende
\end{frame}
...
\newpage% Achtung: Layout für eine A4-Seite im Skript vorgesehen!
\subsubsection[Ortskurvendiagramm, Nullstellen]{Fallunterscheidung bei RLC-Serienschwingkreis, Nullstellen}
...
\s{% Anfang Skript only
    ...
    \begin{figure}[H]\centering
        \begin{subfigure}{0.48\textwidth}\centering
            \includegraphics[alt={Plot: Gezeigt ist eine Ortskurve mit eingezeichneten Eigenwerten eines RLC Serienschwingkreises für die drei Fälle und wie diese sich von Fall zu Fall bewegen. Für D>0 sind beide Eigenwerte verschieden negativ reell und bewegen sich aufeinander zu. Für D=0 sind beide Eigenwerte identisch und negativ reell. Für D<0 bewegen sind die Eigenwerte komplex konjugiert und bewegen sich parallel zur Imaginärachse weg von der reellen Achse.}]{Tikz/pdf/plot_rlcs_ortskurve_nst.pdf}
    ...
    Für verschwindende Anfangsbedingungen ($\frac{\mathrm{d}^{i}}{\mathrm{d}t^{i}}\,u_C(0)=0\ \forall i$) lässt sich die DGL aus Gl. \ref{eq:dgl:rlc:dc}
    ...
    \begin{align}
        u(t) &= LC \cdot \frac{\mathrm{d}^{2}}{\mathrm{d}t^{2}}\,u_{C}(t) + RC \cdot \frac{\mathrm{d}}{\mathrm{d}t}\,u_{C}(t) + u_{C}(t)& &\bigg|\,\mathscr{L}\\
...
\speech{3Schaltvorgaenge5F05RLC2ExkurseOrtskurve}{1}{%
In diesem Exkurs betrachten wir die Fallunterscheidung bei RLC Serienschwingkreisen anhand der Lage der Eigenwerte im komplexen Zahlenraum.
Diese Betrachtungsweise wird in ähnlicherweise auch in der Systemtheorie zur Stabilitätsanalyse von Systemen verwendet. 
Die Eigenwerte entsprechen dabei den Polstellen der Übertragungsfunktion des Systems. Ein genauerer Vergleich wird im Skript angestellt.
Im Ortskurvendiagramm links sind die exemplarisch Eigenwerte für alle drei Fälle dargestellt. 
Da der Realteil aller Eigenwerte in jedem Fall negativ ist, ist das System immer stabil.
Im aperiodischen Fall haben wir zwei reelle, negative Eigenwerte. Dabei dominiert die Nullstelle näher an der Imaginärachse das Einschwingverhalten des Systems, da die zugehörige Exponenzialfunktion langsamer abklingt.
Bei konstanter Abklingkonstante und zunehmender ungedämpfter Eigenkreisfrequenz, mehr dazu gleich, bewegen sich die Eigenwerte aufeinander zu und verschmelzen im aperiodischen Grenzfall zu einer doppelten Nullstelle.
Diese ist, bei konstanter Abklingkonstante, der schnellste aperiodische Fall.
Wenn die ungedämpfte Eigenkreisfrequenz weiter erhöht wird, bewegen sich die Eigenwerte in den periodischen Fall.
Die Eigenwerte sind dann komplex konjugiert und bewegen sich bei zunehmender ungedämpfter Eigenkreisfrequenz parallel zur Imaginärachse nach oben und unten.
Mit zunehmendem Abstand von der Realachse, steigt auch die Schwingungsfähigkeit des Systems und damit die Frequenz der Schwingung. 
}% Speech Ende
\end{frame}
...
&\text{Vgl. homo. DGL:}&
    %0 &= \frac{\mathrm{d}^{2}}{\mathrm{d}t^{2}}\,y + 2\delta \cdot \frac{\mathrm{d}}{\mathrm{d}t}\,y + \omega_0^2 \cdot y&
    0 &= \ddot y + 2\delta \cdot \dot y + \omega_0^2 \cdot y&
    &\text{linear gedämpfte Schwingung} \vphantom{\bigg|}
\end{align*}%
\speech{3Schaltvorgaenge5F06RLC2ExkurseGroessen}{1}{%
In diesem Exkurs betrachten wir den Zusammenhang von Größen bei der Betrachtung von Resonanzphänomenen und von periodischen Schaltvorgängen bei RLC-Serienschwingkreisen.
Resonanzen treten auf, wenn die Frequenz einer externen Anregung mit der Resonanzfrequenz F null, übereinstimmt. 
Resonanzphänomene werden ausführlich in Modul acht erläutert.
Die Resonanzfrequenz F null wird auch ungedämpfte Eigenfrequenz genannt, da sie die Frequenz ist, bei der das System ohne äußere Dämpfung, in diesem Fall ohne Widerstand R, schwingen würde.
Die Gleichung für die zugehörige Resonanzkreisfrequenz Omega null ergibt sich aus der Resonanzbedingung, dass der Blindanteil der Gesamtimpedanz bei Resonanz verschwindet.
An der Gleichung für die Eigenwerte oben rechts können wir durch Einsetzen den Zusammenhang von Omega D und Omega null erkennen.
}% Speech Ende
\silence{1}%
\speech{3Schaltvorgaenge5F06RLC2ExkurseGroessen}{1}{%
Die gedämpfte Eigenkreisfrequenz Omega D entspricht dem geometrischen Mittel aus Omega null und Delta,
das heißt der Wurzel aus der Differenz von Omega null zum Quadrat und Delta zum Quadrat.
Bei verschwindender Dämpfung, also für Delta gleich null, ist die Omega D gleich Omega null.
Ein weiterer Zusammenhang besteht zwischen der Schwingkreisgüte Q S, kurz Güte genannt, und dem Verhältnis von Omega null zu Delta.
Die Güte ist ein Maß für die Schwingungsfähigkeit eines Schwingkreises.
Sie entspricht dem Verhältnis von Blindleistung zu Verlustleistung im Resonanzfall. Beim Serienschwingkreis entspricht Q damit dem Verhältnis vom Blindwiderstand der Induktivität bei Resonanz zur Serienresistenz R.
Die selbe Umformung für die Güte kann auch für den RLC Parallelschwingkreis angewendet werden.
An der Gleichung für die Eigenwerte oben rechts können wir durch Einsetzen den Zusammenhang von Omega D und Omega null erkennen.
}% Speech Ende
\silence{1}%
\speech{3Schaltvorgaenge5F06RLC2ExkurseGroessen}{1}{%
In beiden Fällen ergibt sich der folgende Zusammenhang.
Die Güte ist proportional zum Verhältnis von Omega null zu Delta. 
Dadurch kann die Güte ebenso als Indikator für die Fallunterscheidung herangezogen werden.
Anhand der Koeffizienten der homogenen DGL lassen sich so die Größen Delta und Omega null direkt ablesen.
Die DGL in der unten gezeigten Form beschreibt allgemein das Verhalten von Systemen zweiter Ordnung mit linear gedämpfter Schwingung.
