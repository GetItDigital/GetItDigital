\subsection{Untersuchung der Verstärkung eines OPVs mit variablen Widerständen}
...
\Aufgabe{
    ...
    \begin{figure}[H]
        \centering
        \input{Tikz/src/FigPrakNIOPV.tex}\alttext{Das Bild zeigt einen elektrischen Schaltkreis mit zwei Spannungsquellen \(U_1\) und \(U_2\), die jeweils mit Masse verbunden sind. Oben links ist die Spannungsquelle \(U_1\), die über den Widerstand \(R_1\) zu einem Knoten führt. Von diesem Knoten geht ein Potentiometer \(P_1\) nach unten zu einem weiteren Widerstand \(R_2\), der mit der zweiten Spannungsquelle \(U_2\) verbunden ist. Zwischen dem Potentiometer \(P_1\) und dem Widerstand \(R_\mathrm{E}\) befindet sich der Spannungspunkt \(U_\mathrm{E}\). Der Widerstand \(R_\mathrm{E}\) ist mit dem nicht-invertierenden Eingang (+) eines Operationsverstärkers verbunden. Der invertierende Eingang (-) des Operationsverstärkers ist direkt mit Masse verbunden. Vom Ausgang des Operationsverstärkers führt eine Leitung über den Widerstand \(R_\mathrm{R}\) zum Lastwiderstand \(R_{\mathrm{Last}}\). Die Spannung über dem Lastwiderstand wird als \(U_\mathrm{A}\) bezeichnet. Der Stromfluss in verschiedenen Teilen des Schaltkreises ist durch blaue Pfeile markiert.}
        \label{fig:FigPrakNIOPV}
    \end{figure}
}

\Loesung{
    \textbf{a) Berechnung der Ausgangsspannung $U_\mathrm{A}$} \\
    Gegeben sind:
    \[
