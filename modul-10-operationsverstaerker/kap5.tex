    ...
    \s{
        ...
        \begin{bsp}{Beispiel Analogrechner}{Beispiel Analogrechner}
            ...
            \fu{
                ...
                \alttext{Die Abbildung zeigt eine zweistufige Operationsverstärker-Schaltung, die in zwei farblich markierte Bereiche unterteilt ist.
                Im linken, gelb markierten Bereich befindet sich die erste Verstärkerstufe. Der nichtinvertierende Eingang des Operationsverstärkers ist mit Masse verbunden. Das Eingangssignal \(U_{\mathrm{E1}}\) über den Widerstand \(R_1\) dem invertierenden Eingang zugeführt. Vom Ausgang führt eine Rückkopplung sowohl über einen Widerstand \(R_2\) als auch über einen Kondensator \(C\) zurück zum invertierenden Eingang. Dadurch entsteht eine frequenzabhängige Rückkopplung, sodass diese Stufe als aktives Filter arbeitet.
                Im rechten, violett markierten Bereich befindet sich die zweite Verstärkerstufe. Das Signal der ersten Stufe wird über den Widerstand \(R_3\) an den invertierenden Eingang des zweiten Operationsverstärkers geführt. Der nichtinvertierende Eingang liegt ebenfalls auf Masse. Zusätzlich wird ein zweites Eingangssignal \(U_{\mathrm{E2}\)} über den Widerstand \(R_4\) eingespeist. Über den Widerstand \(R_5\) erfolgt die Rückkopplung vom Ausgang zum invertierenden Eingang.
                Am Ausgang der zweiten Stufe ist die Ausgangsspannung \(U_\mathrm{A}\) eingezeichnet.}}
            ...
            Eine Wahl von x=0 ist nur möglich, wenn der Widerstand $R_2$ weggelassen wird. In diesem Fall ergibt sich ein \glqq idealer\grqq{} OPV, der real so in der Regel jedoch nicht realisiert wird.
...
\begin{frame}
    \b{
        ...
        \begin{equation}
            U_{\textnormal{A}} = \underbrace{\left[ -\frac{1}{R_1 C} \int_{0}^{t} U_{\textnormal{E1}}~dt -\frac{R_2}{R_1} \cdot U_{\textnormal{E1}} \right]}_{Formel~des~Integrators} \underbrace{\cdot\left(-\frac{R_5}{R_3}\right)-\frac{R_5}{R_4}\cdot U_{\textnormal{E2}}}_{Formel~des~Summierers}
        \end{equation}
        Dies kann nun wie folgt umgeformt werden
        \begin{equation}
            U_{\textnormal{A}} = \underbrace{\frac{R_5}{R_1 R_3 C}}_{\stackrel{!}{=}1} \int_{0}^{t} U_{\textnormal{E1}}~dt + \underbrace{\frac{R_2 R_5}{R_1 R_3}}_{\stackrel{!}{=}x} \cdot U_{\textnormal{E1}} -\underbrace{\frac{R_5}{R_4}}_{\stackrel{!}{=}2} \cdot U_{\textnormal{E2}}
        \end{equation}
