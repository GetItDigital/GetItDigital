\begin{circuitikz}[scale=0.7, transform shape]
    \ctikzset{
         ...
         \fill[yellow, opacity=0.2] (-3.2,-1.2) rectangle (1.3,3.5);
         \fill[violet, opacity=0.2] (1.6,-1.6) rectangle (7,2.5);
         \draw[yellow, thick] (-3.2,-1.2) rectangle (1.3,3.5);
         \draw[violet, thick] (1.6,-1.6) rectangle (7,2.5);

         \draw (0,0) node[en amp] (opamp) {}
         (opamp.+) --++ (-0.5,0) node[ground] {} 
         (opamp.-) -- ++(0, 0) to[R, l_=$R_1$] ++(-1.2, 0) node[ocirc, label=left:$U_\mathrm{E1}$] {}
        ...
        (opamp2.-) -- ++(0,-0.5) -- ++(-0.0,0) to[R, l=$R_4$] ++(-1.438,0) node[ocirc, label=left:$U_\mathrm{E2}$] {}
        (opamp2.out) to[short, *-o] ++(0.2,0) node[right] {$U_\mathrm{A}$};
