\begin{circuitikz}[scale=0.6, transform shape]
    ...
    (opamp1.+) --++ (-0.5,0) node[ocirc, label=left:$U_\mathrm{E2}$] (eingang1) {};
    
   \draw (2,0) node[en amp, noinv input up] (opamp2) {}
    (opamp2.+) --++ (-0.5,0) node[ocirc, label=left:$U_\mathrm{E1}$] (eingang2) {}
   ...
   % Dritter OPV mit angepasstem Eingangsabstand
    ...
    (opamp3.out) node[circ, label=below:{$U_\mathrm{A}$}]{} -- ++(0, 2) to[R=$R_5$] ++(-2,0) -- ++(0, -1.45)node[circ]{};
