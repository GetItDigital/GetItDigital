...
\begin{frame}[t]
	...
	\begin{table}[h!]
		...
		\begin{tabular}{|c|c|c|c|}
			...
			\onslide<2->{Zeit} & \onslide<2->{$t$} & \onslide<2->{Sekunde, s} & \onslide<2->{$\Delta\nu$} \\ \hline
			\onslide<3->{Länge} & \onslide<3->{$\ell$} & \onslide<3->{Meter, m} & \onslide<3->{$c, \mathrm{s}$} \\ \hline
			\onslide<4->{Masse} & \onslide<4->{$m$} & \onslide<4->{Kilogramm, kg} & \onslide<4->{$h, \mathrm{s, m}$} \\ \hline
			\onslide<5->{Stromstärke} & \onslide<5->{$I$} & \onslide<5->{Ampere, A} & \onslide<5->{$e, s$} \\ \hline
			\onslide<6->{Temperatur} & \onslide<6->{$T$} & \onslide<6->{Kelvin, K} & \onslide<6->{$k_\mathrm{B}, \mathrm{s, m, kg}$} \\ \hline
			\onslide<7->{Stoffmenge} & \onslide<7->{$n$} & \onslide<7->{Mol, mol} & \onslide<7->{$N_\mathrm{A}$} \\ \hline
			\onslide<8->{Lichtstärke} & \onslide<8->{$I_v$} & \onslide<8->{Candela, cd} & \onslide<8->{$K_{\mathrm{cd}}, \mathrm{s, m, kg}$} \\ \hline
		\end{tabular}
	...
	\b{
	\only<handout:0| beamer:2>{Eine Sekunde ist das 9.192.631.770-fache der Periodendauer des
	 Hyperfeinstrukturübergangs $\Delta v$ im Cäsium-Atom $ ^{133}Cs.$ \\

	 \phantom{.}\\
	$\Delta \nu$ = 9.192.631.770 Hz \quad    (Hyperfeinstrukturübergang $ ^{133}$Cs)}

	\only<handout:0| beamer:3>{Ein Meter ist die Länge der Strecke, die das Licht im Vakuum während der Dauer
	...
	\only<handout:0| beamer:4>{Das Kilogramm ist seit Mai 2019 vom genau definierten Planckschen Wirkungsquantum
...
\only<handout:1| beamer:5>{Das Ampere ist seit 2019 durch die Elementarladung definiert. Ein Ampere ist der
...
\only<handout:0| beamer:6>{Ein Kelvin entspricht einer Änderung der thermodynamischen Temperatur, die mit
einer Änderung der thermischen Energie ($kT$) um $1,380649 \cdot 10^{-23} \, \mathrm{J}$ einhergeht.

\phantom{.}\\

$k_\mathrm{B} = 1,380 649 \cdot 10^{-23} \, \frac{\mathrm{kg \cdot m^2}}{\mathrm{s^2 \cdot K}} $ \quad (Boltzmann-Konstante)
}

\only<handout:0| beamer:7>{Ein Mol ist die Stoffmenge eines Systems, das $6,022 140 76 \cdot 10^{23}$ eines bestimmten
...
\only<handout:0| beamer:8>{Eine Candela ist die Lichtstärke einer Strahlungsquelle, die mit einer Frequenz von
...
\begin{itemize}
	...
	\item Boltzmann-Konstante: $k_\mathrm{B} = 1,380 649 \cdot 10^{-23} \, \frac{\mathrm{kg \cdot m^2}}{\mathrm{s^2 \cdot K}} $
...
\begin{frame}[t]{Abgeleitete SI-Einheiten}
	\vspace{0.25cm}% Abstand wegen Top-aligned frame für "ruckelfreie" Overlays
...
\begin{table}[h!]
	...
	\begin{tabular}{|c|c|c|c|}
		...
		\onslide<2->{Kraft} & \onslide<2->{F} & \onslide<2->{Newton, N} & \onslide<2->{1 N = 1 kg m/s$^2$} \\ \hline
		\onslide<3->{Energie} & \onslide<3->{E} & \onslide<3->{Joule, J} & \onslide<3->{1 J = 1 Ws = 1 Nm = 1 kg m$^2$/s$^2$} \\ \hline
		\onslide<4->{Leistung} & \onslide<4->{P} & \onslide<4->{Watt, W} & \onslide<4->{1 W = 1 J/s = 1 kg m$^2$/s$^3$} \\ \hline
		\onslide<5->{Spannung} & \onslide<5->{U} & \onslide<5->{Volt, V} & \onslide<5->{1 V = 1 W/A = 1 Nm/As = 1 kg m$^2$/s$^3$A} \\ \hline
		\onslide<6->{Ladung} & \onslide<6->{Q} & \onslide<6->{Coulomb, C} & \onslide<6->{1 C = 1 As} \\ \hline
		\onslide<7->{Widerstand} & \onslide<7->{R} & \onslide<7->{Ohm, $\Omega$} & \onslide<7->{1 $\Omega$ = 1 V/A = 1 kg m$^2$/s$^3$A$^2$} \\ \hline
		\onslide<8->{Kapazität} & \onslide<8->{C} & \onslide<8->{Farad, F} & \onslide<8->{1 F = 1 As/V = 1 s$^4$A$^2$/kg m$^2$} \\ \hline
		\onslide<9->{Induktivität} & \onslide<9->{L} & \onslide<9->{Henry, H} & \onslide<9->{1 H = 1 Vs/A = 1 kg m$^2$/s$^2$A$^2$} \\ \hline
		\onslide<10->{magn. Fluss} & \onslide<10->{$\Phi$} & \onslide<10->{Weber, Wb} & \onslide<10->{1 Wb = 1 Vs = 1 kg m$^2$/s$^2$A} \\ \hline
		\onslide<11->{Flussdichte} & \onslide<11->{B} & \onslide<11->{Tesla, T} & \onslide<11->{1 T = 1 Vs/m$^2$ = 1 kg/s$^2$A} \\ \hline
	\end{tabular}
...
\only<handout:1 |beamer:2>{Kraft ist Masse mal Beschleunigung: $F = m \cdot a$\\}
\only<handout:0 |beamer:3>{Oft vergleichbar mit Arbeit, wichtig für Energieerhaltung, Einheit auch Wattsekunde\\}
\only<handout:1 |beamer:4>{Leistung ist Arbeit pro Zeit\\}
\only<handout:0 |beamer:5>{”Stärke“einer Spannungsquelle - Analog zum Druck in einer Leitung\\}
\only<handout:0 |beamer:6>{Menge an freien Elementarladungen\\}
\only<handout:0 |beamer:7>{"Reibung" von elektrischem Strom\\}
\only<handout:0 |beamer:8>{Fähigkeit, Ladungen zu speichern\\}
\only<handout:0 |beamer:9>{Zentrale Eigenschaft einer elektrischen Spule\\}
\only<handout:0 |beamer:10>{Analoge Eigenschaft eines Magnetfeldes zum elektrischen Strom\\}
\only<handout:0 |beamer:11>{Gibt Stärke des magnetischen Flusses an, z.B. im MRT\\}
...
\begin{frame}
	...
	\begin{table}[h!]
		...
		\begin{tabular}{|c|c||c|c|}
			...
			\onslide<2->{Dezi, d} & \onslide<2->{$10^{-1}$} & \onslide<2->{$10^1$} & \onslide<2->{Deka, da} \\ \hline
			\onslide<3->{Zenti, c} & \onslide<3->{$10^{-2}$} & \onslide<3->{$10^2$} & \onslide<3->{Hekto, h} \\ \hline
			\onslide<4->{Milli, m} & \onslide<4->{$10^{-3}$} & \onslide<4->{$10^3$} & \onslide<4->{Kilo, k} \\ \hline
			\onslide<5->{Mikro, $\mu$} & \onslide<5->{$10^{-6}$} & \onslide<5->{$10^6$} & \onslide<5->{Mega, M} \\ \hline
			\onslide<6->{Nano, n} & \onslide<6->{$10^{-9}$} & \onslide<6->{$10^9$} & \onslide<6->{Giga, G} \\ \hline
			\onslide<7->{Piko, p} & \onslide<7->{$10^{-12}$} & \onslide<7->{$10^{12}$} & \onslide<7->{Tera, T} \\ \hline
			\onslide<8->{Femto, f} & \onslide<8->{$10^{-15}$} & \onslide<8->{$10^{15}$} & \onslide<8->{Peta, P} \\ \hline
			\onslide<9->{Atto, a} & \onslide<9->{$10^{-18}$} & \onslide<9->{$10^{18}$} & \onslide<9->{Exa, E} \\ \hline
		\end{tabular}
	...
	\b{
	\hspace{75pt}\onslide<10->{$1 \, \mathrm{m} = 100 \cdot 10^{-2} \, \mathrm{m} = 100\,\mathrm{cm}$}

	\hspace{75pt}\onslide<11->{$1 \, \mathrm{m^2} = 100^2 \cdot (10^{-2} \, \mathrm{m})^2  = 10.000 \, \mathrm{cm}^2$}

	\hspace{75pt}\onslide<12->{$1 \, \mathrm{m^3} = 100^3 \cdot (10^{-2} \, \mathrm{m})^3  = 1.000.000 \, \mathrm{cm}^3$}
