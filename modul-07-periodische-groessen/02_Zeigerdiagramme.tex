...
\begin{frame}
    ...
    \fu{
        ...
        \resizebox{0.95\textwidth}{!}{
            \input{Tikz/src/Zeigerdiagramme/Periodische_Wechselspannung}\alttext{Das Bild zeigt links einen zeitabhängigen Sinusverlauf u(t) über der Zeit t mit markierter Periodendauer T und Nulldurchgängen. Die Amplitude Û ist als gestrichelte Linie eingezeichnet, die Zeitachse ist in Millisekunden beschriftet. Rechts ist ein Zeigerdiagramm dargestellt, das die gleiche Schwingung als rotierenden Zeiger mit Winkel φᵤ zeigt. Die Kreisbewegung veranschaulicht die Beziehung zwischen Zeitverlauf und Phasenwinkel.}
        }
...
\begin{frame}
    ...
    \s{
        Die Kreisfrequenz\index{Kreisfrequenz} $\omega$ kann mithilfe der Periodendauer $T$ oder der Frequenz $f$ angegeben werden. Weiter wird die
...
\begin{frame}
    \ftx{Harmonische\index{Harmonische} Wechselgrößen}
    ...
    \b{
        \begin{itemize}
            \item <1-> Eine Periodendauer entspricht in Winkelgrößen einem Kreisumlauf von $360^o$ oder $2\pi$
            \item <2-> Die Phasenverschiebung $t_\mathrm{0}$ entspricht in der Winkeldarstellung dem Phasenwinkel $\varphi_\mathrm{N}$
        \end{itemize}
    }

    \fu{
        \input{Tikz/src/Zeigerdiagramme/Verschiedene_Massangaben}\alttext{Das Bild zeigt drei übereinanderliegende Skalen für dieselbe Phase: Zeit in Millisekunden, Winkel in Grad und Winkel im Bogenmaß. Eine gestrichelte vertikale Linie markiert den Zeitpunkt t₀ und die zugehörige Phase Phi_N = 0. Die Gradskala reicht von 0 bis 360 Grad, die Bogenmaßskala von 0 bis 2π. Die Darstellung verdeutlicht die Umrechnung zwischen Zeit und Phasenwinkel.}
...
\begin{frame}
    ...
    \s{\index{Amplitude}\index{Drehzeiger}
    ...
    \fu{
        \input{Tikz/src/Zeigerdiagramme/Spannungs_und_Stromzeiger}\alttext{Das Bild zeigt zwei komplexe Zeigerdiagramme mit Realachse (Re) und Imaginärachse (Im). Links ist der Spannungszeiger Û in Blau mit dem Phasenwinkel Phi_U zur Realachse dargestellt. Rechts ist der Stromzeiger Î in Rot mit dem Phasenwinkel Phi_I eingezeichnet. Die Grafik vergleicht die Phasenlage von Spannung und Strom.}
...
\begin{frame}
    ...
    \fu{
        \input{Tikz/src/Zeigerdiagramme/Zeigerdarstellung_komplexe_Wechselspannung}\alttext{Das Bild zeigt ein komplexes Zeigerdiagramm der Spannung U mit Realteil Re(U) und Imaginärteil Im(U). Ein Pfeil vom Ursprung stellt den Spannungszeiger mit Betrag Û und Phasenwinkel Phi_U dar. Gestrichelte Linien zeigen die Projektionen als Û·cos(ωt + φᵤ) auf die Realachse und Û·sin(ωt + φᵤ) auf die Imaginärachse. Zusätzlich ist die komplexe Darstellung U = Û · e^{jφᵤ} angegeben.}
...
\begin{frame}
    ...
    \fu{
        \resizebox{\textwidth}{!}{
            \input{Tikz/src/Zeigerdiagramme/Bauteile_Wechselspannungszeiger}\alttext{Das Bild zeigt drei elektrische Bauelemente: Widerstand (R), Kondensator (C) und Spule (L) mit zugehörigen Strom- und Spannungsrichtungen. Darunter sind jeweils Zeigerdiagramme mit Real- und Imaginärachse dargestellt. Beim Widerstand sind Strom und Spannung phasengleich. Beim Kondensator eilt der Strom der Spannung voraus, bei der Spule hinkt der Strom der Spannung nach.}
        }
