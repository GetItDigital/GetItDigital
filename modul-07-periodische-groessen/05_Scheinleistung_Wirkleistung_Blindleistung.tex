...
\newvideofile{Leistung}{Wirkleistung, Blindleistung und Scheinleistung}

\begin{frame}
    \fta{Scheinleistung, Wirkleistung und Blindleistung}

    \s{\index{Scheinleistung}\index{Wirkleistung}\index{Blindleistung}
    ...
    \speech{LeistungLrnz}{1}{Durch die Verwendung von Wechselströmen sind Leistungen nicht Konstant
    und müssen daher komplex betrachtet werden.
    Am Ende des Kapitels soll der Unterschied zwischen der Augenblicksleistung,
    der Wirkleistung und der Blindleistung bekannt sein.
    Sowie die Scheinleistung bestimmt und kategorisiert werden können.}
\end{frame}
...
\begin{frame}
    ...
    \speech{LeistungAddit}{1}{Ein paar Grundlagen zuvor.
    Zur Herleitung der Augenblicksleistung aus sinusförmigen Quellen
    kann das angegebene Additionstheorem genutzt werden.}
    \speech{LeistungAddit}{2}{Über den Arithmetischen Mittelwert wird ein konstanter Wert für die Leistung bestimmt.
    Das Vorgehen ist hier ähnlich wie bei der Berechnung des Effektivwertes. .}
    \speech{LeistungAddit}{3}{Groß Tee beschreibt hier die Periodendauer und klein Tee den Startpunkt des Betrachtungszeitraumes.
    Oft wird klein Tee als Null gewählt.}
\end{frame}
...
\begin{frame}
    ...
    \b{
        ...
        \begin{itemize}
            \item<2-> Zuerst wird eine Funktion über eine Periodenlänge aufgestellt
            \begin{eq}
                u(t)=\frac{10~V}{T}\cdot t
            \end{eq}
            \item<3-> Arithmetischer Mittelwert $\overline{u(t)}$:
        \end{itemize}
    }

    \fu{
        \resizebox{0.7\textwidth}{!}{
            \input{Tikz/src/Scheinleistung_Wirkleistung_Blindleistung/Saegezahnspannung_Verlauf}\alttext{Das Bild zeigt einen sägezahnförmigen Spannungsverlauf u(t) über der Zeit t. Die Spannung steigt linear von 0 V auf etwa 10 V an und fällt bei den Zeitpunkten T und 2T abrupt auf 0 V zurück. Eine rote gestrichelte Linie markiert den Mittelwert ū(t) bei etwa 5 V. Die schraffierte Fläche verdeutlicht die Mittelwertbildung über eine Periode.}
        }
    ...
    \speech{LeistungArith}{1}{Anhand der angegebenen sonderform einer Dreickecksspannung,
    der Sägezahnspannung, soll der Mittelwert bestimmt werden.
    Die Sägezahnspannung ist hier um einen Gleichanteil angehoben.}
    \speech{LeistungArith}{2}{Hierfür wird die Funktion über eine Periode aufgestellt.}
    \speech{LeistungArith}{3}{Durch den gezeigten Verlauf kann bereits abgeschätzt werden,
    dass der Mittelwert bei Fünf Volt liegen sollte.}
\end{frame}
...
\begin{frame}
    ...
    \speech{LeistungArithBsp}{1}{Die bereits aufgestellte Funktion für den Verlauf der Sägezahnspannung
    wird in die Gleichung für den arithmetischen Mittelwert eingesetzt.}
    \speech{LeistungArithBsp}{2}{So erhält man den dargestellten Ausdruck des arithmetischen Mittelwertes für die Sägezahnspannung.
    Für die untere Grenze Null ergibt sich der Ausdruck zu Null.
    Wird für klein Tee, Groß Tee eingesetzt,
    lässt sich das Tee wegkürzen und man gelangt zu dem Ergebnis von Fünf Volt.}
    \speech{LeistungArithBsp}{3}{Bei reinen Wechselgrößen wird der Mittelwert durch die symmetrie des Verlaufes immer zu Null.}
\end{frame}
...
\begin{frame}
    ...
    \speech{LeistungGleich}{1}{Die Leistung bei Gleichstrom ergibt sich aus dem Produkt der Spannung und dem Strom.
    Durch das Umstellen lässt sich die Leistung auch Über die Quadtratur des Stromes oder der Spannung bestimmen, .}
\end{frame}
...
\begin{frame}
    ...
    \speech{LeistungAugenblick}{1}{Bei Wechselstrom wird die Leistung mit den zeitabhängigen Wechselgrößen berechnet, .}
    \speech{LeistungAugenblick}{2}{Die Zeitabhängigen Größen sind die Spannung Uh von Tee und der Strom Ih von Tee in der angegebenen Form.}
    \speech{LeistungAugenblick}{3}{Die Augenblicksleistung kann über das Produkt für jeden Zeitpunkt Tee berechnet werden.
    Über das zum Anfang vorgestellte Additionsteorehm lässt sich die Augenblicksleistung für Sinusförmige Wechselgrößen bestimmen.
    Hierbei lassen sich die Trigonometrischen Funktionen in einen konstanten Anteil und einen zeitabhängigen Anteil,
    welcher von Tee abhängig ist, unterteilen.}
\end{frame}
...
\begin{frame}
    ...
    \begin{Merksatz}{Augenblicksleistung}
        Die Augenblicksleistung ist das Produkt aus den Augenlickswerten der Spannung und des Stromes.
    ...
    \speech{LeistungMrk1}{1}{Die Augenblicksleistung Pe von Tee ist das Produkt aus den Augenlickswerten der Spannung Uh von Tee und des Stromes Ih von Tee. }
\end{frame}
...
\begin{frame}
    ...
    \b{
        \begin{itemize}
            \item<3-> Mittlere Leistung an einem idealen Wirderstand ohne Phasenverschiebung zwischen Spannung und Strom:
                \begin{eq}
    ...
    \speech{LeistungMittl}{1}{Bei der Berechnung der Leistung wird mit den Effektivwerten der Spannung und des Stromes gerechnet.}
    \speech{LeistungMittl}{2}{Außerdem spielt hier die Phasenverschiebung zwischen der Spannung und dem Strom eine Rolle.
    Bei der Leistungsberechnung wird dieser Faktor als Leistungsfaktor bezeichnet.
    Der Leistungsfaktor gibt außerdem das Verhältnis zwischen den verschiedenen Leistungsarten an.}
    \speech{LeistungMittl}{3}{An einem idealen Widerstand, also einen rein ohmschen Verbraucher ohne Blindanteil,
    Beträgt die Phasenverschiebung zwischen dem Strom und der Spannung Null.
    So beträgt der Leistungsfaktor Eins, sodass reine Wirkleistung übertragen wird.}
\end{frame}
...
\begin{frame}
    ...
    \b{
        ...
        \begin{itemize}
            ...
            \item<2-> Gleichanteil gleich null
    ...
    \fu{
        \resizebox{0.8\textwidth}{!}{
            \input{Tikz/src/Scheinleistung_Wirkleistung_Blindleistung/Verlauf_induktive_Last}\alttext{Das Bild zeigt drei sinusförmige Zeitverläufe über der Zeit t. Die Spannung u(t) ist blau, der Strom i(t) rot und die Leistung p(t) grün dargestellt. Spannung und Strom sind phasenverschoben, wodurch die Leistung zwischen positiven und negativen Werten schwankt. Die Zeitachse ist von 0 bis 4π beschriftet.}
        }
    ...
    \speech{LeistungMittlIndu}{1}{Die Leistung an einer Induktivität wird ähnlich ermittelt.
    Wie bereits bekannt sind an einer Induktivität die Spannung und der Strom Phasenverschoben.
    Die Verläufe werden in Blau und Rot dargestellt.}
    \speech{LeistungMittlIndu}{2}{Hinzu kommt der Verlauf der Leistung in Grün.
    Die Leistung pulsiert mit der doppelten Frequenz.
    Dabei ist die Fläche der positiven Halbwelle Gleich der Fläche der negativen Halbwelle.
    Es wird somit keine Wirkleistung übertragen.}
\end{frame}
...
\begin{frame}
    ...
    \b{
        ...
        \begin{itemize}
            \item<1-> Spannung und Strom um $-\pi/2$ phasenverschoben
            \item<2-> Konstanter Anteil der Augenblicksleistung gleich Null
            \item<2-> Augenblicksleistung pulsiert mit doppelter Frequenz
    ...
    \fu{
        \resizebox{0.8\textwidth}{!}{
            \input{Tikz/src/Scheinleistung_Wirkleistung_Blindleistung/Verlauf_kapazitive_Last}\alttext{Das Bild zeigt drei sinusförmige Zeitverläufe über der Zeit t. Die Spannung u(t) ist blau, der Strom i(t) rot und die Leistung p(t) grün dargestellt. Spannung und Strom sind gegensinnig phasenverschoben. Die Leistung schwankt um null mit abwechselnd positiven und negativen Anteilen.}
        }
    ...
    \speech{LeistungMittlKapa}{1}{Auch bei einer Kapazität liegt eine Phasenverschiebung zwischen dem Strom und der Spannung vor.
    Die jeweiligen Verläufe sind im Grafen Abgebildet.}
    \speech{LeistungMittlKapa}{2}{Auch hier ist bei der Leistung der Gleichanteil gleich Null.
    Die Leistung pulsiert mit der doppelten Frequenz.
    So wird die Aufgenommene Leistung wieder vollständig abgegeben.
    Die Leistung, welche bei der Übertragung anfällt und keine Wirkleistung darstellt wird auch Blindleistung genannt.}
\end{frame}
...
\begin{frame}
    ...
    \s{
        Ist die Augenblicksleistung\index{Augenblicksleistung} größer Null, wird Leistung aufgenommen. Entgegengesetzt wird Leistung abgegeben, wenn die
    ...
    \b{
        Die Blindleistung $Q$ beschreibt denjenigen Anteil der Leistung, welcher zur Erzeugung von elektrischen und magnetischen Feldern
        benötigt wird.
        \begin{itemize}
    ...
    \speech{LeistungBlind}{1}{Die Blindleistung beschreibt denjenigen Anteil der Leistung,
    welcher zur Erzeugung von elektrischen und magnetischen Feldern benötigt wird.
    Wie zuvor kennengelernt weisen die Leistungen von idealen Induktivitäten und Kapazitäten keinen Gleichanteil auf.
    Der Anteil der Augenblicksleistung ohne einen Gleichanteil beschreibt die Blindleistung Kuh.}
    \speech{LeistungBlind}{2}{Die Induktive Blindleistung}
    \speech{LeistungBlind}{3}{und die Kapazitive Blindleistung werden über die angegebenen Gleichungen bestimmt.
    Aufgrund des jeweiligen Phasenwinkels wirkt die Induktive Blindleistung positiv und die Kapazitive Blindleistung negativ
    in den Imaginärteil.}
    \speech{LeistungBlind}{4}{Die Einheit der Blindleistung Kuh ist das Volt-Ampere-Reaktiv. Kurz V Ah Er.}
\end{frame}
...
\begin{frame}
    ...
    \speech{LeistungMrk2}{1}{In der komplexen Ebene wird die Leistung durch die Wirkleistung und die Blindleistung erklärt.
    Die Wirkleistung beschreibt den Realteil und die Blindleistung den Imaginärteil der komplexen Scheinleistung
    .}
\end{frame}
...
\begin{frame}
    ...
    \speech{LeistungSchein}{1}{In der Realität gehen Wirkleistung und Blindleistung einher.
    Die Scheinleistung setzt sich aus den beiden Leistungsarten zusammen.
    An einem idealen Widerstand entspricht die Scheinleistung, durch den rein reellen Widerstand, der Wirkleistung am Widerstand, .}
    \speech{LeistungSchein}{2}{Die komplexe Scheinleistung wird aus der Komplexen Spannung und dem Konjugiert Komplexen Strom berechnet, .}
    \speech{LeistungSchein}{3}{Hier werden alle Drei Leistungen im Zeigerdiagramm dargestellt.
    Die Wirkleistung stellt den Realteil und die Blindleistung den Imaginärteil der Scheinleistung dar.}
\end{frame}
...
\begin{frame}
    ...
    \speech{LeistungArten}{1}{Die komplexe Scheinleistung wird durch die komplexe Spannungen und Ströme hervorgerufen.
    Sie überträgt eine Scheinbare Leistung,
    welche sich aus einer genutzten Wirkleistung Pe und der Blindleistung Q zusammensetzt.}
    \speech{LeistungArten}{2}{Über den Leistungsfaktor können die Wirkleistung,
    die Blindleistung und die Scheinleistung ineinander umgerechnet werden.}
\end{frame}
...
\begin{frame}
    ...
    \speech{LeistungMrk3}{1}{Die komplexe Scheinleistung beschreibt die Leistung, die scheinbar übertragen wird.
    Sie setzt sich aus der Wirkleistung und der Blindleistung zusammen.}
\end{frame}
...
\begin{frame}
    ...
    \fu{
        ...
        \input{Tikz/src/Scheinleistung_Wirkleistung_Blindleistung/Zeigerdiagramm_Leistung}\alttext{Das Bild zeigt ein komplexes Leistungsdiagramm mit Realachse Re(S) und Imaginärachse Im(S). Ein Zeiger stellt die Scheinleistung S dar, mit Wirkleistung P auf der Realachse und Blindleistung Q auf der Imaginärachse. Die vier Quadranten sind als induktiv, kapazitiv, Generatorbetrieb und Verbraucherbetrieb beschriftet. Die Grafik veranschaulicht die Beziehung zwischen Wirk-, Blind- und Scheinleistung.}
        }
    ...
    \speech{LeistungBetrieb}{1}{Je nachdem was für Komponenten oder Verbünde betrachtet werden,
    ist die Scheinleistung eher kapazitiv oder mehr induktiv, nimmt Strom auf oder gibt Strom ab.
    Aus der Scheinleistung kann die Betriebsart eines komplexen Zweipols identifiziert werden.
    Gezeigt wird hier das Koordinatensystem der komplexen Scheinleistung.}
    \speech{LeistungBetrieb}{2}{Wird lediglich positive Wirkleistung Übertragen,
    wird von einem Lastbetrieb gesprochen.
    Beispielsweise wird hier von etwas elektrische Wirkeistung aufgenommen.
    Die Scheinleistung entspricht hier der übertragenen Wirkleistung}
    \speech{LeistungBetrieb}{3}{Wird zusätzlich eine positive Blindleistung Übertragen,
    verursacht zum Beispiel von einer Spule,
    handelt es sich um eine Induktive Blindleistung.}
    \speech{LeistungBetrieb}{4}{Die so hervorgerufene Scheinleistung verortet sich im ersten Quadranten.
    Dieser Betriebsbereich wird als Induktiver Lastbetrieb bezeichnet.}
    \speech{LeistungBetrieb}{5}{Werden so auf diese Weise gegen den Uhrzeigersinn die Quatranten durchgegangen,
    ist im zweiten Quadranten der induktive Generatorbetrieb,
    im dritten Quadranten der kapazitive Generatorbetrieb
    und im vierten Quadranten der kapazitive Lastbetrieb zu verorten.}
\end{frame}
...
\begin{frame}
    ...
    \b{
        ...
        \begin{itemize}
            \item<2-> Erregerstrom des Generators $>$ Bedarf des magnetischen Feldes $\rightarrow$ Übererregt   \\
                Blindleistung wird an das Netz abgegeben $\rightarrow$ Spannungsanhebung    \\
                Anwendung: Spannungshaltung
            \item<3-> Erregerstrom des Generators $<$ Bedarf des magnetischen Feldes $\rightarrow$ Untererregt  \\
    ...
    \speech{LeistungGenBlind}{1}{Allgemein sind die Betriebsarten nun bekannt,
    folgend noch ein wenig zum Generatorbetrieb.}
    \speech{LeistungGenBlind}{2}{Als Übererregt wird der Generatorbetrieb bezeichnet,
    beispielsweise wenn der Erregerstrom eines Generators größer ist,
    als der Bedraf des magnetischen Feldes.
    Die Blindleistung wird dann an das Netz abgegeben,
    was zu einer Anhebung der Systemspannung führt.
    Dieser Sachverhalt findet unter anderem in der Spannungshaltung als Systemdiensleistung eine Anwendung.}
    \speech{LeistungGenBlind}{3}{Falls der Erregerstrom des Generator geringer ist,
    als der Bedarf des magnetischen Feldes,
    ist der Generator Untererregt.
    Das hat einen Bezug von Blindleistung aus dem Netz zur Folge,
    was zu einer Herabsetzung der Systemspannung führt.
    Bei einer unkontrollierten Herabsetzung der Systemspannung,
    kann ein eventueller Ausfall von elektrischen Maschinen hervorgerufen werden.}
\end{frame}
...
\begin{frame}
    ...
    \speech{LeistungZsm}{1}{Die Leistungsarten noch einmal Zusammengefasst.
    Es gibt die Wirkleistung P, die Blindleistung Kuh und die Scheinleistung Es.
    Die Wirkleistung beschreibt die wirklich genutzte Leistung in Watt.
    Die Blindleistung die beispielsweise für Ausgleichsvorgänge benötigte induktive oder kapazitive Leistung in Volt Ampere reaktiv.
    Und die Scheinleistung beschreibt die Scheinbar benötigte Leistung eines betrachteten Systems in Volt Ampere.}
\end{frame}
...
\begin{frame}
    ...
    \begin{enumerate}
        \only<1>{
            ...
            Gegeben sind:   \\
            \begin{eq}
                \hat{U} = 12\ V \qquad   \varphi_\mathrm{U} = 90\degree      \qquad        \hat{I} = 2A\ \nonumber   \qquad  \varphi_\mathrm{I} = 30\degree  \nonumber
            \end{eq}
        ...
        \only<2>{
            ...
            \begin{eqa}
                ...
                = \frac{12\ V}{\sqrt{2}} \cdot \frac{2\ A}{\sqrt{2}} \cdot \cos(60\degree) \nonumber \\
                P &= 6\ W    \nonumber
            \end{eqa}
            Die Blindleistung beträgt:
            \begin{eqa}
                Q &= U \cdot I \cdot \sin(\varphi) = \frac{\hat{U}}{\sqrt{2}} \cdot \frac{\hat{I}}{\sqrt{2}} \cdot \sin(\varphi)
                = \frac{12\ V}{\sqrt{2}} \cdot \frac{2\ A}{\sqrt{2}} \cdot \sin(60\degree) \nonumber \\
                Q &= 10,392\ var    \nonumber
            \end{eqa}
        }
        \only<3-4>{
            ...
            \begin{eqa}
                ...
                \frac{12\ V}{\sqrt{2}} \cdot e^{j(90\degree)} \cdot \frac{2\ A}{\sqrt{2}} \cdot e^{-j(30\degree)} =
                \frac{12\ V \cdot 2\ A}{2} \cdot e^{j(90\degree - 30\degree)} \nonumber
            \onslide<4>{
                \\ \underline{S} &= 12\ VA \cdot e^{j60\degree} = 6\ W + j10,392\ var    \nonumber
            }
            \end{eqa}
            }
    \end{enumerate}
    ...
    \speech{LeistungBsp}{1}{Abschließend soll die folgende Aufgabe bearbeitet werden.
    Gegeben ist der Scheitelwert der Spannung mit Zwölf Volt und einem Phasenwinkel von Neunzig Grad
    und der Scheitelwert des Stromes mit Zwei Ampere und einem Phasenwinkel von Dreißig Grad.
    Hieraus sollen unter Aufgabenteil Ah die Wirkeistung und die Blindleistung bestimmt werden.
    Außerdem soll unter B die Scheinleistung mit dem Konjugiert Komplexen Strom bestimmt werden.}
    \speech{LeistungBsp}{2}{Die Wirkleistung wird über die Vorgestellte Gleichung berechnet.
    Die Scheitelwerte werden über Wurzel Zwei in die Effektivwerte umgerechnet.
    Hieraus ergibt sich eine Wirkleistung von Sechs Watt.
    Ähnlich dazu die Berechnung der Blindleistung.
    Nur wird hier die Sinusfunktion anstatt der Kosinusfunktion genutzt.
    Die Blindleistung beträgt hier Zirka Zehn Komma Vier Volt Ampere Reaktiv.}
    \speech{LeistungBsp}{3}{Die komplexe Scheinleistung wird aus dem Produkt der komplexen Spannung
    und dem Konjugiert komplexen Strom berechnet.
    Der Imaginärteil des Stromes erfährt also einen Vorzeichenwechsel.
    Strom und Spannung können miteinander verrechnet werden und die Winkel werden subtrahiert.}
    \speech{LeistungBsp}{4}{Als Ergebnisse für die Scheinleistung erhält man Zwölf Volt Ampere,
    bei einem Phasenwinkel von Sechzig Grad.
    Wird das Ergebnis in die kartesische Schreibweise umgeformt,
    gelangt man zu den Ergebnissen für die Wirkleistung und die Blindleistung aus dem vorherigen Aufgabenteil Ah
    .}
\end{frame}
