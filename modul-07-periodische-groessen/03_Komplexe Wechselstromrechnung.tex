...
\newvideofile{Wechsel1}{Wechselstromrechnung 1}
...
\begin{frame}
    ...
    Hier soll nun folgend erläutert werden, was die komplexe Impedanz und die komplexe Attmitanz ist.
    Das Verständnis über das Verhalten von Bauelementen an einer Wechselspannung vermittelt werden
    und gezeigt werden, wie Wechselquellen ineinander umgewandelt werden können.}
\end{frame}
...
\begin{frame}
    ...
    \s{
        ...
        \fu{
            \input{Tikz/src/Komplexe_Wechselstromrechnung/Komplexe_Impedanz}\alttext{Das Bild zeigt ein komplexes Impedanzdiagramm mit Realachse Re(Z) und Imaginärachse Im(Z). Ein Zeiger stellt die Impedanz Z mit dem Betrag Ẑ und dem Phasenwinkel Phi_Z dar. Die Projektionen auf die Achsen sind als Widerstand R auf der Realachse und als Blindanteil j·X auf der Imaginärachse markiert. Zusätzlich ist die komplexe Darstellung Z = Ẑ · e^{j Phi_Z} angegeben.}
    ...
    \s{
        So besteht auch die komplexe Impedanz\index{Impedanz} \underline{Z} nach Gleichung \ref{GleichungImpedanz} aus einem Realteil und aus einem Imaginärteil. Der
...
\begin{frame}
    \ftx{Impedanz und Admittanz in der komplexen Ebene}

    \s{\index{Admittanz}
        Wie in der Gleichstromtechnik existiert ein Leitwert des elektrischen Widerstandes in der komplexen Wechselstromtechnik.
        Dieser Kehrwert der komplexen Impedanz wird als Admittanz \underline{Y} bezeichnet (vlg. Gleichung \ref{GleichungImpedanzAdmittanz}). Die Einheit des komplexen Leitwertes bleibt
    ...
    \begin{eq}
        R = \frac{1}{G} \quad \rightarrow \quad \underline{Z} = \frac{1}{\underline{Y}}   \label{GleichungImpedanzAdmittanz}
    \end{eq}

    \s{
        Auch die Admittanz lässt sich in einen Wirkleitwert\index{Wirkleitwert} und einen Blindleitwert differenzieren.
        Der Wirkleitwert G der Admittanz wird als Konduktanz\index{Konduktanz} und der Blindleitwert\index{Blindleitwert} B der Admittanz wird als Suszeptanz\index{Suszeptanz} bezeichnet (vgl. Gleichung \ref{GleichungAdmittanz} ).
    }
...
\begin{frame}
    ...
    \speech{WechselMrk1}{1}{Der Gleichstromwiderstand wird bei einer Wechselspannung zur komplexen Impedanz Z.
    Der Kehrwert des Widerstandes, der Leitwert, wird bei einer Wechselspannung zur komplexen Admittanz Üpsilon.
    Zur Erinnerung.
...
\begin{frame}
    ...
    \speech{WechselWiderErkl}{2}{Ist ein Verbraucher rein Ohmsch, wirkt kein Blindanteil.
    Ix R wird somit zu Null und das Produkt der Reaktanz entfällt.}
    \speech{WechselWiderErkl}{3}{Der ideale Widerstand erfolgt auf diese Weise als reiner Wirkwiderstand}
    \speech{WechselWiderErkl}{4}{Das ohmsche Gesetz findet in der Wechselstromtechnik genauso mit komplexen Größen Verwendung.
    Das gilt für die Darstellung als komplexe Zeiger ebenso wie für die Darstellung mit zeitlichen Größen. .}
\end{frame}
...
\begin{frame}
    ...
    \fu{
        \input{Tikz/src/Komplexe_Wechselstromrechnung/Gegenueberstellung_idealer_realer_Widerstand}\alttext{Das Bild zeigt links einen idealen Widerstand als einfaches Schaltzeichen. Ein Pfeil weist auf eine realistischere Ersatzschaltung rechts. Diese besteht aus einem realen Widerstand mit parasitärer Induktivität in Serie und einer parasitären Kapazität parallel. Die Darstellung verdeutlicht, dass reale Bauteile zusätzliche parasitäre Eigenschaften besitzen.}
...
\begin{frame}
    ...
    \b{
        ...
        \begin{itemize}
            \item<1-> Spannung $u_\mathrm{R}(\omega t)$
            \item<2-> Strom $i_\mathrm{R}(\omega t)$
            \item<3-> Spannung und Strom am idealen ohmschen Wiederstand ohne Phasenverschiebung
        \end{itemize}
    }

    \fu{
        \input{Tikz/src/Komplexe_Wechselstromrechnung/Sinunsschwingung_Widerstand}\alttext{Das Bild zeigt zwei sinusförmige Zeitverläufe über 𝜔𝑡. Die Spannung u_R(t) ist als blaue Linie und der Strom i_R(t) als rote gestrichelte Linie dargestellt. Beide Kurven verlaufen deckungsgleich ohne Phasenverschiebung. Darunter ist angegeben, dass die Phasendifferenz ΔPhi_R = Phi_U - Phi_I = 0 beträgt.}
...
\begin{frame}
    ...
    \s{
        ...
        Kreisfrequenz\index{Kreisfrequenz} $\omega$ ist frequenzabhängig. So ergeben sich bei hohen Frequenzen niedrige Impedanzwerte für den
    ...
    Hierbei steht der Ausdruck jott Omega C im Nenner.
    Zur Berechnung der kapazitiven Attmitanz wird der Kehrwert aus der kapazitiven Impedanz berechnet.
    Ein kapazitiver Blindwiderstand wird im negativen Imaghinärbereich dargestellt.}
    ...
    Der Strom Eilt der Spannung an einer Kapazität um Pi Halbe voraus.}
\end{frame}
...
\begin{frame}
    ...
    \s{
        Bei Induktivitäten und Kapazitäten kommt es zu einer Phasenverschiebung\index{Phasenverschiebung} zwischen der Wechselspannung und dem
    ...
    \b{
        ...
        \begin{itemize}
            \item Spannung $u_\mathrm{C}(\omega t)$
            \item<2-> Strom $i_\mathrm{C}(\omega t)$
    ...
    \fu{
        \input{Tikz/src/Komplexe_Wechselstromrechnung/Phasenverschiebung_Kondensator}\alttext{Das Bild zeigt zwei sinusförmige Zeitverläufe über 𝜔𝑡 für einen Kondensator. Die Spannung u_C(t) ist als blaue Kurve dargestellt, der Strom i_C(t) als rote gestrichelte Kurve. Der Strom eilt der Spannung um eine Viertelperiode voraus. Angegeben ist die Phasenverschiebung ΔPhi_C = Phi_U - Phi_I = −π/2.}
    ...
    Er startet nicht im Ursprung. Die positive Halbwelle startet um Pi halbe Phasenverschoben im Vergleich zur Wechselspannung. .}
    ...
    \speech{WechselKapaPhase}{4}{Diese Phasendifferenz zwischen den beiden Phasenlagen beträgt bei einer Kapazität Pi halbe oder im Gradmaß Neunzig Grad. .}
\end{frame}
...
\begin{frame}
    ...
    \speech{WechselMrk3Kapa}{1}{An der idealen Kapazität eilt der Wechselstrom der Wechselspannung um Neunzig Grad vor.}
\end{frame}
...
\begin{frame}
    ...
    \fu{
        \input{Tikz/src/Komplexe_Wechselstromrechnung/Gegenueberstellung_idealer_realer_Kondensator}\alttext{Das Bild zeigt links einen idealen Kondensator als einfaches Schaltzeichen. Ein Pfeil führt zu einer realen Ersatzschaltung auf der rechten Seite. Diese enthält eine parasitäre Induktivität in Serie, einen realen Kondensator sowie parasitäre Widerstände in Reihe und parallel. Die Grafik verdeutlicht die nichtidealen Eigenschaften eines realen Kondensators.}
    ...
    \speech{WechselKapaBauteil}{1}{Wie auch beim elektrischen Widerstand wird in der Regel von idealen Bauteilen ausgegangen.}
    \speech{WechselKapaBauteil}{2}{Die parasitären Effekte des realen Kondensators werden unter anderem durch
    einen Leitungswiderstand und eine Leitungsinduktivität verdeutlicht.
    Die Selbstentladung des Kondensators kann durch den Parallelwiderstand erklärt werden.}
\end{frame}
...
\begin{frame}
    ...
    \speech{WechselInduErkl}{1}{Eine Induktivität, beispielsweise einer Spule, lässt eine Impedanz Zett Ell wirken.
    Diese induktive Impedanz verfügt über einen Blindwiderstand, welcher mit einem positiven vorzeichen behaftet ist
    und sich im Zeigerdiagramm im positiven Imaghinärbereich verortet.
    Durch den Kehrwert der induktiven Impedanz Zett L wird die induktive Attmitanz Y L gebildet.}
    \speech{WechselInduErkl}{2}{Nach dem Ohmschen Gesetz verhält sich die induktive Impedanz wie die zuvor kennengelernten Bauteile.
    Eine komplexe Spannung U ergibt sich aus dem Produkt aus der komplexen Impedanz Zett L und dem komplexen Strom Ih.
    Ebenso verhält es sich im Zeitbereich mit einer idealen Spule.}
    \speech{WechselInduErkl}{3}{Wie auch bei einer Kapazität kommt es an einer Induktivität zu einer Phasenverschiebung.
    Jedoch verhält es sich an der Induktivität umgekehrt.
    Während an einer Kapazität ein Strom der Spannung voreilt,
    Eilt an einer Induktivität eine Spannung dem Strom vor. .}
\end{frame}
...
\begin{frame}
    ...
    \b{
        ...
        \begin{itemize}
            \item Spannung $u_\mathrm{L}(\omega t)$
            \item<2-> Strom $i_\mathrm{L}(\omega t)$
            \item<3-> Phasenverschiebung zwischen Strom und Spannung
            \item<4-> Phasenverschiebung $\Delta\varphi$
        \end{itemize}
    }
    ...
    \fu{
        \input{Tikz/src/Komplexe_Wechselstromrechnung/Phasenverschiebung_Spule}\alttext{Das Bild zeigt zwei sinusförmige Zeitverläufe über 𝜔𝑡 für eine Spule. Die Spannung u_L(t) ist als blaue Kurve und der Strom i_L(t) als rote gestrichelte Kurve dargestellt. Die Spannung eilt dem Strom um eine Viertelperiode voraus. Angegeben ist die Phasenverschiebung ΔPhi_L = Phi_U - Phi_I = +π/2.}
    ...
    \speech{WechselInduPhase}{1}{Die sich hieraus ergebene Phasenverschiebung an einer Spule wird nun näher betrachtet.
    Es wird bläulich der Verlauf einer Wechselspannung Uh Ell an einer Spule dargestellt}
    \speech{WechselInduPhase}{2}{Der zugehörige Verlauf des Wechselstromes wird in Rot hinterlegt.}
    \speech{WechselInduPhase}{3}{Die beiden Verläufe befinden sich nicht in Phase.}
    \speech{WechselInduPhase}{4}{Die Phasenverschiebung Delta Vieh ergibt sich aus der Differenz zwischen der Phasenlage der Spannung und des Stromes und beträgt bei der Spule Pi halbe.}
\end{frame}
...
\begin{frame}
    ...
    \speech{WechselInduMrk}{1}{Bei einer idealen Induktivität folgt der Wechselstrom der WEchselspannung um 90 Grad oder im Radmaß um Pi Halbe nach.}
\end{frame}
...
\begin{frame}
    ...
    \fu{
        \input{Tikz/src/Komplexe_Wechselstromrechnung/Gegenueberstellung_ideale_reale_Spule}\alttext{Das Bild zeigt links eine ideale Spule als einfaches Schaltzeichen. Ein Pfeil führt zu einer realen Ersatzschaltung auf der rechten Seite. Diese besteht aus einer realen Spule mit parasitärem Serienwiderstand sowie einer parasitären Kapazität parallel. Die Darstellung macht deutlich, dass reale Spulen nicht ideal sind und zusätzliche Verluste besitzen.}
    ...
    \speech{WechselInduBauteil}{1}{Wird von einer idealen Spule ausgegangen, wird in der Regel das einzelne Bauteil betrachtet.}
    \speech{WechselInduBauteil}{2}{In der realen Welt verhält es sich anders.
    Hier werden Beispielsweise die Ohmschen Verluste über die Zu und Ableitungen berüchsichtigt,
    sowie mögliche parasitäre Kapazitäten.
    Je nach Komplexität lassen sich die Ersatzschaltbilder von Bauteilen aufwendiger gestalten.}
\end{frame}



\newvideofile{Wechsel2}{Wechselstromrechnung 2}

\v{
\begin{frame}
	\ftx{Wechselstromrechnung 2}

    \begin{Lernziele}{Wechselstromrechnung 2}
        Die Studierenden
        \begin{enumerate}
            \item kennen Wechselquellen und die dazugehörigen Umwandlungsvorschriften.
            \item können Spannungsteiler und Stromteiler auf komplexe Signale anwenden.
        \end{enumerate}
    \end{Lernziele}

    \speech{WechselLrnz2}{1}{Folgend werden nach den Impedanzen und Attmitanzen von Widerständen,
    Spulen und Kondensatoren, Wechselquellen sowie Spannungs und Stromteiler erläutert.}
\end{frame}}
...
\begin{frame}
    ...
    \fu{
        \input{Tikz/src/Komplexe_Wechselstromrechnung/Quellen_Wechselgroessen}\alttext{Das Bild zeigt zwei einfache Wechselspannungsquellen in einem offenen Stromkreis. Links ist die Quelle mit einem Pfeil für die Spannung U dargestellt. Rechts ist dieselbe Quelle gezeigt, jedoch mit einem Pfeil für den Strom I. Die Grafik verdeutlicht die unterschiedliche Bezugsrichtung für Spannung und Strom.}
    }{{\bf Quellen von Wechselgrößen.} Ideale Wechselspannungsquelle und ideale Wechselstromquelle. \label{BildIdealeQuellen}}

    \speech{WechselQuellenIdRe}{1}{Zuvor wurden für Wechselgrößen immer zu den Idealen Komponenten auch die Realen Komponenten vorgestellt.
    Hier werden nun zu den Realen Wechselspannungsquellen}
    \speech{WechselQuellenIdRe}{2}{und Wechselstromquellen auch die Realen Quellen und ihre Charakteristika beschrieben.}
\end{frame}
...
\begin{frame}
    ...
    \fu{
        \input{Tikz/src/Komplexe_Wechselstromrechnung/Gegenueberstellung_ideale_reale_Wechselspannungsquelle}\alttext{Das Bild zeigt links eine Wechselspannungsquelle mit markierter Spannung U an den Klemmen. Ein Pfeil weist auf eine erweiterte Schaltung rechts. Dort ist an die Quelle eine Innenimpedanz Z_i angeschlossen, durch die ein Strom I fließt. Zusätzlich sind die Quellenspannung U₀ und die Klemmenspannung U eingezeichnet.}
    ...
    \speech{WechselQuellenU}{1}{Hier wird eine ideale Wechselspannungsquelle dargestellt.}
    \speech{WechselQuellenU}{2}{Die reale Wechselspannung verfügt wie auch bei der Betrachtung von Gleichstromnetzwerken einen Innenwiderstand,
    die Innenimpedanz Zett Ih. .}
    \speech{WechselQuellenU}{3}{Die Ausgangsspannung U ist somit in der Regel geringfügig durch den Spannungsabfall an der Innenimpedanz reduziert.}
\end{frame}
...
\begin{frame}
    ...
    \speech{WechselQuellenUCha}{1}{Zunächst wird der Leerlauf von realen Wechselspannungsquellen betrachtet.
    Ohne einen angeschlossenen Verbraucher gibt es keinen geschlossenen Stromkreis, der Strom beträgt damit Null.
    Weiter heißt das, dass über der Innenimpedanz keine Spannung anliegt.}
    \speech{WechselQuellenUCha}{2}{Nach der Maschenregel teilt sich die Quellspannung auf die Teilspannungen der Innenimpedanz und den Ausgang der Quelle auf.
    Da ohne einen Stromfluss an der Innenimpedanz keine Spannung abfällt,
    Liegt die gesamte Wechselspannung am Ausgang der Wechselspannungquelle an.}
    \speech{WechselQuellenUCha}{3}{Wird der Ausgang kurzgeschlossen, so fällt über ihn keine Spannung ab.
    Der Wechselstrom des geschlossenen Stromkreises ist gleich dem anzunehmenden Kurzschlussstrom.}
    \speech{WechselQuellenUCha}{4}{Nach der Maschenregel fällt die gesamte Quellspannung über der Innenimpedanz ab.
    Der Kurzschlussstrom lässt sich so aus dem Verhältnis aus der Spannung über der Innenimpedanz und der Innenimpedanz bestimmen.}
\end{frame}
...
\begin{frame}
            ...
            \only<1>{\item[]
                  ...
                  \fu{
                      \resizebox{0.5\textwidth}{!}{
                          \input{Tikz/src/Komplexe_Wechselstromrechnung/Beispiel_reale_Spannungsquelle}\alttext{Das Bild zeigt eine Wechselspannungsquelle U₀, die eine Reihenschaltung aus Spule L und Widerstand R speist. Durch die Schaltung fließt ein Strom I, der am rechten Anschluss markiert ist. Die Ausgangsspannung U ist an den Klemmen rechts eingezeichnet.}
                      }
    ...
    \speech{WechselBsp1}{1}{Nun wird eine reale Wechselspannungsquelle betrachtet.
    Es sollen die Leerlaufspannung und der Kurzschlussstrom bestimmt werden.}
    \speech{WechselBsp1}{2}{Die Wechselspannungsquelle verfügt über eine Spannungshöhe von Zweihundertdreißig Volt.
    Die komplexe Innenimpedanz setzt sich aus den dem Wirkanteil des Widerstandes und den Blindanteil zusammen,
    welcher widerum von der Spule hervorgerufen wird.
    Die Ausgangsspannung einer Wechselspannungquelle im Leerlauffall ist gleich der Gesamten Wechselspannung der Wechselspannungsquelle.
    Der Kurzschlussstrom wird aus den beiden Größen der Wechselspannung und der Innenimpedanz berechnet.
    Das Ergebnis wird hier in kartesischen Koordinaten und Polarkoordinaten angegeben.}
\end{frame}
...
\begin{frame}
    ...
    }{{\bf Quellen von Wechselgrößen.} Reale Wechselstromquelle und ideale Wechselstromquelle. \label{BildIdealStrom}}
    ...
    \speech{WechselQuellenI}{1}{ Hier folgt die ideale Wechselstromquelle.}
    \speech{WechselQuellenI}{2}{Die reale Wechselstromquelle verfügt wie auch die Wechselspannungsquelle über eine Innenimpedanz.
    Nur das diese Innenimpedanz Zett I nicht in Reihe verschaltet ist, sondern Parallel zur Wechselstromquelle.}
    \speech{WechselQuellenI}{3}{Der Ausgangswechselstrom der reale Wechselstromquelle lässt sich durch die Knotenregel bestimmen,
    da vom Strom der Wechselstromquelle noch der Teilstrom über die Innenimpedanz abgezogen wird.
    Der Teilstrom durch die Innenimpedanz lässt sich über die Größe der Ausgangsspannung und der Innenimpedanz berechnen.}
\end{frame}
...
\begin{frame}
    ...
    \fu{
        \input{Tikz/src/Komplexe_Wechselstromrechnung/Gegenueberstellung_reale_Wechselspannung_Wechselstromquelle}\alttext{Das Bild zeigt zwei äquivalente Ersatzschaltungen einer realen Wechselspannungsquelle und einer realen Wechselstromquelle. Links ist eine Quelle mit Serien-Innenimpedanz Zᵢ dargestellt, mit Quellspannung U₀, Innen-spannung Uᵢ und Ausgangsspannung U sowie Strom I. Rechts ist die Quelle als Parallelschaltung mit Innenimpedanz Zᵢ gezeigt, bei der sich der Quellstrom I₀ in Innenstrom Iᵢ und Außenstrom I aufteilt. Der Doppelpfeil kennzeichnet die elektrische Äquivalenz beider Darstellungen.}
    ...
    \speech{WechselQuellenAqui}{1}{Die Charakteristika der realen Quellen sind jetzt grundsätzlich bekannt.
    Wechselspannungsquellen und Wechselstromquellen können auch ineinander umgewandelt werden.}
    \speech{WechselQuellenAqui}{2}{Verfügen die ineinander umzurechnenden Wechselquellen über eine identische Innenimpedanz Zett Null,
    ist eine Umwandlung in die jeweils andere Quellenart möglich.}
    \speech{WechselQuellenAqui}{3}{Hierbei gibt es zwei Betrachtungsweisen.
    Wird eine Wechselspannungsquelle in eine Wechselstromquelle überführt,
    so ist der Quellstrom der neuen Wechselstromquelle gleich dem Kurzschlusstrom.
    Wird eine Wechselstromquelle in eine Wechselspannungsquelle umgewandelt,
    gibt die Leerlaufspannung der Wechselstromquelle die Quellspannung der neuen Wechselspannungsquelle an.}
\end{frame}
...
\begin{frame}
    ...
    \speech{WechselBsp2}{1}{Folgend soll an einem Beispiel eine reale Wechselspannungsquelle in eine äquivalente Wechselstromquelle umgewandelt werden.
    Hier wird bei der Umwandlung der Kurzschlussstrom der Wechselspannungsquelle benötigt.}
    \speech{WechselBsp2}{2}{Der Kurzschlusstrom I Null wird aus dem Verhältnis aus der Spannung der Wechselspannungsquelle U Null und der Innenimpedanz Zett Null gebildet.}
\end{frame}




%\newvideofile{Wechsel3}{Komplexer Teiler}
...
\begin{frame}
    ...
    \fu{
        \resizebox{0.7\textwidth}{!}{
            \input{Tikz/src/Komplexe_Wechselstromrechnung/Knoten_und_Maschenregel}\alttext{Das Bild besteht aus zwei Teilen. Links ist ein Knoten dargestellt, in den mehrere Ströme hinein- und herausfließen, beschriftet mit I₁ bis I₅. Rechts ist eine elektrische Schaltung mit einer Wechselspannungsquelle und zwei parallelen Impedanzen Z₁ und Z₂ gezeigt. Die Ströme durch die Zweige sowie die Spannungen über den Impedanzen sind markiert, und ein Pfeil kennzeichnet eine Masche zur Anwendung der Kirchhoffschen Regeln.}
        }
    ...
    \speech{WechselKnotMasch}{1}{Die Grundsetzlichen Regel zur Maschenanalyse und Knotenanalyse gelten wie auch bei der Betrachtung von Gleichstromnetzwerken.
    Hierunter fallen im Besonderen die Knotenregel und die Maschenregel.}
    \speech{WechselKnotMasch}{2}{Die Knotenregel wird hier mit einem Beispielknoten und Dazugehörigen Beschreibung dargestellt.
    Die Summe der Komplexen Ströme ist gleich Null. .}
    \speech{WechselKnotMasch}{3}{Ebenso verhält es sich mit der Maschenregel und komplexen Größen.
    Die Summe der komplexen Teilspannung in einem MAschenumlauf ist gleich Null. .}
\end{frame}
...
\begin{frame}
    ...
    \s{
        ...
        \fu{
            \resizebox{0.3\textwidth}{!}{
                \input{Tikz/src/Komplexe_Wechselstromrechnung/Komplexer_Spannungsteiler}\alttext{Das Bild zeigt eine elektrische Schaltung mit einer angelegten Spannung U₀ über zwei in Reihe geschalteten Impedanzen Z₁ und Z₂. Durch die Reihenschaltung fließt ein Strom I von oben nach unten. Die Teilspannungen U₁ über Z₁ und U₂ über Z₂ sind jeweils eingezeichnet. Die Grafik veranschaulicht einen Impedanz-Spannungsteiler.}
            }
    ...
    \speech{WechselSpannungsteiler}{1}{Gemäß der Maschenregel wird hier eine Eingangsspannung U Null auf zwei Teilspannungen aufgeteilt.
    Eine Reihenschaltung mit zwei Impedanzen ist wohl die einfachste Art eines Spannungsteilers.
    Er dient der Reduzierung einer Gesamtspannung.}
    \speech{WechselSpannungsteiler}{2}{Die Gewichtung der Spannung hängt vom Verhältnis der beiden Impedanzen ab.
    Der Gesamtstrom lässt sich aus dem Verhältnis der Eingangsspannung und der Summe der Impedanzen berechnen.
    Gemäß der Knotenregel muss in der Reihenschaltung der komplexe Strom überall gleich sein.
    So lässt sich der komplexe Strom auch über die Verhältnisse der Teilspannungen und den zugehörigen Impedanzen bestimmen.}
    \speech{WechselSpannungsteiler}{3}{Um das komplexe Teilerverhältnis Tee zu bestimmen,
    wird das Verhältnis einer Teilspannung zur Gesamtspannung gebildet.
    Außerdem lässt sich das Teilerverhältnis über die Impedanzen bestimmen.
    Beispielsweise wie hier das Verhältnis aus Zett zwei zur Summe der Impedanzen angezeigt.}
\end{frame}
...
\begin{frame}
    ...
    \s{
        ...
        \fu{
            \resizebox{0.5\textwidth}{!}{
                \input{Tikz/src/Komplexe_Wechselstromrechnung/Gegenueberstellung_Teiler_Hochspannung}\alttext{Das Bild zeigt vier verschiedene Spannungsteiler-Schaltungen nebeneinander. Dargestellt sind ein ohmscher Teiler, ein ohmsch-kapazitiver Teiler, ein kapazitiver Teiler und ein gedämpft kapazitiver Teiler. Jede Schaltung besteht aus mehreren in Reihe angeordneten Bauelementen mit markierter Eingangsspannung U₀. Die Grafik vergleicht unterschiedliche Teilerarten und ihre Bauteilkombinationen.}
            }
    ...
    \speech{WechselSpannungsteilerHochsp}{1}{Ein Anwendungsfall eines komplexen Spannungsteiler findet man in der Hochspannungstechnik.
    Hier wird zwischen verschiedenen Arten von Spannungsteilern unterschieden.
    Zu sehen ist ein Ohmscher Spannungsteiler.
    Wie gerade erklärt, setzt sich der Ohmsche Spannungsteiler durch eine Reihe von Impedanzen zusammen
    und ist vor allem für den Einsatz bei großen Gleichspannungen geeignet.}
    \speech{WechselSpannungsteilerHochsp}{2}{Der Ohmsch Kapazitive Spannungsteiler besteht aus einer Reihe von Er Zeh Gliedern.
    Diese Art von Spannungsteiler kann sowohl für Gleichspannungen als auch für Wechselspannungen verwendet werden.}
    \speech{WechselSpannungsteilerHochsp}{3}{Der Kapazitive Teiler dient der Spannungsteilung bei hohen Wechselspannungen.}
    \speech{WechselSpannungsteilerHochsp}{4}{Als letztes wird noch der gedämpft-kapazitive Spannungsteiler dargestellt.}
\end{frame}
...
\begin{frame}
    ...
    \s{
        ...
        \fu{
            \resizebox{0.3\textwidth}{!}{
                \input{Tikz/src/Komplexe_Wechselstromrechnung/Spannungsteiler_ohmsch-kapazitiv}\alttext{Das Bild zeigt einen ohmsch-kapazitiven Spannungsteiler. Zwei Widerstände R₁ und R₂ sind jeweils parallel zu den Kondensatoren C₁ und C₂ geschaltet und bilden eine Reihenschaltung. Die Eingangsspannung U₀ liegt über der gesamten Schaltung an. Die Ausgangsspannung U₂ wird am unteren Teil des Teilers abgegriffen.}
            }
    ...
    \speech{WechselSpannungsteilerAllg}{1}{Die Impedanzen des zuvor beschrieben komplexen Spannungsteilers können aus Er Ce Gliedern bestehen.
    Hier liegt dann ein ohmsch kapazitives Teilungsverhältnis vor.}
    \speech{WechselSpannungsteilerAllg}{2}{Bei der Betrachtung von komplexen Teilern sind die Impedanzen Frequenzabhängig.}
    \speech{WechselSpannungsteilerAllg}{3}{Somit ist auch das Teilerverhätlnis grundsätzlich Frequenzabhängig.}
    \speech{WechselSpannungsteilerAllg}{4}{In dem dargestellten Ausnahmefall wird das Teilerverhätlnis frei von der Frequenzabhängigkeit.
    Für diesen Fall ist der komplexe Spannungsteiler abgeglichen.}
\end{frame}
...
\begin{frame}
    ...
    \s{
        ...
        \fu{
            \resizebox{0.3\textwidth}{!}{
               \input{Tikz/src/Komplexe_Wechselstromrechnung/Spannungsteiler_ohmsch-kapazitiv2}\alttext{Das Bild zeigt einen ohmsch-kapazitiven Spannungsteiler mit angeschlossenem Tastkopf und Oszilloskop. Der obere Teil mit R₁ und C₁ ist als Tastkopf markiert, der untere Teil mit R₂ und C₂ als Oszilloskop-Eingang. Die Eingangsspannung U₀ liegt über der gesamten Schaltung an. Die gemessene Ausgangsspannung U_A wird am Oszilloskop abgegriffen.}
            }
    ...
    \speech{WechselSpannungsteilerTastk}{1}{Neben der Hochspannungstechnik,
    kommen komplexe Spannungsteiler auch beispielsweise in Tastköpfen für Oszilloskope zum Einsatz.
    Das vereinfachte Ersatzschaltbild des Aufbaus wird hier mit der Unterteilung eines Spannungsteilers in Tastkopf und Oszilloskop dargestellt.
    Die Ausgangsspannung liegt im Oszilloskop an, so wird eine zu große Spannung für die Anwednung reduziert.
    Die verbauten Widerstände ergeben ein Ohmsches Teilerverhätlnis.}
    \speech{WechselSpannungsteilerTastk}{2}{Die Kondensatoren bilden ein kapazitives Teilerverhältnis. .}
    \speech{WechselSpannungsteilerTastk}{3}{Durch die Gleichsetzung der beiden Teilerverhältnisse}
    \speech{WechselSpannungsteilerTastk}{4}{Gelangt man zu der angegebenen Abgleichbedingung.}
\end{frame}
...
\begin{frame}
    ...
    \s{
        ...
        \fu{
            \resizebox{0.3\textwidth}{!}{
                \input{Tikz/src/Komplexe_Wechselstromrechnung/Stromteiler_komplex}\alttext{Das Bild zeigt eine Parallelschaltung aus zwei Admittanzen Y₁ und Y₂. Der Gesamtstrom I₀ teilt sich am oberen Knoten in die Teilströme I₁ und I₂ auf. Beide Zweige liegen an derselben Spannung U. Die Grafik veranschaulicht die Stromaufteilung in einer Parallelschaltung.}
            }
    ...
    \b{
        ...
        \begin{minipage}[t]{0.5\textwidth}
            \begin{itemize}
                \item<1-> Der Gesamtstrom teilt sich im Verhältnis der komplexen Admittanzen auf:
                      \begin{eq}
                          \underline{U} = \frac{\underline{I}_\mathrm{0}}{\underline{Y}_\mathrm{1}+\underline{Y}_\mathrm{2}} =
                          \frac{\underline{I}_\mathrm{1}}{\underline{Y}_\mathrm{1}} =
    ...
    \speech{WechselStromteiler}{1}{Neben dem komplexen Spannungsteiler gibt es noch den komplexen Stromteiler.
    Er funktioniert ähnlich wie bei der betrachtung von Gleichstromnetzwerken, aber eben mit komplexen Größen.
    Ausschlaggebend sind die Admittanzen.
    So teilt sich der Gesamtstrom im Verhältnis zu der Summe der gesamten Admittanz
    in das Verhältnis eines Zweigstromes und der Zweigadmittanz. }
    \speech{WechselStromteiler}{2}{Wie auch beim Spannungsteiler sind in der Wechselstromtechnik die Impedanzen Frequenzabhängig.
    Die komplexen Teilerverhätnisse Tee I Eins und Tee I Zwei für die Zweigströme I Eins und I Zwei werden folgend dartgestellt.}
\end{frame}
