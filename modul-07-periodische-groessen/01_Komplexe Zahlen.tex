...
\begin{frame}
    ...
    \s{
        Eine komplexe Zahl\index{komplexe Zahl} \underline{Z} beschreibt einen Ort in der komplexen Ebene. Um in einem zweidimensionalen
    ...
    \b{
        Kartesische Koordinaten mit dem Realteil und dem Podukt aus imaginärer Einheit\index{imaginäre Einheit} und Imaginärteil:
    }
    ...
    \onslide<2->{
    \b{
        Der Realteil\index{Realteil} wird mit $\Re$ und der Imaginärteil\index{Imaginärteil} mit $\Im$ abgekürzt:
    }
...
\begin{frame}
    ...
    \fu{
        \input{Tikz/src/Komplexe_Zahlen/Zeigerdiagramm_Komplexe_Zahlen}\alttext{Das Bild zeigt ein Koordinatensystem der komplexen Zahlen mit horizontaler Realachse (Re) und vertikaler Imaginärachse (Im). Vom Ursprung aus verläuft ein Pfeil schräg nach oben rechts in dem ersten Quadranten. Die horizontale Projektion ist als Re(Z) blau markiert, die vertikale Projektion als Im(Z) rot. Der Winkel Phi_Z zwischen dem Pfeil und der positiven Realachse ist eingezeichnet.}
...
\begin{frame}
    ...
    \fu{
        \input{Tikz/src/Komplexe_Zahlen/Addition_Komplexer_Zahlen}\alttext{Das Bild zeigt ein komplexes Koordinatensystem mit Realachse (Re) und Imaginärachse (Im). Zwei komplexe Zahlen Z₁ (rot) und Z₂ (blau) sind als Pfeile vom Ursprung eingezeichnet. Ein weiterer Pfeil stellt die Summe Z₁ + Z₂ dar und verläuft diagonal nach oben rechts. Gestrichelte Hilfslinien zeigen, dass sich die Real- und Imaginärteile addieren zu Re(Z₁ + Z₂) und Im(Z₁ + Z₂).}
...
\begin{frame}
    ...
    \fu{
        \input{Tikz/src/Komplexe_Zahlen/Subtraktion_Komplexer_Zahlen}\alttext{Das Bild zeigt ein komplexes Koordinatensystem mit Realachse (Re) und Imaginärachse (Im). Die komplexen Zahlen Z₁ (rot) und Z₂ (blau) sind als Pfeile dargestellt. Ein weiterer Pfeil zeigt die Differenz Z₂ - Z₁ und verläuft schräg nach links oben. Die gestrichelte Linien markieren die Verschiebung von Real- und Imaginärteile von Z_1.}
...
\begin{frame}
    ...
    \fu{
        \input{Tikz/src/Komplexe_Zahlen/Multiplikation_Komplexer_Zahlen}\alttext{Das Bild zeigt ein komplexes Koordinatensystem mit Realachse (Re) und Imaginärachse (Im). Zwei komplexe Zahlen Z₁ (rot) und Z₂ (blau) sind als Pfeile vom Ursprung aus eingezeichnet, jeweils mit ihren Winkeln Phi_1 und Phi_2 zur Realachse. Ein weiterer Pfeil stellt das Produkt Z₂ · Z₁ dar und ist grün markiert. Der Winkel dieses Produktes entspricht der Summe der Winkel Phi_1 + Phi_2.}
