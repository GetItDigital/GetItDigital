...
\v{
	\begin{frame}{Lernziele}
		\begin{Lernziele}{Synchronmaschine}
			...
			\end{itemize}
		\end{Lernziele}
		\speech{LernzieleSynchronmaschine}{1}{In diesem Kapietel betrachten wir eine weitere, für viele Zwecke eingesetzte, elektrische Maschine: Die Synchronmaschine. Dabei gehen wir auf den Aufbau und die Vor- und Nachteile dieser elektrischen Maschine ein. Wir werfen einen Blick auf das Statorfeld, das aus drei Phasen besteht und letztlich den Rotor in Bewegung versetzt. Außerdem thematisieren wir dabei den Zusammenhang aus dem Polradwinkel und dem Drehmoment einer Synchronmaschine.}
		\silence{2}
	\end{frame}
}
...
\b{
	\begin{frame}
		...
		\speech{EinfuehrungSynchronmaschine}{1}{Die Synchronmaschine ist eine Drehfeldmaschine, die folglich mit einem dreiphasigen Drehstrom im Stator-kreis betrieben wird. Der dreiphasige Drehstrom wurde bereits im Modul 7 behandelt. Der Rotor kann entweder permanent, also mit Permanentmagneten, oder durch einen Gleichstromkreis erregt werden. Dieser Maschinentyp wird in Kraftwerken zur Stromgewinnung eingesetzt. Da er über einen sehr hohen Wirkungsgrad verfügt, wird er aber auch in der modernen Antriebstechnik in kleineren Leistungsklassen immer häufiger genutzt. Ein wesentlicher Nachteil besteht jedoch darin, dass die Synchronmaschine nur mithilfe einer Leistungselektronik betrieben werden kann, da sie an einem einfachen Drehstromnetz nicht anläuft.}
		\silence{2}
	\end{frame}
}
...
\begin{frame}
	...
	\end{itemize}
	\speech{SynchronmaschineVorNachteile}{1}{Die Synchronmaschine dreht netzsynchron mit konstanter Drehzahl. Daher ihr Name. Sehen wir uns nun zunächst die Vor- und Nachteile einer Synchronmaschine an.}
	\silence{2}
	\speech{SynchronmaschineVorNachteile}{2}{Die Vorteile liegen insbesondere in der Robusten Bauweise, weswegen sie sehr wartungsarm ist. Sie hat eine hohe Regeldynamik, was bedeutet, dass sie schnell und genau auf Last- oder Sollwertänderungen reagiert und Frequenz sowie Drehmoment stabil hält. Sie hat bei einer kleinen Baugröße eine hohe Leistungsdichte und ist zudem Überlastfähig, das heißt, dass sie kurzfristig über ihren Nennstrom oder ihrer Nennleistung hinaus betrieben werden kann, um etwa Spitzenlasten zu decken, und das ohne Schaden zu nehmen.}
	\silence{2}
	\speech{SynchronmaschineVorNachteile}{3}{Die Nachteile de Synchronmaschine liegen vor allem in der dafür notwendigen, aufwändeigen Regeltechnik. Der Betrieb erfordert komplexe und teure Leistungselektronik, also Umrichter, um Frequenz und Spannung passend bereitzustellen. Zudem ist die Regelung der Maschine anspruchsvoll, weil Drehzahl, Erregung und Last synchron abgestimmt und dynamisch geregelt werden müssen. Außerdem ist ein gezielt steuerbarer Betrieb, zum Beispiel schnelle Drehzahl- oder Drehmoment-änderungen nur mit hohem technischem Aufwand oder gar nicht praktikabel, sodass flexible, einfache Regelstrategien schwer umzusetzen sind. – Ganz im Gegensatz zur Asynchronmaschine, die wir im späteren Kapitel betrachten werden.}
	\silence{4}
\end{frame}
...
\begin{frame}\ftb{Feldverläufe}
	...
	\speech{SynchronmaschineFeldverlaufe1}{1}{Sehen wir uns das einmal Schritt für Schritt genauer an. Im Folgenden wird die Drehung des Rotors einer Synchronmaschine um Dreihundertsechzig Grad in vier Schritten gezeigt. Die zeitlichen Verläufe der Ströme Ih-1, Ih-2 und Ih-3, die die Stromflüsse durch die drei Stator-wicklungen darstellen, bewegen sich dabei sinus-förmig. Im ersten Schritt, bei einer Rotor-drehung von Null Grad, ist der Strom Ih-1 an seinem positiven Maximum, während die Ströme Ih-2 und Ih-3 negativ bei mittlerer Stärke ausfallen. Im Querschnitt des Rotos sind die Stromflüsse jeweils durch Kreuze, für in die Bildfläche hinein bewegende Ströme, und durch Punkte, für aus der Bildfläche hinaus bewegende Ströme dargestellt. Außerdem ist hier auch die Flussrichtung der jeweiligen Ströme an den Pfeilen abzulesen. Das von den drei Stator-wicklungen erzeugte Magnetfeld versetzt den Rotor in Bewegung. Es wird hier mit orangefarbenen Linien angedeutet.}
	\silence{2}
\end{frame}
...
\b{ % Quasi-Animation für Vorlesungsdatei
	\begin{frame}\ftb{Feldverläufe}
		...
		\speech{SynchronmaschineFeldverlaufe2}{1}{Im nächsten Schritt sieht es nun anders aus: Wegen der sinusförmigen, um 120 Grad zueinander versetzten Stromverläufe, haben sich die Stromstärken und Flussrichtungen verändert. Der Strom Ih-2 hat sein positives Maximum erreicht, während sich Ih-1 und Ih-3 bei mittlerer Stärke im negativen Bereich befinden. Diese Stromflüsse können wir auch im Querschnitt ablesen, genauso wie das daraus resultierende Magnetfeld, das den Rotor um 120 Grad im Uhrzeigersinn weiter gedreht hat.}
	\end{frame}
	\begin{frame}\ftb{Feldverläufe}
		...
		\speech{SynchronmaschineFeldverlaufe3}{1}{Nach weiteren Hundertzwanzig Grad, bei einer Rotor-drehung von Zweihundertvierzig Grad, hat nun der Strom Ih-3 sein positives Maximum erreicht, während die anderen beiden Ströme bei einer mittleren Stärke im negativen Bereich liegen.}
	\end{frame}
	\begin{frame}\ftb{Feldverläufe}
		\foo{}{height=0.8\textheight}{Synchronmaschine_Feldverlaeufe_a1}{
		...
		\speech{SynchronmaschineFeldverlaufe4}{1}{Bewegt sich die sinusförmige Modulation der Ströme in den drei Erregerspulen um weitere 120 Grad, ist die Ausgangsposition wieder erreicht und der Rotor hat eine Dreihundertsechzig-Grad-Drehung im Uhrzeigersinn vollzogen. Diese Drehung wurde zur Veranschaulichung in vier Schritten dargestellt, weil sich damit die Stromänderungen in den drei Statorwicklungen gut zeigen lassen. Tatsächlich müsste zwischen diesen Schritten jeweils eine quasi unendliche Zahl an Zwischenschritten gedacht werden, um die sinusförmigen Verläufe abzubilden.}
		\silence{2}
	\end{frame}
}
...
\begin{frame}\ftx{Drehmoment}
	...
	\speech{SynchronmaschineDrehmoment}{1}{Bei der Synchronmaschine bewirkt die magnetische Kopplung zwischen Stator und Rotor das Drehmoment. Im Motorbetrieb folgt das Rotorfeld dem Statorfeld. Die mechanische Drehzahl muss daher immer gleich der elektrischen Drehfrequenz sein. Zwischen dem Stator und dem Rotor befindet sich der lastabhängige Pohlrad-winkel Theta. Bei Theta gleich 90 Grad ist das Drehmoment am größten. Wird im Motorbetrieb der Pohlrad-winkel durch eine zu hohe Belastung größer als 90 Grad, sinkt das erzeugte Drehmoment und die Maschine kippt. Das heißt, sie bleibt schlagartig stehen.}
	\silence{2}
\end{frame}
...
\begin{frame}
	...
	\speech{SynchronmaschineVollpolgenerator}{1}{Wird eine Synchronmaschine nicht als Antrieb, sondern umgekehrt als Generator genutzt, spricht man vom Vollpohl-generator. Diese Generatoren zeichnen sich durch die Verwendung von Vollpohl-wicklungen aus, die eine gleichmäßige und symmetrische elektrische Leistung erzeugen. Bei Vollpohl-generatoren sind die Wicklungen so gestaltet, dass sie einen vollen magnetischen Pohl ausnutzen, was bedeutet, dass jeweils die gesamte Pohl-einheit für die Stromerzeugung genutzt wird. Diese Anordnung führt zu einer optimalen Nutzung des magnetischen Flusses. Hier dargestellt ist ein Ersatzschaltbild eines einphasigen Vollpohl-generators. Im Netzbetrieb sind die Klemmenspannung U-1 und die Frequenz Omega fest vorgegeben. Das ist bei der Synchronmaschine ein häufiger Betriebsfall.}
	\silence{4}
\end{frame}
