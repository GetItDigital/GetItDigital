...
\newvideofile{03KapazitaetundKondensatoren}{Kapazität und Kondensatoren}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% Anfang - Kapazität	 %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\begin{frame}%; TODO kapazitätundKondensator.txt steht im video.pdf  ist das ein problem?
	...
	\speech{03_Kapazitaet_und_Kondensator1}{1}{
		...
		Du kennst die wichtigsten Parameter rund um den Kondensator und die Kapazität und
		kannst die Kapazität eines einfachen Plattenkondensators berechnen.
	}
...
\begin{frame}[t]
	...
	\b{
		\begin{minipage}[t]{1\textwidth}
			\begin{equation*}
				[C] =  1\, \text{Farad} = 1\, \text{F} =  1\,\, \frac{\text{C}}{\text{V}} = 1\, \frac{\text{A} \text{s}}{\text{V}} \label{eq:cqu_1}
			\end{equation*}
			\vspace{0.1cm}
			\begin{equation*}
				C = \frac{Q}{U}
	...
	% ; TODODO: Hillgärtner: 	Die Folie enthält die Formel C = Q / U sowohl oben als auch unten, was redundant wirkt. 
	...
	\speech{03_Kapazitaet_und_Kondensator4}{1}{ %; TODO: only 1
		...
		Formelmäßig also: C gleich Q durch U.
		Alternativ in SI-Einheiten: Farad ist gleich Ampere-sekunde pro Volt.
	}

	\speech{03_Kapazitaet_und_Kondensator4}{2}{ %; TODO: only 2
		Wir können die Formel C ist gleich Q durch U auch umstellen zu Q ist gleich C mal U.
	}
...
\s{
	...
	\begin{equation*}
		[C] =  1\, \text{Farad} = 1\, \text{F} =  1\,\, \frac{\text{C}}{\text{V}} = 1\, \frac{\text{A} \text{s}}{\text{V}} \label{eq:cqu_2}
	\end{equation*}
	\vspace{0.1cm}
	\begin{equation}
		C = \frac{Q}{U}
...
\begin{frame}
	...
	\speech{03_Kapazitaet_und_Kondensator5}{1}{
		...
		Denn die Platten müssen ja voneinander isoliert sein. Das heißt, man wickelt dort ein Material zwischen die Platten ein in Elektrolyt getränktes Material. Das sind sogenannte Elektrolytkondensatoren. Das hat nur eine gewisse Dicke und hinzufolge auch nur eine gewisse Spannungsfestigkeit.
...
\s{
	...
	wie es im folgenden Schaltbild \ref{fig:idealer Kondensator_ESB_1} dargestellt ist.
...
\begin{frame}[t]
		...
		\only<2->{
			...
			\begin{figure}[H]
				\begin{center}
					...
					\label{fig:realer Kondensator_ESB_1}
				\end{center}
			\end{figure}
		}
	}

	\speech{03_Kapazitaet_und_Kondensator6}{1}{ % ; TODO: only 1
	...
	\speech{03_Kapazitaet_und_Kondensator6}{2}{ % ; TODO: only 2
		...
		Sodass wir hier im Prinzip gegensätzliche Bauteileigenschaften haben. In der Realität ist es tatsächlich so, dass ein Kondensator abhängig von seinem Aufbau irgendwann nicht mehr die gewünschte Kapazität schaltungstechnisch darstellt, sondern sich eher wie eine Induktivität verhält. Da die parasitäre Induktivität LS dominant wird. Und je größer das Bauelement geometrisch, desto schlechter ist dieses Verhältnis zwischen Nennkapazität und parasitärer Induktivität zueinander.
		D.h. die gerade dargestellten Elektrolytkondensatoren, die räumlich sehr groß sind, die kannst du tatsächlich nur bei relativ niedrigen Frequenzen verwenden. Da ist dann der kapazitive Anteil noch dominant. D.h. diese Kondensatoren werden z.B. benutzt, um Eingangsspannung zu stabilisieren. Wohingegen für hochfrequentere Anwendungen, Filterschaltung und so, kommen eher die ganz kleinen SMD-Bauteile zum Tragen, weil dann das kapazitive Verhalten des Kondensators bis hin zu höheren Frequenzen garantiert werden kann.
	}
...
\s{
	\begin{figure}[h]
		\begin{center}
			...
			\label{fig:idealer Kondensator_ESB_2}
		\end{center}
	\end{figure}
	...
	des Bauteils, nachbildet. Abbildung \ref{fig:realer Kondensator_ESB_1} zeigt ein solches Ersatzschaltbild.
...
\begin{frame}
	...
	\speech{03_Kapazitaet_und_Kondensator7}{1}{
		Wir können uns also merken, dass der Kondensator immer nur der verzweifelte Versuch ist, eine Kapazität nachzubilden. Wir hatten das ja schon beim Widerstand, dass das Bauteil Widerstand eben noch andere Eigenschaften hat, als nur die gewünschte Eigenschaft des  Widerstandes. So ist das letztlich beim Kondensator auch. Und diese ungewünschten Eigenschaften fallen insbesondere dann ins Gewicht, wenn wir zu höheren Frequenzen hingehen.
	}
...
\begin{frame}
	...
	\end{figure}
	%; TODO Volatisationseffekt -> Polarisationseffekte
...
\begin{frame}[t]
	...
	\speech{03_Kapazitaet_und_Kondensator9}{1}{
		Wir geben diese Möglichkeit der Erhöhung des elektrischen Feldes in einer elektrischen Flussdichte an über die Dielektrizitätszahlen. Die relative Dielektrizitätszahl gibt quasi an, wie viel das elektrische Feld verstärkt wird. Vakuum hat eine Dielektrizitätszahl von 1, Luft ziemlich genau von 1, wobei der häufig verwendete Kunststoff PE irgendwo so bei 2,3 liegt. Aber wir können auch Keramiken finden, die bis zu 300 als Dielektrizitätszahl haben oder Tantaloxid von 25 bis 30. Wasser ist ebenfalls ein sehr gutes Dielektrikum. Das Problem bei Wasser ist nur, dass es bei kleinsten Verunreinigungen bereits elektrisch leitfähig werden wird und damit diese die elektrischen Eigenschaften verliert.
	}
...
\begin{frame}[t]
	...
	\b{
		...
		\only<3->{
			\begin{equation*} %; TODO formel Wort Coulomb durch C ersetzen und gespeochenene text anch bauen (herleitung von As/m^2 zeigen)
	...
	\speech{03_Kapazitaet_und_Kondensator12}{1}{ % ; TODO: only 1
	...
	\speech{03_Kapazitaet_und_Kondensator12}{2}{ % ; TODO: only 2
	...
	\speech{03_Kapazitaet_und_Kondensator12}{3}{ % ; TODO: only 3
...
\begin{frame}[t]
	...
	\speech{03_Kapazitaet_und_Kondensator13}{1}{ % ; TODO: only 1
	...
	\speech{03_Kapazitaet_und_Kondensator13}{2}{ % ; TODO: only 2
		Wenn wir jetzt beide Seiten nach der Zeit T ableiten, also die Q nach die T ist gleich C mal die U nach die T.
	}

	\speech{03_Kapazitaet_und_Kondensator13}{3}{ % ; TODO: only 3
		So erhalten wir: d-Q nach d-T ist ja nichts anderes als I von T.
	}

	\speech{03_Kapazitaet_und_Kondensator13}{4}{ % ; TODO: only 4
		So dass wir schreiben können, der Strom I-C von T, der durch die Kapazität fließt, ist gleich C mal die UC von T nach dT.
		...
		Denn wenn wir jetzt eine Gleichspannung hätten, dann wäre die U-C nach d-T ja gleich 0. Und demzufolge ist der Strom I-C von T gleich 0.
	}
	...
	\speech{03_Kapazitaet_und_Kondensator13}{5}{ % ; TODO: only 5
...
\begin{frame}[t] %laden eines Kondensators
	...
	\b{
		...
		\begin{figure}[H]
			...
			\centering
			\input{Tikz/src/03_Kapazitaet_Kondensator/Laden_Kondensator_ESB_Schalter_oeffnen}
			%\end{subfigure}
	...
	\speech{03_Kapazitaet_und_Kondensator14}{1}{
		Ja, wenn wir jetzt betrachten, was passiert denn, wenn wir einen Kondensator, einen Widerstand in Reihe schalten und wir öffnen oder schließen einen Schalter, sodass die Spannungsquelle U-Q dieses System entweder mit Spannung versorgt oder nicht.
		...
		Da aber die Spannung am Kondensator wie erwähnt nicht springen kann, und U-C am Anfang 0 Volt beträgt, der Kondensator ist ungladen, fällt die gesamte Spannung am Widerstand ab.
	}

	\speech{03_Kapazitaet_und_Kondensator14}{2}{
		Zudem fließt am Anfang der Strom I-max.
		Mit zunehmender Zeit lädt sich der Kondensator auf.
		...
		Deshalb können auch vom Netz getrennte Schaltungen gefähtrlich sein, weil die verbauten Kondensatoren lange ihre Ladung halten können.
...
\begin{frame}
	...
	\speech{03_Kapazitaet_und_Kondensator15}{1}{ % ; TODO: only 2
		Die Spannungsquelle wird nun gegen einen Kurzschluss getauscht, und bei t gleich 0 wird der Schalter geschlossen.
		Die Spannung U-Q treibt den Strom I-max kurzzeitig durch die Schaltung und entlädt sich dabei, der Strom I wird kleiner und geht gegen
		Warum entläd sich der Kondensator? der Strom durch die Schaltung wir von der Spannung U-C also der Ladungsdifferenz der Platten
		Die Frage die sich hierbei stellt ist, wie lange dauert dieser entlade oder auflade Vorgang. Hierfür wird Tau verwendet. 1 Tau
		Beim entladen ist es ähnlich, 1 tau entspricht der Zeit, bis die Kondensatorspannugn auf 37 Prozent ihres Ursprungswert abfällt.
		Tau kann aber auch aus der Kapazität und dem Serienwiderstand berechent werden. Tau ist gleich R mal C.
	}
