...
\begin{frame}[t]
	...
	\b{
		\begin{minipage}[t]{1\textwidth}
			...
			\[
			\begin{aligned}
				[C] =  1\, \text{Farad} &= 1\, \text{F} =  1\,\, \frac{\text{C}}{\text{V}} = 1\, \frac{\text{A} \text{s}}{\text{V}} \\[10pt]
				C &= \frac{Q}{U}\\[10pt]
				Q &= C \cdot U \quad
			\end{aligned}\]
			\iffalse
...
\begin{frame}
	...
	\speech{03_Kapazitaet_und_Kondensator5}{1}{%; TODO sehr viele Ähms und langsam -> neuer seed
...
\begin{frame}[t]
	...
	\speech{03_Kapazitaet_und_Kondensator6}{2}{% ; TODO feintuning nötig
		...
		Sodass wir hier im Prinzip gegensätzliche Bauteileigenschaften haben. In der Realität ist es tatsächlich so, dass ein Kondensator abhängig von seinem Aufbau irgendwann nicht mehr die gewünschte Kapazität schaltungstechnisch darstellt, sondern sich eher wie eine Induktivität verhält. Da die parasitäre Induktivität L-S dominant wird. Und je größer das Bauelement geometrisch, desto schlechter ist dieses Verhältnis zwischen Nennkapazität und parasitärer Induktivität zueinander.
...
\begin{frame}[t]
	...
	\speech{03_Kapazitaet_und_Kondensator9}{1}{
		Wir geben diese Möglichkeit der Erhöhung des elektrischen Feldes in einer elektrischen Flussdichte an über die Dielektrizitätszahlen. Die relative Dielektrizitätszahl gibt quasi an, wie viel das elektrische Feld verstärkt wird. Vakuum hat eine Dielektrizitätszahl von 1, Luft ziemlich genau von 1, wobei der häufig verwendete Kunststoff PE irgendwo bei 2,3 liegt. Aber wir können auch Keramiken finden, die bis zu 300 als Dielektrizitätszahl haben oder Tantal-oxid von 25 bis 30. Wasser ist ebenfalls ein sehr gutes Dielektrikum. Das Problem bei Wasser ist nur, dass es bei kleinsten Verunreinigungen bereits elektrisch leitfähig werden wird und damit diese dielektrischen Eigenschaften verliert.
	}
	\silence{4}
\end{frame}
...
\begin{frame}
	...
	\b{
		...
		\begin{align*}
			...
			\boldsymbol\varepsilon_\mathrm{0} & : \text{Elektrische Feldkonstante (8.85421878 $\cdot10^{-12}\frac{As}{Vm}$)} \\
			\boldsymbol\varepsilon_\mathrm{r} & : \text{Relative Dielektrizitätszahl}                                        \\
		\end{align*}
	}

	\speech{03_Kapazitaet_und_Kondensator10}{1}{% ; TODO  sprachfehler vorhanden
		Wofür brauchen wir jetzt die relative Dielektrizitätszahl? Die brauchen wir zum Berechnen von der Dielektrizitätskonstante Epsilon. Diese hat die Einheit Farad pro Meter, denn sie gibt letztlich an, wie stark das Dielektrikum das elektrische Feld verstärkt. Und sie setzt sich zusammen aus Epsilon null und Epsilon r. Epsilon null ist die elektrische Feldkonstante  8 komma 8-5 mal 10 hoch minus 12 Farad pro Meter oder Ampere-Sekunden pro Volt-Meter und Epsilon r ist die einheitenlose elektrische Dielektrizitätszahl.
	}
...
\begin{frame}[t]
	...
	\speech{03_Kapazitaet_und_Kondensator12}{2}{
		Im statischen Fall, das heißt, wir betrachten erstmal nur Gleichspannung und Gleichstrom, dabei kann die Polarisation P eigentlich vernachlässigt werden.
	}
