\newvideofile{Modul03SpannungStromquellen1}{Spannungs- und Stromquellen 1}
...
\s{
    ...
    Energie kann nur gewandelt, nicht erzeugt werden. Dennoch wird in der Elektrotechnik das Zuführen von Energie in den zu betrachtenden Stromkreisen mit Hilfe von Spannungs- bzw. Stromquellen  beschrieben.
    ...
    Im Folgenden werden die Spannungs- und Stromquellen beschrieben und die jeweiligen Unterschiede erläutert.}


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% Lernziele für Spannungs- und Stromquellen %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%  \begin{Lernziele}{Spannungs- und Stromquellen}
...
\v{
    ...
    \begin{Lernziele}{Spannungs- und Stromquellen}
        Du kannst
        \begin{itemize}
            \item zwischen Spannungs- und Stromquellen unterscheiden und kennst deren Schaltsymbole.
            \item reale Spannungs- und Stromquellen modellieren.
            \item Beispiele für verschiedene Quellen benennen.
        \end{itemize}
    \end{Lernziele}
    
    \speech{02_Spannungs_und_Stromquellen1}{1}{
        In dem nun folgenden Abschnitt des Moduls werden wir uns mit Spannungs- und Stromquellen beschäftigen. Diese sind sehr wichtig.
        ...
        Am Ende dieses Abschnittes solltest du den Unterschied zwischen Strom- und Spannungsquellen kennen und die dazugehörigen Schaltungssymbole.
        Du kennst danach auch, wie reale Spannungs und Stromquellen modelliert werden und kannst verschiedene Beispiele für diese Quellen benennen.
        }
    \end{frame}
}
...
\s{
    ...
    Im idealen Fall zeichnet sich dadurch aus, dass sie unabhängig vom angeschlossenen Verbraucher eine konstante Spannung ausgibt.
...
\begin{frame}[t]
    \ftx{Die ideale Spannungsquelle}
    \b{
        \begin{itemize}
            \item Die ideale Spannungsquelle liefert stets eine konstante Spannung $U_\mathrm{q}$, unabhängig von der Last oder dem Strom
        \end{itemize}
    }
    \begin{figure}[H]
        \centering
        \b{\input{Tikz/src/02_Spannungs_Stromquellen/Ideale_Spannungsquelle_kurzschluss}}
        \s{\input{Tikz/src/02_Spannungs_Stromquellen/Ideale_Spannungsquelle}
            \caption{\textbf{Ideale allgemeine Spannungsquelle.} Liefert stets eine konstante Spannung $U_\mathrm{q}$, unabhängig von der Last und passt den Strom automatisch an, um die konstante Spannung aufrechtzuerhalten.}}
    ...
    \b{
        \only<3->{
            \begin{itemize}
                \centering
                \item $ U_\mathrm{q} = R \cdot I \Rightarrow \lim\limits_{R \to 0} \frac{U_\mathrm{q}}{R} = \infty$
            \end{itemize}
        }
    }
    \speech{02_Spannungs_und_Stromquellen2}{1}{
        ...
        Eine Spannungsquelle liefert eine konstante Quellspannung, U-Q und zwar völlig egal, wie groß der Strom ist, der fließen muss,
        um die angeschlossene Last zu versorgen. Abhängig davon, was wir jetzt hier an diesen Klemmen anhängen, welcher Widerstand,
        stellt sich entsprechend ein Strom I ein, wobei die Ausgangsspannung, U-Q in jedem Fall immer gleich bleiben wird.
        Das ist eben genau die ideale Spannungsquelle.
    }
    \silence{3}

    \speech{02_Spannungs_und_Stromquellen2}{2}{Was würde jetzt passieren, wenn wir an die Ausgangsklemmen einen Kurzschluss anschließen würden? Ein Kurzschluss ist definitionsgemäß eine Verbindung mit einem Widerstand von 0 Ohm.}

    \speech{02_Spannungs_und_Stromquellen2}{3}{
        Wenn wir jetzt auf das Ohmsche Gesetz schauen, U ist gleich R mal I beziehungsweise I gleich U durch R, und wir teilen die Spannung U-Q durch 0 Ohm,
        dann erhalten wir einen unendlichen Strom. Und genau das tut diese sogenannte ideale Spannungsquelle im Falle eines Kurzschluss.
        ...
        Wenn wir uns anschauen, wie hier Strom und Spannung gepfeilt sind, dann stellen wir fest,
        dass sie entgegengesetzt gezeigt werden, also: Strom I und Spannung U-Q zeigen in entgegengesetzte Richtung.
        ...
        Zusammengefasst:
        Die Quellspannung U-Q ist bei der idealen Spannungsquelle immer konstant. Egal welche Last angeschlossen ist.
        Dies bedeutet aber auch: Bei einem Kurzschluss würde die abgegebene Leistung unendlich sein, was verhindert werden muss.
    }
...
\s{
    ...
    dann würde die Quelle versuchen die Spannung aufrechtzuerhalten und es würde ein unendlich hoher Strom fließen.
    Warum dies so gefährlich ist, wird im Abschnitt \ref{sec:mod re spannungsquelle} anhand einer realen Spannungsquelle deutlich.
}
...
\begin{frame}[t]
    \ftx{Die realle Spannungsquelle}
    \b{
        \begin{itemize}
            \item Bei der reallen Spannungsquelle wird die Spannung $U_\mathrm{q}$ durch den Innenwiderstand \(R_\mathrm{i}\) beeinflusst und ist an den Ausgangsklemmen dadurch geringer
        \end{itemize}
    ...
    \begin{figure}[H]
      ...
      \begin{itemize}
        \centering
        \item $ \displaystyle U_\mathrm{q} = U_\mathrm{R_i} + U $
      \end{itemize}
    \end{figure}
         }
        \s{
        ...
        \centering
        \input{Tikz/src/02_Spannungs_Stromquellen/Reale_Spannungsquelle}
        \caption{\textbf{Reale Spannungsquelle mit Innenwiderstand.} Die Spannung $U_\mathrm{q}$ wird durch den Innenwiderstand \(R_\mathrm{i}\) beeinflusst und ist an den Ausgangsklemmen dadurch geringer.}
        \label{fig:re spannungsquelle}
        \end{figure} 
        % \begin{equation*}
        %     U_\mathrm{q} = U_\mathrm{R_i} + U
        % \end{equation*}
         }
   
    %speech:neuer seed bei "einfügt" wir eineinfügt gesagt
    \speech{02_Spannungs_und_Stromquellen3}{1}{
        ...
        An diesen internen Leitern fällt eine Spannung U-R-I ab abhängig vom fließenden Strom.
        Wie kann man nun reale Spannungsquellen mit Hilfe der idealen Spannungsquelle modellieren?
        Genau indem man zusätzlich einen Innenwiderstand R-I einfügt.
        Abhängig vom Strom fällt ein Teil der Quellspannung U-Q am Innenwiderstand R-I ab.
        Das bedeutet: Die tatsächlich nutzbare Ausgangsspannung U ist kleiner als U-Q, um genau den Betrag U-R-I.
        Wenn wir die Spannungsbilanz aufstellen ergibt sich:
        U-Q ist gleich U-R-I plus U.
        Und was passiert bei einem Kurzschluss?
        Dann würde der gesamte Strom nur durch den Innenwiderstand R-I begrenzt werden und die gesamte Spannung U-Q fällt am innenwiderstand ab, was diesen extrem aufheizen würde.
        Die Quelle würde sehr heiß und im schlimmsten Fall zerstört werden.
        Wie sollte der Innenwiderstand R-I also idealerweise sein?
        Möglichst klein im Idealfall 0 Ohm dann verhält sich die reale Quelle wie eine ideale.
        In der Praxis erreichen wir durchaus sehr kleine Werte. Zum Beispiel 1 milliOhm bei guten Autobatterien.
        Aber auch dann ist ein Kurzschluss keine gute Idee, denn bei 12 Volt und 1 milliOhm Innenwiderstand würden theoretisch 12.000 Ampere fließen.
        Das zeigt: Auch reale Spannungsquellen müssen geschützt werden, insbesondere gegen Kurzschlüsse.
    }
...
\begin{frame}
    ...
    \begin{Merksatz}{Spannungsquelle}
    \begin{itemize}
    ...
    \speech{02_Spannungs_und_Stromquellen4}{1}{
        Wir können also für die Spannungsquelle folgende beiden Merksätze zusammenfassen:
        erstens, Eine Spannungsquelle, egal ob real oder ideal, darf nie kurzgeschlossen werden,
        weil ansonsten unendlich hohe bzw. immer noch sehr hohe Ströme fließen werden.
        Zweitens, stellt sich bei der Spannungsquelle immer der Strom ein.
        Die Spannung bleibt im Fall der idealen Spannungsquelle immer gleich,
        wobei die Spannung bei der realen Spannungsquelle sich um den Spannungsabfall am Innenwiderstand reduziert.
    }
...
\begin{frame}[t]
    \ftx{Die ideale Stromquelle}
    \b{
        \begin{itemize}
            \item Die ideale Stromquelle liefert stets einen konstanten Strom $I_\mathrm{q}$, unabhängig von der Last und passt die Spannung automatisch an, um den Strom aufrechtzuerhalten
        \end{itemize}
    }
    \begin{figure}[H]
        \centering
    ...
    \speech{02_Spannungs_und_Stromquellen5}{1}{
        Wir haben die Spannungsquelle kennengelernt. Das ist auch das, was wir aus unserem bisherigen Alltag am besten kennen, weil die meisten Quellen, die wir verwenden, sich wie Spannungsquellen verhalten. Dennoch gibt es auch in der Realität Stromquellen. Hier dargestellt als ideale Stromquelle. Und bei der idealen Stromquelle ist der Strom, der fließt, immer gleich. Das heißt, der Quellstrom I-Q fließt tatsächlich völlig unabhängig davon, welche Lasten wir einstellen. Die Ausgangsspannung, stellt sich eben entsprechend der Last ein.
        ...
        Das ist etwas, das wir aus unserem alltäglichen Leben eigentlich kaum kennen. Warum? Weil wir eigentlich immer mit Spannungsquellen zu tun haben, die wir eben nicht kurzschließen dürfen, aber durchaus leer laufen lassen dürfen. Bei einer Stromquelle ist es genau umgekehrt. Die benutzen wir vor allem zum Modellieren von verschiedenen Sachen. Und ein Beispiel für eine Quelle, die wir sehr gut als Stromquelle modellieren können, ist zum Beispiel der Blitz. Bei einem Blitz haben wir einen eingeprägten Strom, der eben durch die Entladung zwischen Wolke und Erde entsteht. Der Strom sind 50 kilo Ampere. Die Spannung des Blitzes stellt sich dann entsprechend der Last ein. Und das ist der Grund, warum zum Beispiel Blitzschutzeinrichtungen an Gebäuden möglichst niederohmig sein sollten, also sehr kleine Widerstände haben sollten, damit die Spannung, die sich daran aufbaut, möglichst klein ist.
...
\subsection{Modellierung realer Stromquellen}
\s{
...
\begin{frame}[t]
    ...
    \b{
        \begin{itemize}
            \item Bei der reallen Stromquelle mit Innenwiderstand $R_\mathrm{i}$ wird der Strom $I_\mathrm{q}$ beeinflusst und ist an den Ausgangsklemmen geringer
        \end{itemize}       
    }
    \begin{figure}[H]
        \centering
    ...
    \b{ \begin{itemize}
        \centering
            \item $ \displaystyle I = I_{\mathrm{q}} - I_{\mathrm{R_{\mathrm{i}}}} = I_{\mathrm{q}} - \frac{U}{R_{\mathrm{i}}} $
        \end{itemize} }
    ...
    \speech{02_Spannungs_und_Stromquellen6}{1}{
        ...
        Wenn jetzt die angeschlossene Last einen sehr viel kleineren Widerstand hat als der Innenwiderstand R-I, dann fließt ein Großteil des Quellstroms durch die Last, und ein sehr viel kleinerer Teil des Stroms durch den Innenwiderstand. Du wirst in den folgenden Modulen auch noch die Knotenregel kennenlernen und sehen, dass der Quellstrom I-Q genau gleich der Summe aus I und I-R-I ist.
        Wie muss also der Innenwiderstand sein, damit eine Stromquelle möglichst ideales Verhalten zeigt? Der Innenwiderstand einer Stromquelle sollte möglichst groß sein. Auch das widerspricht dem natürlichen Gefühl. Da sagt man normalerweise: Innenwiderstände sollten klein sein, damit die Verluste klein sind. Hier ist es genlau umgekehrt. Der Innenwiderstand sollte möglichst groß sein. Achtung: Bei der realen Stromquelle ist ein möglichst großer Innenwiderstand absolut wünschenswert. Denn Wenn der Innenwiderstand klein wäre und wir dann eine Last anschließen würden, würde der meiste Strom durch den Innenwiderstand fließen und nicht durch die Last.
        %die Quelle leerlaufen lassen, flösse der gesamte Strom I-Q durch den Innenwiderstand. Wir hätten einen ähnlichen Effekt wie beim Kurzschluss der realen Spannungsquelle, im R-I würde eine extrem hohe Leistung umgesetzt, was zur Zerstörung der Quelle führen würde.
    }
    \silence{2}
...
\begin{frame}
    ...
    \speech{02_Spannungs_und_Stromquellen7}{1}{
        ...
        Bei der Stromquelle ist der Strom immer derselbe, völlig egal, welche Last wir anschließen, bzw. bei der realen Stromquelle ist der Strom abhängig vom Verhältnis der Widerstände. Die Spannung, die wir an der Klemme abgreifen können, stellt sich entsprechend ein.
    }
...
\newvideofile{Modul03SpannungStromquellen2}{Spannungs und Stromquellen 2}
...
\s{
    ...
    Abbildung \ref{fig:Umrechnung2} zeigt die Überführung der Schaltbilder ineinander.
    Weiterführende Inhalte zur Umrechnung und Leistungsanpassung folgen in Modul 4.
}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% Ende - Umrechnen Strom in Spannungsquelle %%%l%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\begin{frame}
    ...
    \b{
        \begin{figure}[H]
            ...
            \input{Tikz/src/02_Spannungs_Stromquellen/Umwandlung_Gleichspannung_Strom_beamer}

            \label{fig:Umrechnung1}

        \end{figure}
    }
    \speech{02_Spannungs_und_Stromquellen8}{1}{
        Wir haben jetzt gesehen, dass es Strom- und Spannungsquellen gibt. Und letztlich haben beide an ihren Klemmen einen Ausgangsstrom I und eine Ausgangsspannung U. Bei realen Quellen wird das tatsächliche Verhalten maßgeblich vom Innenwiderstand R-I mitbestimmt Es ergibt sich für eine Spannungsquelle das folgende Bild. Wir haben eine Quellspannung U-q und einen Strom I, der sowohl durch den Innenwiderstand als auch durch den angeschlossenen Widerstand fließt, und demzufolge eine Spannung U, die an dem Widerstand R abfällt.
        %Letztlich kommt es also darauf an, welcher Strom I durch den Lastwiderstand R fließt und dann einen Spannungsabfall U hervorruft.
        Das können wir so wie hier dargestellt mit einer Spannungsquelle natürlich erreichen.}
\silence{4}
        %guter seed
    \speech{02_Spannungs_und_Stromquellen8}{2}{
        Wir können aber genau denselben Spannungsabfall U und genau denselben Strom I durch eine Stromquelle mit einem parrallelen Widerstand R-I erreichen. Maßgeblich für die Umrechnung dieser beiden Quellen ist, dass sie dasselbe Verhalten an der Last zeigen. Das heißt, der Strom I und die Spannung U an dem Lastwiderstand R für beide Quellen gleich ist.
        Wie kann man das jetzt erreichen?
        }
\silence{4}
    \speech{02_Spannungs_und_Stromquellen8}{3}{
        Wir betrachten zuerst die Spannungsquelle auf der linken Seite und berechnen die Spannung U, die an dem Lastwiderstand abfällt. Da es eine Serienschaltung aus R und R-I ist, berechnet sich die Spannung einfach über den Spannungsteiler U ist gleich U-q mal R durch R-i plus R.
        Wir haben nun auch die Anforderung, das der Strom durch die beiden Lastwiderstände gleich sein soll.
        Das erreicht man, indem die selbe Spannung die über den Lastwiderstand bei der Spannungsquelle abfällt auch in der Parallelschaltung der Stromquelle auf der rechten Seite anliegt.
    }
    \silence{2}
    \speech{02_Spannungs_und_Stromquellen8}{4}{
        Wir erinnern uns aber, bei einer Stromquelle wird der Strom i-q vorgegeben und die Spannung stellt sich ein. Welchen Strom benötigen wir also? Richtig. Wir stellen den Strom ein, der durch die Parallelschaltung von Last und innenwiderstand der Stromquelle fließen würde, wenn die Spannung U anliegen würde.
        Das ist der Strom i-q ist gleich U durch R parallel R-i    .}
\silence{2}

    \speech{02_Spannungs_und_Stromquellen8}{5}{
        Wenn wir nun unsere Spannungsteilerformel für U an der Spannungsquelle in die Formel für I-q einsetzen, können wir einiges kürzen und erhalten das Ergebnis i-q ist gleich u-q geteilt durch R-i.}
        %Wenn nun R-i bei beiden Quellen gleich ist, sehen wir, das diese Quellen nun komplett äquivalent sind.
        %Man rechnet einfach und stellt dann bei den Rechnungen fest, dass I-Q gleich U-Q durch R-I sein muss, wobei R-I in beiden Fällen gleich ist. Wenn das gegeben ist, sind diese beiden Quellen komplett äquivalent.}


    \iffalse
        originaler sprechtext ohne Mathematische herleitung
        \speech{02_Spannungs_und_Stromquellen8}{2}{
            Wir können aber genau denselben Spannungsabfall U und genau denselben Strom I durch eine Stromquelle mit einem parrallelen Widerstand R-I erreichen. Maßgeblich für die Umrechnung dieser beiden Quellen ist, dass sie dasselbe Verhalten an der Last zeigen. Das heißt, der Strom I und die Spannung U an dem Lastwiderstand R für beide Quellen gleich ist.
            Wie kann man das jetzt erreichen? Man rechnet einfach und stellt dann bei den Rechnungen fest, dass I-Q gleich U-Q durch R-I sein muss, wobei R-I in beiden Fällen gleich ist. Wenn das gegeben ist, sind diese beiden Quellen komplett äquivalent.
        }
    \fi
    %; Done: Hillgärtner: Hier steht jetzt 0 ist gleich RI mal I0. I0 taucht nirgendwo auf. Das kann also schon mal nicht sein. Das müsste dann I sein. Oder aber es bezieht sich auf die Stromquelle. Das ist dann aber auf der Folie 68 sehr verwirrend, muss man dazu sagen. Ferner liegt die Spannung U an dem Widerstand R an. Ich würde die dann, wenn, UR nennen. Das ist eindeutiger vielleicht. Oder UA für Ausgangsspannung. Und vielleicht auch den Widerstand nicht zwischen den beiden offenen Kreisen legen, sondern nochmal eine Drahtbrücke nach rechts legen, dass man deutlich sieht, dass das die Ausgangsspannung U an den Punkten belassen. Weil so könnte man meinen, dass die Formel U0 ist gleich RI mal I0 mal U. Das gibt ja überhaupt keinen Sinn. Also das Bild muss bitte nochmal angepasst werden. 
    % ; done: Hillgärtner: Die Berechnung dazu oder die Berechnungen dazu sind jetzt hier nicht aufgeführt. Das fände ich aber durchaus sehr sinnvoll. Das macht vieles klarer. Das kann ich nicht alles über die Tonspur bringen, glaube ich. 
...
\s{
    \begin{figure}[H]
        ...
        \label{fig:Umrechnung2}%%ist gleiches bild wie Label{Urechnung1} 
...
\begin{frame}
    ...
    \b{
        % \begin{itemize}
        % \item $I=\frac{U_\mathrm{0}}{R_\mathrm{i}+R_\mathrm{L}}$ ; \quad $U_\mathrm{R_L}=\frac{U_\mathrm{0}}{R_\mathrm{L}+R_\mathrm{i}}\cdot R_\mathrm{L}$
        %  \end{itemize}
        \begin{figure}[H]

            \hspace{-2cm}
            \begin{minipage}{0.9\textwidth}
                \input{Tikz/src/02_Spannungs_Stromquellen/graph_spannung_strom_Ri_1.tex}
            \end{minipage}
            \hspace{-1cm}
            \begin{minipage}{0.08\textwidth}
                \scalebox{0.7}{
                    \input{Tikz/src/02_Spannungs_Stromquellen/reale_spannungsquelle_neben_graph.tex}
                }%scalebox ende
            \end{minipage}
        \end{figure}
    }
    
    \speech{02_Spannungs_und_Stromquellen9}{1}{
        Das ermöglicht uns aber auch, reale Quellen zu bewerten, ob sie sich eher wie eine Spannungsquelle oder eher eine Stromquelle verhalten. Wir nehmen mal als Beispiel einen reale Spannungsquelle mit einem variablen Lastwiderstand von 100 bis 1 kilo Ohm und einen Innenwiderstand von einem Ohm. Das heißt, der Lastwiderstand ist sehr, sehr viel größer als der Innenwiderstand. Damit haben wir tatsächlich eher ein Spannungsquellenverhalten.
    }
    %; todo viel zu schnell
    \speech{02_Spannungs_und_Stromquellen9}{2}{
        Denn wenn jetzt der Innenwiderstand ein Ohm ist und der Lastwiderstand einhundert Ohm beträgt, dann fließt bei U-0 gleich zehn Volt etwa ein Strom von hundert milli-Ampere. Wenn wir jetzt den Lastwiderstand auf ein kilo-Ohm erhöhen, dann ändert sich der Strom, in rot zu sehen, aber die Spannung in blau, die an dem Lastwiderstand abfällt, bleibt, wie im graphen zu sehen ist, nahezu konstant bei etwa Zehn Volt.
        Das heißt immer dann, wenn der Lastwiderstand groß ist im Vergleich zum Innenwiderstand, haben wir eher Spannungsquellenverhalten. Völlig egal, ob wir die Schaltung einer Strom oder Spannungsquelle haben.
        Wenn wir nun den linken Teil des graphen betrachten, sehen wir die Spannung U-R fällt bei kleinen Lastwiderständen ab. Das liegt daran, das der Lastwiderstand so klein wird, dass der großteil der Spannung nun am innenwiderstand abfällt.
        Wenn der Lastwiderstand R Null Ohm hat, also ein Kurzschluss ist, dann fällt die gesamte Spannung am Innenwiderstand R-i ab. . .
    }
    \silence{4}
\end{frame}
...
\begin{frame}
    ...
    \b{
        ...
        \begin{figure}[H]
            ...
            \hspace{-2cm}
            \begin{minipage}{0.9\textwidth}
                \input{Tikz/src/02_Spannungs_Stromquellen/graph_spannung_strom_Ri_1k.tex}
            \end{minipage}
            \hspace{-1cm}
            \begin{minipage}{0.08\textwidth}
                \scalebox{0.7}{
    ...
    \speech{02_Spannungs_und_Stromquellen10}{1}{
        Umgekehrt, wenn der Lastwiderstand klein ist gegenüber dem Innenwiderstand, dann haben wir eher Stromquellenverhalten.
        Nehmen wir als Beispiel, einen Innenwiderstand von R-i ist gleich 1 kilo Ohm, dann sehen wir bei unterschiedlichen Lastwiderständen R, dass der der Strom nahezu konstant bleibt.
        %Das heißt, der Strom stellt sich entsprechend der Last ein und die Ausgangsspannung bleibt nahezu konstant, weil sie überwiegend am Innenwiderstand abfällt – auch da wieder völlig unabhängig davon, ob wir die linke oder die rechte Schaltung haben.
        Wir haben nun also eine Spannungsquelle mit einem sehr großen innenwiderstand. Durch diesen großen innenwiderstand verhält sie sich aber eher wie eine Stromquelle, da der Lastwiderstand klein im Vergleich zum Innenwiderstand ist.
...
\s{
    ...
    In Abbildung \ref{fig:quellen} sind die Schaltzeichen der verschiedenen Quellen dargestellt.\vspace{0.2cm}
}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% Übersicht der verschiedenen Schaltzeichen von Spannungs- und Stromquellen %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

%%%%%%%%%% neue Variable für diesen Fall
\newlength{\mpwidth}
\setlength{\mpwidth}{0.18\textwidth}
%\newlength{\mpheight}
%\setlength{\mpheight}{3.8cm}

\begin{frame}[t]
    \ftx{Verschiedene Spannungs- und Stromquellen}
    \b{
        \begin{figure}[H]
            ...
            \uncover<3->{\begin{minipage}[t]{\mpwidth}
                    \input{Tikz/src/02_Spannungs_Stromquellen/Allgemeine_Stromquelle}
                    %\captionsetup{skip=-7pt}
                    \caption*{\footnotesize Allgemeine\\ Stromquelle}
                \end{minipage}
                \hfill
                \hspace{3.9cm}
                \begin{minipage}[t]{\mpwidth}
                    ...
                    \caption*{\footnotesize Wechselstromquelle}
                \end{minipage}
                \hfill
                \begin{minipage}[t]{\mpwidth}
                    \input{Tikz/src/02_Spannungs_Stromquellen/Zeitlich_veraenderliche_Stromquelle}
                    %\captionsetup{skip=-5pt,width=1.3\textwidth,justification=raggedright, singlelinecheck=false}
                    \caption*{\footnotesize Zeitlich beliebig veränderliche Stromquelle}
                \end{minipage}}

        \end{figure}
    }% nur Folie

    \speech{02_Spannungs_und_Stromquellen11}{1}{
        Wir haben bisher das Symbol kennengelernt für eine allgemeine Spannungsquelle.
        Das heißt, bei diesem Symbol, was du hier siehst, ein Kreis, durch den senkrecht ein Strich geht,
        das bezeichnet eine allgemeine Spannungsquelle.
        Mit diesem Symbol wird nicht gesagt, welche Form die Spannung hat.
        Ob es sich um eine Gleichspannung, eine Wechselspannung oder eine zeitlich veränderliche Spannung handelt, das kann alles sein. .
    }

    \speech{02_Spannungs_und_Stromquellen11}{2}{
    ...
    \speech{02_Spannungs_und_Stromquellen11}{3}{
        ...
        Wenn wir uns die allgemeinen Darstellungen der Spannungs- und Stromquellen noch einmal anschauen,
...
\s{
    \begin{figure}[H]
        ...
        \alttext{Übersicht mit sieben Schaltsymbolen für Quellen. Obere Reihe: allgemeine Spannungsquelle mit markierter Spannung u von t, Gleichspannungsquelle mit konstanter Spannung U, Wechselspannungsquelle mit sinusförmigem Symbol und Spannung u von t, sowie eine zeitlich beliebig veränderliche Spannungsquelle mit Spannung u von t. Untere Reihe: allgemeine Stromquelle mit markiertem Strom i von t, Wechselstromquelle mit sinusförmigem Symbol und Strom i von t, sowie eine zeitlich beliebig veränderliche Stromquelle mit Strom i von t. Unter jedem Symbol steht die jeweilige Bezeichnung.}
        \label{fig:quellen}
    \end{figure}
}
