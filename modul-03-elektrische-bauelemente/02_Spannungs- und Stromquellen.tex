...
\begin{frame}
    ...
    \begin{figure}[H]
        ...
        \s{\caption{\textbf{Ideale allgemeine Spannungsquelle.} Liefert stets eine konstante Spannung $U_\mathrm{q}$, unabhängig von der Last und passt den Strom automatisch an, um die konstante Spannung aufrechtzuerhalten.}}
    ...
    \speech{02_Spannungs_und_Stromquellen2}{1}{
        ...
        \speech{02_Spannungs_und_Stromquellen2}{2}{
...
\begin{frame}
    ...
    \speech{02_Spannungs_und_Stromquellen5}{1}{%;TODO neuer seed, stimme klingt anders
        ...
        Das ist etwas, das wir aus unserem alltäglichen Leben eigentlich kaum kennen. Warum? Weil wir eigentlich immer mit Spannungsquellen zu tun haben, die wir eben nicht kurzschließen dürfen, aber durchaus leer laufen lassen dürfen. Bei einer Stromquelle ist es genau umgekehrt. Die benutzen wir vor allem zum Modellieren von verschiedenen Sachen. Und ein Beispiel für eine Quelle, die wir sehr gut als Stromquelle modellieren können, ist zum Beispiel der Blitz. Bei einem Blitz haben wir einen eingeprägten Strom, der eben durch die Entladung zwischen Wolke und Erde entsteht. Der Strom sind 50 kilo Ampere. Die Spannung des Blitzes stellt sich dann entsprechend der Last ein, die wir haben. Und das ist der Grund, warum zum Beispiel Blitzschutzeinrichtungen an Gebäuden möglichst niederohmig sein sollten, also sehr kleine Widerstände haben sollten, damit die Spannung, die sich daran aufbaut, möglichst klein ist.
...
\begin{frame}
    ...
    %; TODO die begründung für den möglichst großen Innenwiderstand eienr Stromquelle ist falsch
    \speech{02_Spannungs_und_Stromquellen6}{1}{%;TODO schlimmer seed
...
%; TODO hier fehlen only abbildungen die zum sprechtext passen
\begin{frame}
    ...
    %; TODO: only 1 , bzw. ich bin nicht sicher ob der Text zur Umwandlung von Strom in Spannungsquelle geeignet ist
    \speech{02_Spannungs_und_Stromquellen8}{1}{
        ...
        Es ergibt sich für eine Spannungsquelle das folgende Bild: Wir haben eine Ausgangsspannung U-0. Wir haben einen Strom I, der sowohl durch den Innenwiderstand als auch durch den angeschlossenen Widerstand fließt, und demzufolge eine Spannung U, die an dem Widerstand R abfällt.
    ...
    \speech{02_Spannungs_und_Stromquellen8}{2}{
        Wir können aber genau denselben Spannungsabfall U und genau denselben Strom I durch eine Stromquelle mit einem Widerstand R-I erreichen. Maßgeblich für die Umrechnung dieser beiden Quellen ist, dass sie dasselbe Verhalten an der Last zeigen. Das heißt, der Strom I und die Spannung U an dem Lastwiderstand R für beide Quellen gleich ist.
        Wie kann man das jetzt erreichen? Man rechnet einfach und stellt dann bei den Rechnungen fest, dass I-0 gleich U-0 durch R-I sein muss, wobei R-I in beiden Fällen gleich ist. Wenn das gegeben ist, sind diese beiden Quellen komplett äquivalent.
    }
...
%; TODO only 3 fehlt in "Umwandlung_Gleichspannung_Strom.tex" muss weiter eingeteilt werden in kleinere onlys und einzelne textelemente
...
\begin{tikzpicture}
    ...
    \centering
    \begin{axis}[
        xlabel={Widerstand $R_\mathrm{L}$ / $\Omega$},
        ylabel={Spannung $U_\mathrm{R_L}$ / $V$},
        ...
        samples=200,
        legend style={
            at={(0.83,0.95)},      % Position der Legende (x,y)(unterhalb)
    ...
    \end{axis}
\only<2->{
    % --- Rechte Achse nur für Skala (Strom) ---
    \begin{axis}[
        ylabel={Strom $I$ / mA},
        ...
        scaled y ticks=false,
        legend style={
        at={(0.83,0.83)},      % Position der Legende (unterhalb)
        anchor=north,          % Bezugspunkt
        legend columns=1,      % <-- EINSPALTIG = untereinander
        draw=none              % kein Rahmen (optional)
        }
    ]
        \addplot[red, thick, dashed, domain=0:1000, samples=200]{10/(1e3+x)*1e3}; % in mA
        \addlegendentry{$I(R_\mathrm{i}=1\,\mathrm{k}\Omega)$}
    \end{axis}
}
\end{tikzpicture}
    ...
    \speech{02_Spannungs_und_Stromquellen9}{1}{
        ...
        Denn wenn jetzt der Innenwiderstand 1 Ohm ist, der Lastwiderstand 100 Ohm, dann fließt bei 10 Volt U-0 ziemlich genau ein Strom von 100 mA – genau genommen 99,9 mA. Wenn wir jetzt den Lastwiderstand erhöhen auf 1 kOhm, dann ändert sich der Strom, aber die Spannung, die an dem Lastwiderstand abfällt, bleibt nahezu gleich bei nahezu 10 Volt.
...
\begin{frame}
    ...
    \speech{02_Spannungs_und_Stromquellen10}{2}{ 
    ...
    \speech{02_Spannungs_und_Stromquellen10}{3}{ 
